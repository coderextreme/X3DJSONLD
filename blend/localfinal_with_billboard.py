...
def handle_billboard(element, parent_object):
    try:
        billboard_empty = bpy.data.objects.new(element.get('DEF', 'Billboard'), None)
        bpy.context.collection.objects.link(billboard_empty)

        # Position the Billboard correctly based on parent transformations
        translation, rotation, scale, center = parse_transform(element)
        billboard_empty.location = translation

        if parent_object:
            billboard_empty.parent = parent_object

        # For each child node of the Billboard, handle them accordingly
        for child in element:
            if child.tag.endswith('Shape'):
                create_shape(child, billboard_empty)
            elif child.tag.endswith('Transform'):
                create_transform(child, billboard_empty)
            # Add other child node handlers if needed
...
def parse_x3d_file_with_billboard(filepath):
    try:
        tree = ET.parse(filepath)
        root = tree.getroot()

        for element in root.iter():
            if element.tag.endswith('Billboard'):
                handle_billboard(element, None)
            elif element.tag.endswith('Transform'):
                create_transform(element, None)
            # Handle other elements as needed

    except Exception as ex:
        print(f"Error parsing X3D file: {ex}")

# Example usage:
parse_x3d_file_with_billboard('/mnt/data/Jin20fBillboarded5.x3d')
