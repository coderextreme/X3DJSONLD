...
function printSubArray(attrType, type, values,  co, j, trail) {
	...
	for (var i = 0; i < values.length; i += 840) {
		...
		codeno++;
		code[co] = "private "+type+"[] "+type+"_array_"+co+"() {\n";
		code[co] += "\treturn new "+type+"[] {"+values.slice(i, max).join(j)+trail+"}\n";
		code[co] += ";\n}\n";
		if (attrType.startsWith("MF")) {
			str += ".append("+type+"_array_"+co+"())";
		} else {
			str += ".setValue("+type+"_array_"+co+"())";
		}
		co = codeno;
	}
	return str;
