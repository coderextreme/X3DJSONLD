...
Features Expanded:
  ...
  • FULL HANIM SKINNING SUPPORT (NEW): HAnimHumanoid + HAnimJoint.skinCoordIndex / skinCoordWeight.
    ...
    • Adds JOINTS_0 + WEIGHTS_0 attributes + skin index to every primitive that uses skinCoord indices.
"""
...
def compute_world_matrices(gltf_nodes):
    ...
    def get_local_matrix(node):
        ...
        # Quaternion → rotation matrix (column-major, matches glTF)
        qx, qy, qz, qw = r
...
# ---------------------------------------------------------------------------
# Mesh Primitives (NOW records original skinCoord indices for later skinning)
# ---------------------------------------------------------------------------
...
def process_mesh_primitives(geom_nodes, gltf, bin_blob, def_map, mat_list, base_path, temp_skin_info):
    ...
    for node in geom_nodes:
        ...
        for face_set in facesets:
            ...
            p_indices_list = []                     # NEW: original skinCoord index for every unified vertex
    ...
    # Record skin data for every primitive we just added (so we can add JOINTS_0 / WEIGHTS_0 later)
...
def traverse_x3d_node(xml_node, parent_gltf_node_idx, gltf, bin_blob, def_map, mat_list, def_to_node_idx, node_to_center, base_path, joint_skinning, temp_skin_info):
    ...
    if xml_node.tag in structural_tags:
        ...
        # NEW: collect HAnimJoint skinning data (skinCoordIndex / skinCoordWeight)
...
def convert_x3d_to_glb(x3d_filepath, glb_filepath):
    ...
    # NEW: storage for HAnim skinning
    joint_skinning = {}
    temp_skin_info = []                     # (mesh_idx, prim_local_idx, list_of_original_p_indices)

    gltf = GLTF2(
        asset=Asset(generator="X3D-to-GLB Advanced Converter + HAnim Skinning", version="2.0"),
    ...
    # Traverse (now passes joint_skinning + temp_skin_info)
    ...
    if joint_skinning:
        ...
        if joint_node_indices:
            ...
            # 3. Normalize + cap at 4 influences per vertex (glTF requirement)
            ...
            for mesh_idx, prim_local_idx, p_indices_list in temp_skin_info:
                ...
                # primitives are dicts in this converter
