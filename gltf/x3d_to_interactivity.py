...
Features Expanded:
  ...
  • Interactivity (KHR_interactivity): Maps TouchSensor, ProximitySensor, Logic Nodes,
    and ROUTEs to the draft glTF interactivity graph.
  • Metadata: Embeds NavigationInfo and <meta> tags into glTF Scene/Asset extras.
...
def traverse_x3d_node(xml_node, parent_gltf_node_idx, gltf, bin_blob, def_map, mat_list, def_to_node_idx, node_to_center, base_path, interactivity_nodes, scene_extras):
    structural_tags = {'Transform', 'Group', 'HAnimJoint', 'HAnimSegment', 'HAnimHumanoid', 'HAnimSite'}
    logic_tags = {'BooleanFilter', 'BooleanSequencer', 'ProximitySensor', 'TimeTrigger', 'TouchSensor'}

    if xml_node.tag == 'NavigationInfo':
        scene_extras["NavigationInfo"] = {
            "type": parse_mfstring(xml_node.get('type', '"EXAMINE" "ANY"')),
            "avatarSize": parse_array(xml_node.get('avatarSize', '0.2 1.6 0.75')),
            "speed": float(xml_node.get('speed', '1.0')),
            "visibilityLimit": float(xml_node.get('visibilityLimit', '0.0'))
        }
        return

    if xml_node.tag in logic_tags:
        node_def = xml_node.get('DEF') or f"{xml_node.tag}_{len(interactivity_nodes)}"
        config = {}
        if xml_node.tag == 'TouchSensor':
            if parent_gltf_node_idx is not None: config['targetNode'] = parent_gltf_node_idx
            config['enabled'] = xml_node.get('enabled', 'true').lower() in ('true', '1')
        elif xml_node.tag == 'ProximitySensor':
            config['center'] = parse_array(xml_node.get('center', '0 0 0'))
            config['size'] = parse_array(xml_node.get('size', '0 0 0'))
            config['enabled'] = xml_node.get('enabled', 'true').lower() in ('true', '1')
        elif xml_node.tag == 'BooleanSequencer':
            config['key'] = parse_array(xml_node.get('key', ''))
            kv = xml_node.get('keyValue', '').strip().split()
            config['keyValue'] = [v.lower() in ('true', '1') for v in kv]

        interactivity_nodes[node_def] = {
            "id": node_def,
            "type": xml_node.tag,
    ...
    if xml_node.tag in ('Inline', 'InlineGeometry'):
        ...
        if os.path.exists(local):
            try:
                ...
                traverse_x3d_node(ext.find(f".//*[@DEF='{frag}']") if frag else ext, parent_gltf_node_idx, gltf, bin_blob, def_map, mat_list, def_to_node_idx, node_to_center, base_path, interactivity_nodes, scene_extras)
            except: pass
        return
    ...
    if xml_node.tag in structural_tags:
        ...
        for child in xml_node:
            traverse_x3d_node(child, inner_idx, gltf, bin_blob, def_map, mat_list, def_to_node_idx, node_to_center, base_path, interactivity_nodes, scene_extras)
        ...
        for child in xml_node:
            traverse_x3d_node(child, parent_gltf_node_idx, gltf, bin_blob, def_map, mat_list, def_to_node_idx, node_to_center, base_path, interactivity_nodes, scene_extras)

# ---------------------------------------------------------------------------
# Animations and Interactivity Mapping
# ---------------------------------------------------------------------------
...
def convert_interactivity(root, gltf_dict, interactivity_nodes, scene_extras):
    routes = []
    for route in root.findall('.//ROUTE'):
        routes.append({
            "node": route.get('fromNode'),
            "socket": route.get('fromField'),
            "destinationNode": route.get('toNode'),
            "destinationSocket": route.get('toField')
        })

    for ts in root.findall('.//TimeSensor'):
        ts_def = ts.get('DEF')
        if ts_def and ts_def not in interactivity_nodes:
            interactivity_nodes[ts_def] = {
                "id": ts_def,
                "type": "TimeSensor",
                "configuration": {
                    "cycleInterval": float(ts.get('cycleInterval', '1.0')),
                    "loop": ts.get('loop', 'false').lower() in ('true', '1'),
                    "enabled": ts.get('enabled', 'true').lower() in ('true', '1')
                }
            }

    if interactivity_nodes or routes:
        gltf_dict.setdefault('extensionsUsed', []).append('KHR_interactivity')
        gltf_dict.setdefault('extensions', {})['KHR_interactivity'] = {
            "nodes": list(interactivity_nodes.values()),
            "connections": routes
        }

    if scene_extras and len(gltf_dict.get('scenes', [])) > 0:
        gltf_dict['scenes'][0].setdefault('extras', {}).update(scene_extras)
...
def convert_x3d_to_glb(x3d_filepath, glb_filepath):
    ...
    traverse_x3d_node(scene_root, 0, gltf, bin_blob, def_map, mat_list, def_to_node_idx, node_to_center, base_path, interactivity_nodes, scene_extras)
    ...
    # Apply Interaction Graph mapping
    convert_interactivity(root, gltf_dict, interactivity_nodes, scene_extras)

    write_glb(gltf_dict, bin_blob, glb_filepath)
    size_kb = os.path.getsize(glb_filepath) / 1024
    interactivity_count = len(gltf_dict.get('extensions', {}).get('KHR_interactivity', {}).get('nodes', []))
    print(f"  OK: {glb_filepath}  ({size_kb:.1f} KB) | nodes={len(gltf_dict.get('nodes', []))} | meshes={len(gltf_dict.get('meshes', []))} | anims={len(gltf_dict.get('animations', []))} | logic_nodes={interactivity_count}")
