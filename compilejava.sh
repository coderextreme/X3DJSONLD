...
(ls *.java | grep -v RunSaxon; find www_web3d_org/ -name '*.java') | xargs -L 1 -P 8 javac -J-Xss16m -J-Xmx4096M 2>&1 1> javacerrors.txt

(ls *.class | grep -v RunSaxon; find www_web3d_org/ -name '*.class') | sed 's/\.class$//' | xargs -L 1 -P 8 java -d64 -Xss16m -Xmx4096M 2>&1 1> javaerrors.txt
