...
"""]]></xsl:text>
    ...
    def hasChild(self):
        ...
        <xsl:text>
    # output function - - - - - - - - - -
    def XML(self, indentLevel=0, syntax="XML", recordDefs=True):
...
#       print('XML serialization complete.', flush=True)
    ...
    # output function - - - - - - - - - -
    def VRML97(self, indentLevel=0, recordDefs=True):
        """ Provide VRML97 output serialization suitable for .wrl file. """
	if recordDefs:
		globalRecordDefs = {}
        return VRML(self, indentLevel=0, VRML97=True, recordDefs=True)
    # output function - - - - - - - - - -
    def ClassicVRML(self, indentLevel=0, recordDefs=True):
        """ Provide ClassicVRML output serialization suitable for .x3dv file. """
	if recordDefs:
		globalRecordDefs = {}
        return VRML(self, indentLevel=0, VRML97=False, recordDefs=True)
    # output function - - - - - - - - - -
#    def X_ITE(self, recordDefs=True): # TODO implement
...
#        return X3D.X_ITE_HEADER + result + self.XML(indentLevel=0, syntax="XML") + X3D.X_ITE_FOOTER:
    # output function - - - - - - - - - -
    def X3DOM(self, indentLevel=0, recordDefs=True):
...
#       print('XML serialization complete.', flush=True)
        ...
        <xsl:text>
    # output function - - - - - - - - - -
    def JSON(self, indentLevel=0, syntax="JSON", recordDefs=True):
...
#       print('JSON serialization complete.', flush=True)
        ...
        <xsl:text>
    # output function - - - - - - - - - -
    def HTML5(self, indentLevel=0, recordDefs=True):
	...
	if recordDefs:
        ...
        <xsl:text>
    # output function - - - - - - - - - -
    def VRML(self, indentLevel=0, VRML97=False, recordDefs=True):
            ...
            result += '\n'</xsl:text>
                ...
                <xsl:choose>
                  <xsl:when test="not($elementName = 'ROUTE') and not(contains($elementName, 'Proto')) and not(starts-with($elementName, 'field')) and
                    ...
                    <xsl:text>
        if self.DEF and globalRecordDefs[self.DEF] is None:
            globalRecordDefs[self.DEF] = True
            result += 'DEF ' + self.DEF + ' ' + '</xsl:text>
                    <xsl:value-of select="$elementName"/>
                    <xsl:text>' + ' {'
        elif self.USE or (self.DEF and globalRecordDefs[self.DEF]):
