...
var fs = require('fs');
if (typeof load !== 'function') { // load is Nashorn, I think
    fs = require("fs");
    http = require("http");
    https = require("https");
    // TODO this causes node-java 0.12.2 to hang
...
var X3DJSONLD = {
    x3djsonNS: "",
    Browser: {
        print: function (str) { if (typeof console !== 'undefined' && typeof str !== 'undefined')
            console.error(str); },
        println: function (str) { if (typeof console !== 'undefined' && typeof str !== 'undefined')
            console.error(str); },
                    ...
                    (function (url) {
                        ...
                        try {
                            if (protocol === "http") {
                                ...
                                var data_1 = fetch(url);
                                loadedCallback(data_1, url, protoexp, done, externProtoDeclare, obj);
                            }
                            else if (protocol.startsWith("https")) {
                                // console.error("Loading HTTPS URL", url);
                                var response = fetch(url);
                                loadedCallback(response, url, protoexp, done, externProtoDeclare, obj);
                            }
                                ...
                                var data_2 = fetch(url);
                                loadedCallback(data_2, url, protoexp, done, externProtoDeclare, obj);
                            }
                        }
                        catch (e) {
                            console.error("Didn't load", url, " Error ", e);
                        }
                    })(url);
                }
                catch (e) {
                ...
                if (key === 'connect' || key === 'fieldValue' || key === 'field' || key === 'meta' || key === 'component' || key === 'unit') {
                    ...
                    */
                    for (var childkey in object[key]) { // for each field
                        // Strip off last three meta array elements
                        if (key !== 'meta' || parseInt(childkey) < object[key].length - 3) {
                            if (typeof object[key][childkey] === 'object') {
                                var child_1 = X3DJSONLD.CreateElement(xmlDoc, key, X3DJSONLD.x3djsonNS, containerField);
                                X3DJSONLD.ConvertToX3DOM(xmlDoc, object[key][childkey], childkey, child_1, path, undefined);
                                element.appendChild(child_1);
                                element.appendChild(xmlDoc.createTextNode("\n"));
                            }
                        }
                    }
                    ...
                    var child_2 = X3DJSONLD.CreateElement(xmlDoc, key, X3DJSONLD.x3djsonNS, containerField);
                    X3DJSONLD.ConvertToX3DOM(xmlDoc, object[key], key, child_2, path, undefined);
                    element.appendChild(child_2);
                    element.appendChild(xmlDoc.createTextNode("\n"));
                }
            }
        }
            ...
            if (isArray) {
                ...
                if (key === 'X3D') {
                    ...
                    if (key === "-skin" || key === "-skeleton" || key === "-value" || key === "-segments") {
                        ...
                        X3DJSONLD.ConvertObject(xmlDoc, key, object, element, path, undefined);
                    }
                }
            }
    ...
    loadJsonIntoDom: function (domImplementation, jsobj, path) {
        ...
        var child = X3DJSONLD.CreateElement(xmlDoc, 'X3D', X3DJSONLD.x3djsonNS, undefined);
    ...
    fixXML: function (xmlstr) {
        ...
        xmlstr = xmlstr.replace(/([\u0080-\uFFFF])/g, function (v) { return String.fromCharCode(parseInt(v, 16)); });
        // xmlstr = xmlstr.replace(/(\\+)&quot;/g, '\\&quot;');
        do {
            var xmlstr2 = xmlstr;
