...
if (typeof load !== 'function') { // load is Nashorn, I think
	...
	if (typeof xmldom !== 'undefined') {
		var domserializer : XMLSerializer = new xmldom.XMLSerializer();
		var domParser : DOMParser = xmldom.DOMParser;
	} else {
		var domserializer : XMLSerializer = new XMLSerializer();
 ...
 * loading.
 */
processURLs: function(localArray : string[], path : string) : string[] {
			...
			*/
			...
			if (key === 'connect' || key === 'fieldValue' || key === 'field' || key === 'meta' || key === 'component' || key === 'unit') {
				...
				*/
				for (var childkey in object[key]) {  // for each field
					...
					if (key !== 'meta' || parseInt(childkey) < object[key].length - 3) {
						if (typeof object[key][childkey] === 'object') {
							let  child : Element = X3DJSONLD.CreateElement(xmlDoc, key, X3DJSONLD.x3djsonNS, containerField);
							X3DJSONLD.ConvertToX3DOM(xmlDoc, object[key][childkey], childkey, child, path, undefined);
							element.appendChild(child);
							element.appendChild(xmlDoc.createTextNode("\n"));
						}
					}
				...
				let child : Element = X3DJSONLD.CreateElement(xmlDoc, key, X3DJSONLD.x3djsonNS, containerField);
				X3DJSONLD.ConvertToX3DOM(xmlDoc, object[key], key, child, path, undefined);
				element.appendChild(child);
				element.appendChild(xmlDoc.createTextNode("\n"));
			}
		}
 ...
 */
loadJsonIntoDom: function(domImplementation : DOMImplementation, jsobj : object, path : string) : any[] {
	var xmlDoc : Document = X3DJSONLD.prepareDocument(domImplementation, jsobj);
	var child : Element = X3DJSONLD.CreateElement(xmlDoc, 'X3D', X3DJSONLD.x3djsonNS, undefined);
...
fixXML : function(xmlstr: string) {
	...
	xmlstr = xmlstr.replace(/([\u0080-\uFFFF])/g, 
		function (v) : string { return String.fromCharCode(parseInt(v, 16))}
	);
	// xmlstr = xmlstr.replace(/(\\+)&quot;/g, '\\&quot;');
	do {
		var xmlstr2 = xmlstr;
