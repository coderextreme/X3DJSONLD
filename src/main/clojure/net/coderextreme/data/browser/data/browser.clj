...
(def X3D0 (doto (X3D.)
    ...
    (.setScene (doto (Scene. )
        (.addChildren (doto (Script. )
            (.setDEF "Browser")
(.setSourceCode "\\newline                function initialize() {\\newline		    Browser.print('DUDES\\n'+'\"DUDETTES');\\newline                }\\newline")        ))
        (.addChildren (doto (Script. )
            (.setDEF "Clouds")
(.setSourceCode "\\newline\\newline\\newlinefunction cumulustranslation() // These values designate the boundary location of the cloud\\newline{\\newlinevar xxx = ' '+' '+\\newline'	Transform		\\n'+\\newline'    ' + '               	\\n';\\newline\\newline}\\newline")        ))
    ))
))
(.toFileX3D X3D0 "../data/browser.new.clojure.x3d")
(.toFileJSON X3D0 "../data/browser.new.clojure.json")
(defn -main []
