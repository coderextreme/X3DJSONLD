(ns WinterSpringThanksAllTitleBox
...
(def X3D0 (doto (X3D.)
    ...
    (.setHead (doto (head. )
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "title")
            (.setContent "WinterAndSpringThanksAllTitleBox.x3d")
        ))
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "description")
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "identifier")
            (.setContent "https://www.web3d.org/x3d/content/examples/HumanoidAnimation/WinterAndSpring/WinterAndSpringThanksAllTitleBox.x3d")
        ))
    ))
    (.setScene (doto (Scene. )
        (.addChildren (doto (WorldInfo. )
            (.setTitle "WinterAndSpringThanksAllTitleBox.x3d")
        ))
        (.addChildren (doto (Viewpoint. )
            (.setDEF "WSThanksAllView")
            (.setDescription "Look at Thanks title")
            (.setPosition (float-array [10 11 -10]))
            (.setOrientation (float-array [-0.15 0 0 0.2]))
        ))
        (.addChildren (doto (Transform. )
            (.setDEF "WSThanksAllTitle")
            (.setTranslation (float-array [10 10 -15]))
            (.addChild (doto (Shape. )
                (.setAppearance (doto (Appearance. )
                    (.setMaterial (doto (Material. )
                    ))
                    (.setTexture (doto (ImageTexture. )
                        (.setUrl (into-array ["images/WSThanksR2trimmed.png" "https://www.web3d.org/x3d/content/examples/HumanoidAnimation/WinterAndSpring/images/WSThanksR2trimmed.png"]))
                    ))
                ))
...
(.toFileX3D X3D0 "../../../../../data/WinterSpringThanksAllTitleBox.new.clojure.x3d")
(.toFileJSON X3D0 "../../../../../data/WinterSpringThanksAllTitleBox.new.clojure.x3dj")
(defn -main []
