...
(def X3D0 (doto (X3D.)
    ...
    (.setHead (doto (head. )
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "TODO")
            (.setContent "rescale smaller than 30m! convert inches to meters? what about motion scaling?")
        ))
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "description")
        ...
        (.addChildren (doto (HAnimHumanoid. )
            ...
            (.setMetadata (doto (MetadataSet. )
                ...
                (.addValue (doto (MetadataString. )
                    (.setName "authorName")
                    (.setValue (into-array ["Jin Hoon Lee and Min Joo Lee, Chul Hee Jung and Myeong Won Lee"]))
                ))
                (.addValue (doto (MetadataString. )
                    (.setName "authorEmail")
                ...
                (.addValue (doto (MetadataFloat. )
                    (.setName "weight")
                    (.setValue (float-array [50]))
                ))
                (.addValue (doto (MetadataString. )
                    (.setName "humanoidVersion")
                    (.setValue (into-array ["validation testing"]))
                ))
            ))
...
(.toFileX3D X3D0 "../data/JinLOA1.new.clojure.x3d")
(.toFileJSON X3D0 "../data/JinLOA1.new.clojure.json")
(defn -main []
