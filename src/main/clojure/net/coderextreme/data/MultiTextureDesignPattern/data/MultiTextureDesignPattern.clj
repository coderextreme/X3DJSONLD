...
(.addComments "================================")
        ...
        (.addChildren (doto (Background. )
            (.setSkyColor (float-array [0.72549 1 0.721569]))
        ))
        ...
        (.addChildren (doto (Shape. )
            (.setGeometry (doto (Text. )
                (.setString (into-array ["Source shows design pattern for" "MultiTexture, MultiTextureCoordinate," " and MultiTextureTransform nodes"]))
            ...
            (.setAppearance (doto (Appearance. )
                (.setMaterial (doto (Material. )
                    (.setDiffuseColor (float-array [0.2 0.4 0.8]))
                ))
            ))
        ))
(.addComments "================================")
...
(.addComments "add a single geometry node here")
            (.setGeometry (doto (IndexedFaceSet. )
                (.setTexCoord (doto (MultiTextureCoordinate. )
(.addComments "add multiple TextureCoordinate nodes here, match corresponding MultiTexture child texture nodes")
                    (.addTexCoord (doto (TextureCoordinate. )
                        (.setPoint (float-array [0 0 1 0 1 1 0 1]))
                    ))
                    (.addTexCoord (doto (TextureCoordinate. )
                        (.setPoint (float-array [0 0 1 0 1 1 0 1]))
                    ))
            ...
            (.setAppearance (doto (Appearance. )
                ...
                (.setTexture (doto (MultiTexture. )
                    (.setAlpha 0.8)
                    (.setColor (float-array [0.9 1 0.2]))
                    (.setFunction (into-array ["COMPLEMENT" "ALPHAREPLICATE"]))
                    (.setMode (into-array ["MODULATE" "REPLACE" "BLENDDIFFUSEALPHA"]))
                    (.setSource (into-array ["DIFFUSE" "SPECULAR" "FACTOR"]))
(.addComments "add multiple texture nodes here")
                    (.addTexture (doto (ImageTexture. )
                    ))
                    (.addTexture (doto (MovieTexture. )
                    ))
                    (.addTexture (doto (PixelTexture. )
                    ))
                ))
                (.setTextureTransform (doto (MultiTextureTransform. )
(.addComments "add multiple TextureTransform nodes here, match corresponding MultiTexture child texture nodes")
                    (.addTextureTransform (doto (TextureTransform. )
                    ))
                    (.addTextureTransform (doto (TextureTransform. )
                    ))
                    (.addTextureTransform (doto (TextureTransform. )
...
(.toFileX3D X3D0 "../../../../../data/MultiTextureDesignPattern.new.clojure.x3d")
(.toFileJSON X3D0 "../../../../../data/MultiTextureDesignPattern.new.clojure.x3dj")
(defn -main []
