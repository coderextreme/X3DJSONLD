...
(def X3D0 (doto (X3D.)
    ...
    (.setHead (doto (head. )
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "identifier")
            (.setContent "https://coderextreme.net/X3DJSONLD/src/main/data/x3domflowers.x3d")
        ))
    ))
    (.setScene (doto (Scene. )
...
;Images courtesy of Paul Debevec's Light Probe Image Gallery
        ...
        (.addChildren (doto (Group. )
            (.addChildren (doto (ExternProtoDeclare. )
                (.setName "FlowerProto")
                (.setUrl (into-array ["../data/flowerproto.json#FlowerProto"]))
                (.addField (doto (field. )
                    (.setName "vertex")
...
(.toFileX3D X3D0 "../data/x3domflowers.new.clojure.x3d")
(.toFileJSON X3D0 "../data/x3domflowers.new.clojure.json")
(defn -main []
