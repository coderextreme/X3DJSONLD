        ...
        (.addChildren (doto (Transform. )
            ...
            (.addChild (doto (Shape. )
                (.setGeometry (doto (Text. )
                    (.setString (into-array ["Compare special character escaping"]))
                ...
                (.setAppearance (doto (Appearance. )
                    ...
                    (.setMaterial (doto (Material. )
                        (.setDiffuseColor (float-array [0.1 0.7 0.7]))
                    ))
                ))
            ))
        ))
        (.addChildren (doto (Transform. )
            (.setTranslation (float-array [-3 0 0]))
            (.addChild (doto (Shape. )
                (.setGeometry (doto (Text. )
                    (.setString (into-array ["I don't think so" "" "he said \"Hi\""]))
                    (.setFontStyle (doto (FontStyle. )
...
(.toFileX3D X3D0 "../../../../../data/TextExamples.new.clojure.x3d")
(.toFileJSON X3D0 "../../../../../data/TextExamples.new.clojure.json")
(defn -main []
