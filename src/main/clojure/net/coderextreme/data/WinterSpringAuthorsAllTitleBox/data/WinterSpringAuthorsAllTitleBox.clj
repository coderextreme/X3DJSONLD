(ns WinterSpringAuthorsAllTitleBox
...
(def X3D0 (doto (X3D.)
    ...
    (.setScene (doto (Scene. )
        ...
        (.addChildren (doto (Viewpoint. )
            ...
            (.setPosition (float-array [10 11 0]))
            (.setOrientation (float-array [-0.15 0 0 0.2]))
        ))
        (.addChildren (doto (Transform. )
            (.setDEF "WSAuthorsTitle")
            (.setTranslation (float-array [10 10 -5]))
            (.addChild (doto (Shape. )
                (.setAppearance (doto (Appearance. )
                    (.setMaterial (doto (Material. )
                    ))
                    (.setTexture (doto (ImageTexture. )
                        (.setUrl (into-array ["images/WSAuthorsAll.png" "https://www.web3d.org/x3d/content/examples/HumanoidAnimation/WinterAndSpring/images/WSAuthorsAll.png"]))
                    ))
                ))
...
(.toFileX3D X3D0 "../../../../../data/WinterSpringAuthorsAllTitleBox.new.clojure.x3d")
(.toFileJSON X3D0 "../../../../../data/WinterSpringAuthorsAllTitleBox.new.clojure.x3dj")
(defn -main []
