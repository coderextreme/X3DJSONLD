...
(def X3D0 (doto (X3D.)
    ...
    (.setScene (doto (Scene. )
        ...
        (.addChildren (doto (Transform. )
            (.setRotation (float-array [7 8 9 3.14]))
            (.setScale (float-array [4 5 6]))
            (.setTranslation (float-array [1 2 3]))
        ))
    ))
))
(.toFileX3D X3D0 ".././app.new.clojure.x3d")
(.toFileJSON X3D0 ".././app.new.clojure.json")
(defn -main []
