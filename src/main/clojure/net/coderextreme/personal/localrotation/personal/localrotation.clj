...
(def X3D0 (doto (X3D.)
    ...
    (.setHead (doto (head. )
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "identifier")
            (.setContent "https://coderextreme.net/X3DJSONLD/src/main/personal/localrotation.x3d")
        ))
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "description")
            (.setContent "chained boxes")
        ))
    ))
    (.setScene (doto (Scene. )
        (.addChildren (doto (WorldInfo. )
            (.setTitle "localrotation.x3d")
        ))
        ...
        (.addChildren (doto (Transform. )
            (.setTranslation (float-array [-4 0 0]))
            (.addChild (doto (Shape. )
                (.setGeometry (doto (Box. )
                ))
            ))
            (.addChildren (doto (Transform. )
                (.setDEF "TransformTargetParent")
                (.setTranslation (float-array [4 0 0]))
                (.addChild (doto (Shape. )
                    (.setGeometry (doto (Box. )
                    ))
        ...
        (.addChildren (doto (OrientationInterpolator. )
            ...
            (.setKeyValue (float-array [0 0 1 0 0 0 1 1.57]))
        ))
        (.addChildren (doto (ROUTE. )
            (.setFromNode "Clock")
        ...
        (.addChildren (doto (ROUTE. )
            ...
            (.setToNode "TransformTargetParent")
            (.setToField "set_rotation")
        ))
        (.addChildren (doto (ROUTE. )
            (.setFromNode "Rotater")
            (.setFromField "value_changed")
            (.setToNode "TransformTargetChild")
            (.setToField "set_rotation")
        ))
    ))
))
(.toFileX3D X3D0 "../personal/localrotation.new.clojure.x3d")
(.toFileJSON X3D0 "../personal/localrotation.new.clojure.json")
(defn -main []
