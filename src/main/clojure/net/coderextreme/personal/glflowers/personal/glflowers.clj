...
(def X3D0 (doto (X3D.)
    ...
    (.setHead (doto (head. )
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "identifier")
            (.setContent "https://coderextreme.net/X3DJSONLD/src/main/personal/glflowers.x3d")
        ))
    ))
    (.setScene (doto (Scene. )
        ...
        (.addChildren (doto (Group. )
            (.addChildren (doto (ExternProtoDeclare. )
                (.setName "FlowerProto")
                (.setUrl (into-array ["../personal/flowerproto.x3d#FlowerProto"]))
                (.addField (doto (field. )
                    (.setName "vertex")
...
(.toFileX3D X3D0 "../personal/glflowers.new.clojure.x3d")
(.toFileJSON X3D0 "../personal/glflowers.new.clojure.json")
(defn -main []
