...
(def X3D0 (doto (X3D.)
    ...
    (.setHead (doto (head. )
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "created")
            (.setContent "Jan 17 2022")
        ))
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "modified")
            (.setContent "Sep 3 2023")
        ))
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "description")
        ...
        (.addMeta (doto (org.web3d.x3d.jsail.Core.meta. )
            (.setName "identifier")
            (.setContent "https://coderextreme.net/X3DJSONLD/src/main/personal/sphereflowers.x3d")
        ))
    ))
    (.setScene (doto (Scene. )
        ...
        (.addChildren (doto (Group. )
            (.addChildren (doto (ExternProtoDeclare. )
                (.setName "FlowerProto")
                (.setUrl (into-array ["../personal/flowerproto.x3d#FlowerProto" "https://coderextreme.net/X3DJSONLD/src/main/personal/flowerproto.x3d#FlowerProto"]))
                (.addField (doto (field. )
                    (.setName "vertex")
                    ...
                    (.addChildren (doto (Group. )
                        (.addChildren (doto (ProtoInstance. )
                            ...
                            (.addFieldValue (doto (fieldValue. )
                                (.setName "vertex")
                                (.setValue "\"../shaders/x_ite_flowers_chromatic.vs\"")
                            ))
                            (.addFieldValue (doto (fieldValue. )
                                (.setName "fragment")
                                (.setValue "\"../shaders/x_ite.fs\"")
                            ))
                        ))
                    ))
                ))
            ...
            (.addChildren (doto (Sound. )
                ...
                (.setMinBack 20.0)
                (.setMinFront 20.0)
                (.setLocation (float-array [0 1 0]))
                (.setSource (doto (AudioClip. )
                    (.setDEF "AudioClip")
                    (.setDescription "Chandubabamusic #1")
                    (.setUrl (into-array ["../resources/chandubabamusic1.wav" "https://coderextreme.net/X3DJSONLD/src/main/resources/chandubabamusic1.wav"]))
                ))
            ))
...
(.toFileX3D X3D0 "../personal/sphereflowers.new.clojure.x3d")
(.toFileJSON X3D0 "../personal/sphereflowers.new.clojure.json")
(defn -main []
