export function writeUint8ArrayBufferToEXIEditor(uint8Array, uint8ArrayLength) {
	var hex = "";
...
export function encodeJSON() {
	...
	if(textJSON.trim().length === 0) {
		// Do not alert, just clear the EXI field
		$("#exi").val('');
	} else {
		try {
			...
			writeUint8ArrayBufferToEXIEditor(uint8Array, uint8ArrayLength);
		} catch(err) {
			// Do not alert, show error in the field
			$("#exi").val("Error while encoding JSON. " + err);
		}
...
export function decodeJSON() {
	...
	if(textEXI.length === 0) {
		...
		try {
			var sp = textEXI.split(" ").filter(s => s.length > 0); 
