...
var map = {
	...
	"fieldValue": "SoField",
	"Scene": "SoNode",
	"Appearance": "SoVRMLAppearance"
};

function maptag(x3d_name) {
	return map[x3d_name] || "So"+x3d_name;
}
...
`;
		...
		bodystr += maptag(element.nodeName)+REF+" "+maptag(element.nodeName)+stack[0]+" = "+NEW+" "+maptag(element.nodeName)+"();\n";
	...
	stringValue : function(attrsa, attr, attrType, element) {
		...
		if (nodeValue === 'NULL') {
			...
			*/
			...
			strval = this.printSubArray(attr, attrType, "QString",
				...
				map(function(x) {
					...
					return y;
				}), this.codeno, '"), QString("', 'QString("', '")', element); // ... json, lead, tail
	...
	subSerializeToString : function(element, mapToMethod, fieldTypes, n, stack) {
		...
		for (let a in element.attributes) {
			...
			try {
				...
				if (attrs.hasOwnProperty(a) && attrsa.nodeType === 2) {
					...
					str += maptag(element.nodeName)+stack[0];
					let startshim = "";
		...
		for (let cn in element.childNodes) {
			...
			if (element.childNodes.hasOwnProperty(cn) && node.nodeType === 1) {
				...
				nodeName = maptag(nodeName);

				ch += nodeName+REF+" "+maptag(node.nodeName)+stack[0]+" = "+NEW+" "+nodeName+"();\n";
				...
				*/
				...
				method = method.replace("setScene", "setSceneGraph");
				/*
				...
				*/
				...
				ch += maptag(element.nodeName)+stack[1];
				...
				*/
				...
				ch += startshim+method+"("+shim+maptag(node.nodeName)+stack[0]+endshim+");\n\n";
				str += ch;
				stack.shift();
				...
				str += "\n//"+maptag(element.nodeName)+stack[0];
				str += OBJ+'setSourceCode(QString("'+node.nodeValue.split(/[\r\n]+/).map(function(x) {
					return x.
					        replace(/\\/g, '\\\\').
						replace(/"/g, '\\"')
						replace(/$/g, '\\')
