...
export default function TruffleRubySerializer () {
this.code = [];
this.codeno = 0;
this.preno = 0;
}
...
TruffleRubySerializer.prototype = {
	loadTextArea : function(text, stack, json, element, clazz, mapToMethod, fieldTypes) {
		...
		bodystr += "ConfigurationProperties.setXsltEngine(ConfigurationProperties.XSLT_ENGINE_NATIVE_JAVA);\n";
		...
		str += bodystr;
		str += enn+stack[0]+".toFileX3D \""+clazz+".new.truffleruby.x3d\"\n";
		str += enn+stack[0]+".toFileJSON \""+clazz+".new.truffleruby.x3dj\"\n";
		stack.shift();
		return str;
	},
	serializeToString : function(json, element, clazz, mapToMethod, fieldTypes) {
		...
		if (typeof fs === 'object') {
			str += this.loadTextArea(fs.readFileSync("../truffleruby/x3drsail.rb").toString(), stack, json, element, clazz, mapToMethod, fieldTypes);
			return str;
		} else {
		    return (async function (serializer) {
		      try {
			const response = await fetch("../truffleruby/x3drsail.rb");
			if (!response.ok) {
			  throw new Error(`HTTP error! status: ${response.status}`);
			}
			str += serializer.loadTextArea(await response.text(), stack, json, element, clazz, mapToMethod, fieldTypes);
			document.getElementById('truffleruby').value = str;
			return str;
		      } catch (error) {
			console.error('Error loading Ruby file:', error);
			document.getElementById('truffleruby').value = 'Error loading file: ' + error.message;
		      }
		    })(this);
		}
	},
	...
	printSubArray : function (attrType, type, values, co, j, lead, trail) {
		var rubyType = type.replace("java.lang.String", "string")
                       .replace(/\./g, '_');
                if (type === "int") {
                        for (var v in values) {
		...
		*/
		...
		return 'to_java_'+rubyType+'_array(['+lead+values.join(j)+trail+'])';
	},
			...
			strval = this.printSubArray(attrType, "java.lang.String",
				...
				map(function(x) {
					var y = x.
					       ...
					       replace(/\\n/g, '\\n');
					if (y !== x) {
						// console.error("TruffleRuby Replacing "+x+" with "+y);
					}
					return y;
				}), this.codeno, '", "', '"', '"');
		} else if (
