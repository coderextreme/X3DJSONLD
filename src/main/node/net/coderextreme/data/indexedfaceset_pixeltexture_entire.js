...
ConfigurationProperties.setStripTrailingZeroesSync(true);
      ...
      .setHeadSync(new autoclass.head()
        .addMetaSync(new autoclass.meta().setNameSync("title").setContentSync("indexedfaceset_pixeltexture_entire.x3d"))
        ...
        .addMetaSync(new autoclass.meta().setNameSync("description").setContentSync("Test browser ability to completely map one PixelTexture onto the surface of an IndexedFaceSet geometry. Four colored squares should map onto each face of the IndexedFaceSet. The PixelTexture consists of red quarter (lower left), green quarter (lower right), white quarter (upper left) and yellow quarter (upper right). PixelTexture should map once onto the surface of the IndexedFaceSet, with the S (horizontal) axis of the texture corresponding to the X axis of the geometry."))
        .addMetaSync(new autoclass.meta().setNameSync("identifier").setContentSync("http://www.web3d.org/x3d/content/examples/ConformanceNist/GeometricProperties/TextureCoordinate/indexedfaceset_pixeltexture_entire.x3d"))
      ...
      .setSceneSync(new autoclass.Scene()
        ...
        .addChildSync(new autoclass.Shape()
          .setAppearanceSync(new autoclass.Appearance()
            ...
            .setTextureSync(new autoclass.PixelTexture().setImageSync(java.newArray("int", [2,2,4,4278190335,16711935,4294967295,4294902015])).setRepeatSSync(false).setRepeatTSync(false)))
          .setGeometrySync(new autoclass.IndexedFaceSet().setCoordIndexSync(java.newArray("int", [0,1,3,2,-1,4,5,7,6,-1,6,7,1,0,-1,2,3,5,4,-1,6,0,2,4,-1,1,7,5,3,-1]))
            .setCoordSync(new autoclass.Coordinate().setPointSync(java.newArray("float", [java.newFloat(-2), java.newFloat(1.5), java.newFloat(1), java.newFloat(-2), java.newFloat(-1.5), java.newFloat(1), java.newFloat(2), java.newFloat(1.5), java.newFloat(1), java.newFloat(2), java.newFloat(-1.5), java.newFloat(1), java.newFloat(2), java.newFloat(1.5), java.newFloat(-1), java.newFloat(2), java.newFloat(-1.5), java.newFloat(-1), java.newFloat(-2), java.newFloat(1.5), java.newFloat(-1), java.newFloat(-2), java.newFloat(-1.5), java.newFloat(-1)]))))))      ;
