...
meta3.setContent(CString("Fri, 28 Aug 2015 13:01:09 GMT"));
head1.addMeta(&meta3);
...
meta6.setContent(CString("Fri, 28 Aug 2015 13:01:09 GMT"));
head1.addMeta(&meta6);
...
ProtoInstance24.setDEF(CString("_1"));
IS& IS25 =  IS();
...
connect29.setNodeField(CString("b"));
connect29.setProtoField(CString("bool"));
...
_T("function set_startTime(value, time) { b = data.value; }")+
_T("function initialize() { if (data.startTime == 1) set_startTime(data.value, 1); }"));
ProtoBody13.addChild(&Script20);

ROUTE& ROUTE30 =  ROUTE();
ROUTE30.setFromNode(CString("_1"));
ROUTE30.setFromField(CString("startTime_changed"));
ROUTE30.setToNode(CString("_bool"));
