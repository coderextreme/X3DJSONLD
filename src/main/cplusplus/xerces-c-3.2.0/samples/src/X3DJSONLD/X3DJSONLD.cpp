...
void X3DJSONLD::elementSetVector(DOMElement * element, const char * key, vector<JsonValue> value) {
	...
	cerr << endl;
	element->setAttribute(X(key), X(sb.str()));
}
	...
	if (object.getTag() == JSON_OBJECT) {
		for (auto keyiter : object) {
			...
			case JSON_NUMBER: {
				...
				ostrstream oss;
				oss << num;
				elementSetAttribute(child, key+1, X(oss.str()));
					  }
				break;
			case JSON_STRING: {
...
void X3DJSONLD::convertJsonArray(DOMDocument * document, JsonValue array, const char * parentkey, DOMElement * element, const char * containerField) {
	...
	if (array.getTag() == JSON_ARRAY) {
		for (auto keyiter : array) {
			...
			ostrstream oss;
			oss << key;
			const char * keyStr = oss.str();
			cerr << endl << "array loop " << key << endl;
			if (ok.getTag() == JSON_OBJECT) {
				...
				if (!(parentkey[0] == '-') && kii) {
					...
					convertJsonValue(document, ok, keyStr, element, parentkey+1);
				}
			} else if (ok.getTag() == JSON_ARRAY) {
				convertJsonValue(document, ok, keyStr, element, containerField);
			} else {
				cerr << "adding" << endl;
...
DOMDocument * X3DJSONLD::loadJsonIntoDocument(JsonValue jsobj) {
	...
	convertJsonObject(document, jsobj, "X3D", element, 0);
	/*
...
int main(int argc, char **argv) {
	...
	int status = jsonParse(source, &endptr, &jsobj, allocator);
	if (status != JSON_OK) {
