load('X3Dautoclass.js');
var ConfigurationProperties = Packages.org.web3d.x3d.jsail.ConfigurationProperties;
ConfigurationProperties.showDefaultAttributes = false;
ConfigurationProperties.xsltEngine = ConfigurationProperties.XSLT_ENGINE_NATIVE_JAVA;
ConfigurationProperties.deleteIntermediateFiles = false;
ConfigurationProperties.setStripTrailingZeroes(true);
      var X3D0 =  new X3DObject().setProfile("Immersive").setVersion("3.0")
      .setHead(new headObject()
        .addMeta(new metaObject().setName("title").setContent("ArtDecoPrototypes.x3d"))
        .addMeta(new metaObject().setName("description").setContent("Prototype declarations defining values for X3D/VRML materials, originally converted from SGI's Open Inventor material examples."))
        .addMeta(new metaObject().setName("creator").setContent("David Roussel"))
        .addMeta(new metaObject().setName("translator").setContent("James Harney, Don Brutzman NPS"))
        .addMeta(new metaObject().setName("created").setContent("7 April 2002"))
        .addMeta(new metaObject().setName("modified").setContent("20 October 2019"))
        .addMeta(new metaObject().setName("reference").setContent("http://vrmlstuff.free.fr/materials"))
        .addMeta(new metaObject().setName("subject").setContent("Universal Media Material Library"))
        .addMeta(new metaObject().setName("identifier").setContent("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials/ArtDecoPrototypes.x3d"))
        .addMeta(new metaObject().setName("generator").setContent("Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html"))
        .addMeta(new metaObject().setName("license").setContent("../license.html")))
      .setScene(new SceneObject()
        .addChild(new WorldInfoObject().setTitle("ArtDecoPrototypes.x3d"))
        .addChild(new ProtoDeclareObject().setName("ArtDeco00").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25).setDiffuseColor(Java.to([0.282435,0.085159,0.134462], Java.type("float[]"))).setShininess(0.127273).setSpecularColor(Java.to([0.276305,0.11431,0.139857], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco01").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.254777).setDiffuseColor(Java.to([0.685208,0.134679,0.332385], Java.type("float[]"))).setShininess(0.071429).setSpecularColor(Java.to([0.122449,0.050035,0.050035], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco02").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .addComments(new CommentsBlock("computed conversion ambientIntensity=1.745282, normalized to 1.0"))
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(1).setDiffuseColor(Java.to([0.536861,0.0529,0.245479], Java.type("float[]"))).setShininess(0.832432).setSpecularColor(Java.to([0.805292,0.765198,0.747416], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco03").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.248649).setDiffuseColor(Java.to([1,0.452381,0.40339], Java.type("float[]"))).setShininess(0.902703).setSpecularColor(Java.to([0.686486,0.396903,0.419275], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco04").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25).setDiffuseColor(Java.to([0.748016,0.298362,0.342624], Java.type("float[]"))).setShininess(0.69697).setSpecularColor(Java.to([0.345455,0.345455,0.345455], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco05").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.24359).setDiffuseColor(Java.to([0.945455,0.318988,0.321717], Java.type("float[]"))).setShininess(0.018182).setSpecularColor(Java.to([0.072727,0.021705,0.010732], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco06").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25946).setDiffuseColor(Java.to([0.228655,0.195513,0.425484], Java.type("float[]"))).setShininess(0.542553).setSpecularColor(Java.to([0.324504,0.404255,0.404255], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco07").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.333333).setDiffuseColor(Java.to([0.200348,0.100857,0.320833], Java.type("float[]"))).setShininess(0.133333).setSpecularColor(Java.to([0.311358,0.387879,0.387879], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco08").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.242424).setDiffuseColor(Java.to([0.330519,0.3389,0.6], Java.type("float[]"))).setShininess(0.787879).setSpecularColor(Java.to([0.290909,0.290909,0.290909], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco09").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.333333).setDiffuseColor(Java.to([0.237059,0.096273,0.215625], Java.type("float[]"))).setShininess(0.133333).setSpecularColor(Java.to([0.311358,0.387879,0.387879], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco10").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.242425).setDiffuseColor(Java.to([0.304,0.09025,0.207364], Java.type("float[]"))).setShininess(0.072727).setSpecularColor(Java.to([0.6665,0.579046,0.830303], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco11").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.258928).setDiffuseColor(Java.to([0.335631,0.207672,0.238304], Java.type("float[]"))).setEmissiveColor(Java.to([0.010638,0.010638,0.010638], Java.type("float[]"))).setShininess(0.021277).setSpecularColor(Java.to([0.198631,0.075403,0.138803], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco12").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.240838).setDiffuseColor(Java.to([0.876873,0.14715,0.00856], Java.type("float[]"))).setShininess(0.076531).setSpecularColor(Java.to([0.193878,0.029416,0.029416], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco13").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25).setDiffuseColor(Java.to([0.794445,0.249425,0], Java.type("float[]"))).setShininess(0.551515).setSpecularColor(Java.to([0.62904,0.194211,0.033663], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco14").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25).setDiffuseColor(Java.to([0.510609,0.17264,0.059872], Java.type("float[]"))).setShininess(0.933333).setSpecularColor(Java.to([0.533333,0.533333,0.533333], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco15").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.255814).setDiffuseColor(Java.to([0.914894,0.444404,0.348914], Java.type("float[]"))).setShininess(0.12766).setSpecularColor(Java.to([0.345745,0.143066,0], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco16").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.240838).setDiffuseColor(Java.to([0.876873,0.323147,0.199564], Java.type("float[]"))).setShininess(0.80102).setSpecularColor(Java.to([0.816327,0.278677,0.278677], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco17").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.242424).setDiffuseColor(Java.to([0.515625,0.315496,0.252441], Java.type("float[]"))).setShininess(0.933333).setSpecularColor(Java.to([0.533333,0.533333,0.533333], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco18").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.264706).setDiffuseColor(Java.to([0,0.346939,0.253624], Java.type("float[]"))).setShininess(0.316327).setSpecularColor(Java.to([0,0.311074,0.357143], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco19").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.259259).setDiffuseColor(Java.to([0,0.251004,0.239248], Java.type("float[]"))).setShininess(0.060606).setSpecularColor(Java.to([0.177935,0.249369,0.229278], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco20").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.24).setDiffuseColor(Java.to([0.228298,0.385771,0.186794], Java.type("float[]"))).setShininess(0.393939).setSpecularColor(Java.to([0.200464,0.300145,0.293518], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco21").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25).setDiffuseColor(Java.to([0.315389,0.544,0.258052], Java.type("float[]"))).setShininess(0.509389).setSpecularColor(Java.to([0.456,0.456,0.456], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco22").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.246032).setDiffuseColor(Java.to([0.251104,0.312562,0.201724], Java.type("float[]"))).setShininess(0.086735).setSpecularColor(Java.to([0.209184,0.113842,0.111328], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco23").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.242424).setDiffuseColor(Java.to([0.392348,0.456,0.417708], Java.type("float[]"))).setShininess(0.933333).setSpecularColor(Java.to([0.533333,0.533333,0.533333], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco24").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.253968).setDiffuseColor(Java.to([1,0.854922,0], Java.type("float[]"))).setShininess(1).setSpecularColor(Java.to([0.872449,0.247119,0.254214], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco25").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25641).setDiffuseColor(Java.to([0.795918,0.505869,0.093315], Java.type("float[]"))).setShininess(0.397959).setSpecularColor(Java.to([0.923469,0.428866,0.006369], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco26").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .addComments(new CommentsBlock("computed conversion ambientIntensity=2.226234, normalized to 1.0"))
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(1).setDiffuseColor(Java.to([0.331633,0.296582,0.279057], Java.type("float[]"))).setShininess(0.096939).setSpecularColor(Java.to([0.311224,0.25183,0.133042], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco27").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.263158).setDiffuseColor(Java.to([0.345455,0.163262,0.122622], Java.type("float[]"))).setShininess(0.048649).setSpecularColor(Java.to([0.212121,0.107475,0], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco28").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.240506).setDiffuseColor(Java.to([0.277281,0.104336,0.0799], Java.type("float[]"))).setShininess(0.09697).setSpecularColor(Java.to([0.305587,0.141916,0.270572], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco29").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.215686).setDiffuseColor(Java.to([0.087034,0.025888,0], Java.type("float[]"))).setShininess(0.045918).setSpecularColor(Java.to([0.224138,0.104091,0.104091], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco30").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0).setDiffuseColor(Java.to([0,0,0], Java.type("float[]"))).setShininess(0.081633).setSpecularColor(Java.to([0.293243,0.297387,0.290421], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco31").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25641).setDiffuseColor(Java.to([0.236364,0.236364,0.236364], Java.type("float[]"))).setShininess(0.054546).setSpecularColor(Java.to([0.29697,0.245839,0.295962], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco32").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.242424).setDiffuseColor(Java.to([0.38087,0.382957,0.417708], Java.type("float[]"))).setShininess(0.933333).setSpecularColor(Java.to([0.533333,0.533333,0.533333], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco33").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.230089).setDiffuseColor(Java.to([0.610811,0.610811,0.610811], Java.type("float[]"))).setShininess(0.897297).setSpecularColor(Java.to([0.767568,0.756757,0.764964], Java.type("float[]"))))))
        .addChild(new ProtoDeclareObject().setName("ArtDeco34").setAppinfo("UniversalMediaMaterials prototype").setDocumentation("https://www.web3d.org/x3d/content/examples/Basic/UniversalMediaMaterials")
          .setProtoBody(new ProtoBodyObject()
            .addChild(new MaterialObject().setAmbientIntensity(0.25).setShininess(0).setSpecularColor(Java.to([0.2,0.2,0.2], Java.type("float[]"))))))
        .addChild(new AnchorObject().setDescription("ArtDecoPrototypeExample").setParameter(Java.to(["target=_blank"], Java.type("java.lang.String[]"))).setUrl(Java.to(["../data/ArtDecoExamples.json","../data/ArtDecoExamples.x3d","ArtDecoExamples.json","ArtDecoExamples.x3d"], Java.type("java.lang.String[]")))
          .addChild(new ShapeObject()
            .setAppearance(new AppearanceObject()
              .setMaterial(new MaterialObject().setDiffuseColor(Java.to([0.8,0.4,0], Java.type("float[]")))))
            .setGeometry(new TextObject().setString(Java.to(["ArtDecoExamples.x3d","is a Materials Prototype declaration file.","","For an example scene using these node,","click this text and view","ArtDecoExamples.x3d"], Java.type("java.lang.String[]")))
              .setFontStyle(new FontStyleObject().setJustify(Java.to(["MIDDLE","MIDDLE"], Java.type("java.lang.String[]"))).setSize(0.8))))))      ;
    X3D0.toFileX3D("../data/ArtDecoPrototypes.new.x3d");
