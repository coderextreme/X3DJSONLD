...
public class x3dconnector
{
  ...
  .setHead(new head()
    .addMeta(new meta().setName(meta.NAME_TITLE      ).setContent("x3dconnector.x3d"))
  ...
  .setScene(new Scene()
    ...
    .addChild(new Transform("G2").setTranslation(1.0,-1.0,0.01)
      .addChild(new Shape()
        .setAppearance(new Appearance()
          .setMaterial(new Material().setDiffuseColor(0.2,0.7,0.2)))
        .setGeometry(new Sphere().setRadius(0.1)))
      .addChild(new PlaneSensor("PS2").setDescription("Grab to move").setOffset(1.0,-1.0,0.01))
...
//      System.out.println("X3D model construction complete.");
        ...
        if      (argumentsLoadNewModel)
                System.out.println("WARNING: \"net.x3djsonld.data.x3dconnector\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		...
		String validationResults = thisExampleX3dModel.validationReport();
            //  System.out.println("-----------------------------------"); // omit when duplicated outputs problem is solved/refactored
                System.out.print("net.x3djsonld.data.x3dconnector self-validation test results: ");
