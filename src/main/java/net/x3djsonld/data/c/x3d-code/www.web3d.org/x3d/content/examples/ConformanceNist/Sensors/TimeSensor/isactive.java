...
/**
 * <p> Test if TimeSensor sends isActive true event when it is running, and sends isActive false when it is stoped. A central white Sphere geometry has an associated TouchSensor that sends its touchTime event to the TimeSensors startTime field. The TimeSensors isActive event is sent to a PointLights "on" field. As a result: when the pointing device indicates the central Sphere, the TimeSensor should turn the PointLight on, and keep it on for 3 seconds (the cycleInterval value). After the cycleInterval completes, the light should turn off. </p>
 <p> Related links: <a href="../../../../Sensors/TimeSensor/isactive.java">isactive.java</a> source, <a href="../../../../Sensors/TimeSensor/isactiveIndex.html" target="_top">isactive catalog page</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dResources.html" target="_blank">X3D Resources</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dSceneAuthoringHints.html" target="_blank">X3D Scene Authoring Hints</a>, and <a href="https://www.web3d.org/x3d/content/X3dTooltips.html" target="_blank">X3D Tooltips</a>. </p>
	<table style="color:black; border:0px solid; border-spacing:10px 0px;">
		...
		<tr>
			...
			<td> <a href="../../../../Sensors/TimeSensor/isactive.x3d">isactive.x3d</a> </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> MovingImage </i> </td>
			<td> <a href="../../../../Sensors/TimeSensor/isactive-movie.mpg">isactive-movie.mpg</a> </td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td> Test if TimeSensor sends isActive true event when it is running, and sends isActive false when it is stoped. A central white Sphere geometry has an associated TouchSensor that sends its touchTime event to the TimeSensors startTime field. The TimeSensors isActive event is sent to a PointLights "on" field. As a result: when the pointing device indicates the central Sphere, the TimeSensor should turn the PointLight on, and keep it on for 3 seconds (the cycleInterval value). After the cycleInterval completes, the light should turn off. </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> identifier </i> </td>
			<td> <a href="https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/TimeSensor/isactive.x3d" target="_blank">https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/TimeSensor/isactive.x3d</a> </td>
		</tr>
		<tr>
...
public class isactive
{
  ...
  .setHead(new head()
    ...
    .addMeta(new meta().setName(meta.NAME_DESCRIPTION).setContent("Test if TimeSensor sends isActive true event when it is running, and sends isActive false when it is stoped. A central white Sphere geometry has an associated TouchSensor that sends its touchTime event to the TimeSensors startTime field. The TimeSensors isActive event is sent to a PointLights \"on\" field. As a result: when the pointing device indicates the central Sphere, the TimeSensor should turn the PointLight on, and keep it on for 3 seconds (the cycleInterval value). After the cycleInterval completes, the light should turn off."))
    .addMeta(new meta().setName(meta.NAME_IDENTIFIER ).setContent("https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/TimeSensor/isactive.x3d"))
  ...
  .setScene(new Scene()
    ...
    .addChild(new Group()
      .addChild(new TimeSensor("TIME").setCycleInterval(3))
      .addChild(new PointLight("LIGHT").setOn(false))
      .addChild(new Transform().setTranslation(2.0,0.0,0.0)
        .addChild(new Shape("BALL")
          .setAppearance(new Appearance()
            .setMaterial(new Material().setDiffuseColor(0.5,0.0,0.0)))
          .setGeometry(new Sphere()))
        .addChild(new PointLight().setUSE("LIGHT")))
      .addChild(new Transform().setTranslation(-2.0,0.0,0.0)
    ...
    .addChild(new ROUTE().setFromNode("TIME").setFromField("isActive").setToNode("LIGHT").setToField("on")));
    }
