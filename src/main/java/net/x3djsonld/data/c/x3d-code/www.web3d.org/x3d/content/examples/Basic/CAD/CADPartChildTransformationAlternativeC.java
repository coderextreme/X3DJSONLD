...
public class CADPartChildTransformationAlternativeC
{
	...
	public CADPartChildTransformationAlternativeC ()
	{
	  initialize();
	}
  ...
  .setHead(new head()
    ...
    .addMeta(new meta().setName(meta.NAME_TITLE      ).setContent("CADPartChildTransformationAlternativeC.x3d"))
    .addMeta(new meta().setName(meta.NAME_DESCRIPTION).setContent("Developer scene to explore Mantis Issue 528 Alternative C for transformation of of CADFace children: allow Transform between CADPart and CADFace nodes."))
    ...
    .addMeta(new meta().setName(meta.NAME_IDENTIFIER ).setContent("https://www.web3d.org/x3d/content/examples/Basic/CAD/CADPartChildTransformationAlternativeC.x3d"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit"))
    .addMeta(new meta().setName(meta.NAME_LICENSE    ).setContent("../license.html")))
  .setScene(new Scene()
    .addChild(new WorldInfo().setTitle("CADPartChildTransformationAlternativeC.x3d"))
    ...
    .addChild(new CADAssembly().setName("DesignPatternAssembly")
      .addChild(new CADPart().setName("CADPartExample")
        ...
        .addChild(new Transform().setRotation(1.0,0.0,0.0,1.5708).setTranslation(0.0,1.5,0.0)
          .addChild(new CADFace().setName("topCap")
            .setShape(new Shape()
              .setGeometry(new IndexedTriangleSet().setCcw(true).setColorPerVertex(true).setIndex(new int[] {0,1,2,2,1,3,2,3,4,4,3,5,4,5,6,6,5,7,6,7,8,8,7,9,8,9,10,10,9,11,10,11,12,12,11,13,12,13,14,14,13,15,14,15,16,16,15,17,16,17,18,18,17,19,18,19,20,20,19,21,20,21,22,22,21,23,22,23,24,24,23,25,24,25,26,26,25,27,26,27,28,28,27,29,28,29,30,30,29,31,30,31,32,32,31,33,32,33,34,34,33,35,34,35,36,36,35,37,36,37,38,38,37,39,38,39,40,40,39,41,40,41,42,42,41,43,42,43,44,44,43,45,44,45,46,46,45,47,46,47,48,48,47,49,48,49,50,50,49,51,50,51,52,52,51,53,52,53,54,54,53,55,54,55,56,56,55,57,56,57,58,58,57,59,58,59,60,60,59,61,60,61,62,62,61,63,62,63,64,64,63,65}).setNormalPerVertex(true).setSolid(true)
                .setCoord(new Coordinate("HollowCircleCoordinates").setPoint(getHollowCircleCoordinates_9_30_point())))
              .setAppearance(new Appearance().setUSE("AppearanceGrey")))))
        .addChild(new Transform().setRotation(-1.0,0.0,0.0,1.5708).setTranslation(0.0,-1.5,0.0)
    ...
    {
        X3D thisExampleX3dModel = new CADPartChildTransformationAlternativeC().getX3dModel();
		...
		if      (argumentsLoadNewModel)
			System.out.println("WARNING: \"net.x3djsonld.data.CADPartChildTransformationAlternativeC\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		else if (hasArguments) // if no arguments provided, this method produces usage warning
			thisExampleX3dModel.handleArguments(args);

		if (validate)
		{
			System.out.print("Java program \"net.x3djsonld.data.CADPartChildTransformationAlternativeC\" self-validation test results: ");
