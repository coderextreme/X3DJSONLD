 ...
 <p> Related links: <a href="../../../../Sensors/PlaneSensor/translation_changed.java">translation_changed.java</a> source, <a href="../../../../Sensors/PlaneSensor/translation_changedIndex.html" target="_top">translation_changed catalog page</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dResources.html" target="_blank">X3D Resources</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dSceneAuthoringHints.html" target="_blank">X3D Scene Authoring Hints</a>, and <a href="https://www.web3d.org/x3d/content/X3dTooltips.html" target="_blank">X3D Tooltips</a>. </p>
	<table style="color:black; border:0px solid; border-spacing:10px 0px;">
		...
		<tr>
			...
			<td> <a href="../../../../Sensors/PlaneSensor/translation_changed.x3d">translation_changed.x3d</a> </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> MovingImage </i> </td>
			<td> <a href="../../../../Sensors/PlaneSensor/translation_changed-movie.mpg">translation_changed-movie.mpg</a> </td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td> Test of the translation_changed field. A Box geometry has an associated PlaneSensor, which, when the Box is indicated by the pointing device, should route its translation_changed event to the Transform of a red Sphere geometry. The translation_changed value should equal the relative change in position between the current pointing device intersection point with the PlaneSensor and the previous position, PLUS the offset value (in this case a value of 0 0 1). The result is, the the primary pointing device button is pressed while over the Box, a red Sphere should appear initially on the Box, and move an equal distance in both X and Y as the pointing device, as it is dragged along the invisible XY plane. Since the Sphere is offset from pointing device, it will not be coincident with the pointing device bearing, but WILL move in the exact same relative direction. </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> identifier </i> </td>
			<td> <a href="https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/PlaneSensor/translation_changed.x3d" target="_blank">https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/PlaneSensor/translation_changed.x3d</a> </td>
		</tr>
		<tr>
...
public class translation_changed
{
	/** Default constructor to create this object. */
	public translation_changed ()
	{
	  initialize();
	}
  ...
  .setHead(new head()
    .addMeta(new meta().setName(meta.NAME_TITLE      ).setContent("translation_changed.x3d"))
    .addMeta(new meta().setName(meta.NAME_MOVINGIMAGE).setContent("translation_changed-movie.mpg"))
    ...
    .addMeta(new meta().setName(meta.NAME_DESCRIPTION).setContent("Test of the translation_changed field. A Box geometry has an associated PlaneSensor, which, when the Box is indicated by the pointing device, should route its translation_changed event to the Transform of a red Sphere geometry. The translation_changed value should equal the relative change in position between the current pointing device intersection point with the PlaneSensor and the previous position, PLUS the offset value (in this case a value of 0 0 1). The result is, the the primary pointing device button is pressed while over the Box, a red Sphere should appear initially on the Box, and move an equal distance in both X and Y as the pointing device, as it is dragged along the invisible XY plane. Since the Sphere is offset from pointing device, it will not be coincident with the pointing device bearing, but WILL move in the exact same relative direction."))
    .addMeta(new meta().setName(meta.NAME_IDENTIFIER ).setContent("https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/PlaneSensor/translation_changed.x3d"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit"))
    .addMeta(new meta().setName(meta.NAME_LICENSE    ).setContent("../../license.html")))
  .setScene(new Scene()
    .addChild(new WorldInfo().setTitle("translation_changed.x3d"))
    ...
    .addChild(new Group()
      .addChild(new Transform("MYBOX")
        .addChild(new Transform("BOX_BALL")
          .addChild(new Shape()
            .setAppearance(new Appearance()
              .setMaterial(new Material().setDiffuseColor(1.0,0.0,0.0)))
            .setGeometry(new Sphere().setRadius(0.3))))
        .addChild(new PlaneSensor("BOXSENSOR").setDescription("click and drag to move object").setOffset(0.0,0.0,1.0))
        .addChild(new Shape()
          .setAppearance(new Appearance()
            .setMaterial(new Material().setDiffuseColor(1.0,1.0,0.0)))
          .setGeometry(new Box()))))
    .addChild(new ROUTE().setFromNode("BOXSENSOR").setFromField("translation_changed").setToNode("BOX_BALL").setToField("set_translation")));
    }
	 ...
	 * @return translation_changed model
	 */
    ...
    {
        X3D thisExampleX3dModel = new translation_changed().getX3dModel();
		...
		if      (argumentsLoadNewModel)
			System.out.println("WARNING: \"net.x3djsonld.data.translation_changed\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		else if (hasArguments) // if no arguments provided, this method produces usage warning
			thisExampleX3dModel.handleArguments(args);

		if (validate)
		{
			System.out.print("Java program \"net.x3djsonld.data.translation_changed\" self-validation test results: ");
