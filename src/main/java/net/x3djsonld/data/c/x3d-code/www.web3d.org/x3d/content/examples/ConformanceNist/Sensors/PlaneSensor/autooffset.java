...
/**
 * <p> Two Box geometry each have a PlaneSensor associated with them. The PlaneSensor for each Box has an offset value that translates the Box geometry -1 1 0 (for the left Box) and 1 1 0 (for the right Box) direction when that geometry is initially selected. The left PlaneSensor, associated with the blue box has its autoOffset value set to false. The right PlaneSensor, associated with the green Box has its autoOffset field set to true. The result is,both boxes should translate to their initial offset when indicated. After that, the blue box should return to that position each time it is selected. The green box should NOT return to that position each time it is selected. </p>
 <p> Related links: <a href="../../../../Sensors/PlaneSensor/autooffset.java">autooffset.java</a> source, <a href="../../../../Sensors/PlaneSensor/autooffsetIndex.html" target="_top">autooffset catalog page</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dResources.html" target="_blank">X3D Resources</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dSceneAuthoringHints.html" target="_blank">X3D Scene Authoring Hints</a>, and <a href="https://www.web3d.org/x3d/content/X3dTooltips.html" target="_blank">X3D Tooltips</a>. </p>
	<table style="color:black; border:0px solid; border-spacing:10px 0px;">
		...
		<tr>
			...
			<td> <a href="../../../../Sensors/PlaneSensor/autooffset.x3d">autooffset.x3d</a> </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> MovingImage </i> </td>
			<td> <a href="../../../../Sensors/PlaneSensor/autooffset-movie.mpg">autooffset-movie.mpg</a> </td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td> Two Box geometry each have a PlaneSensor associated with them. The PlaneSensor for each Box has an offset value that translates the Box geometry -1 1 0 (for the left Box) and 1 1 0 (for the right Box) direction when that geometry is initially selected. The left PlaneSensor, associated with the blue box has its autoOffset value set to false. The right PlaneSensor, associated with the green Box has its autoOffset field set to true. The result is,both boxes should translate to their initial offset when indicated. After that, the blue box should return to that position each time it is selected. The green box should NOT return to that position each time it is selected. </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> identifier </i> </td>
			<td> <a href="https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/PlaneSensor/autooffset.x3d" target="_blank">https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/PlaneSensor/autooffset.x3d</a> </td>
		</tr>
		<tr>
...
public class autooffset
{
  ...
  .setHead(new head()
    .addMeta(new meta().setName(meta.NAME_TITLE      ).setContent("autooffset.x3d"))
    .addMeta(new meta().setName(meta.NAME_MOVINGIMAGE).setContent("autooffset-movie.mpg"))
    ...
    .addMeta(new meta().setName(meta.NAME_DESCRIPTION).setContent("Two Box geometry each have a PlaneSensor associated with them. The PlaneSensor for each Box has an offset value that translates the Box geometry -1 1 0 (for the left Box) and 1 1 0 (for the right Box) direction when that geometry is initially selected. The left PlaneSensor, associated with the blue box has its autoOffset value set to false. The right PlaneSensor, associated with the green Box has its autoOffset field set to true. The result is,both boxes should translate to their initial offset when indicated. After that, the blue box should return to that position each time it is selected. The green box should NOT return to that position each time it is selected."))
    .addMeta(new meta().setName(meta.NAME_IDENTIFIER ).setContent("https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/PlaneSensor/autooffset.x3d"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit"))
    .addMeta(new meta().setName(meta.NAME_LICENSE    ).setContent("../../license.html")))
  .setScene(new Scene()
    .addChild(new WorldInfo().setTitle("autooffset.x3d"))
    .addChild(new Group()
      .addChild(new Transform("BLUE_BOX").setTranslation(-1.0,0.0,0.0)
        .addChild(new PlaneSensor("BLUE_SENSOR").setDescription("click and drag to move object").setAutoOffset(false).setOffset(-1.0,1.0,0.0))
        .addChild(new Shape()
      ...
      .addChild(new Transform("GREEN_BOX").setTranslation(1.0,0.0,0.0)
        .addChild(new PlaneSensor("GREEN_SENSOR").setDescription("click and drag to move object").setOffset(1.0,1.0,0.0))
        .addChild(new Shape()
          .setAppearance(new Appearance()
    ...
    {
        X3D thisExampleX3dModel = new autooffset().getX3dModel();
		...
		if      (argumentsLoadNewModel)
			System.out.println("WARNING: \"net.x3djsonld.data.autooffset\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		else if (hasArguments) // if no arguments provided, this method produces usage warning
			thisExampleX3dModel.handleArguments(args);

		if (validate)
		{
			System.out.print("Java program \"net.x3djsonld.data.autooffset\" self-validation test results: ");
