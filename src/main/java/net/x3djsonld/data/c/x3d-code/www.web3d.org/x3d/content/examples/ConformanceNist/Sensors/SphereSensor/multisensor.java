 ...
 <p> Related links: <a href="../../../../Sensors/SphereSensor/multisensor.java">multisensor.java</a> source, <a href="../../../../Sensors/SphereSensor/multisensorIndex.html" target="_top">multisensor catalog page</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dResources.html" target="_blank">X3D Resources</a>, <a href="https://www.web3d.org/x3d/content/examples/X3dSceneAuthoringHints.html" target="_blank">X3D Scene Authoring Hints</a>, and <a href="https://www.web3d.org/x3d/content/X3dTooltips.html" target="_blank">X3D Tooltips</a>. </p>
	<table style="color:black; border:0px solid; border-spacing:10px 0px;">
		...
		<tr>
			...
			<td> <a href="../../../../Sensors/SphereSensor/multisensor.x3d">multisensor.x3d</a> </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> MovingImage </i> </td>
			<td> <a href="../../../../Sensors/SphereSensor/multisensor-movie.mpg">multisensor-movie.mpg</a> </td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td> Test if a SphereSensor grabs all further motion events from pointing device when the sensor generates "isActive" true event for primitive geometry. The Box geometry has an associated SphereSensor that routes its "rotation_changed" events to that geometrys Transform node. As a result, pressing the primary pointing device button while over the Box should rotate that geometry. In addition, a TouchSensor is assocated with the Box geometry and a PointLight, but is nested "higher" in the sensor hierarchy. Passing the primary pointing device over the Box should route and isActive event to a PointLight and illuminate the IndexedFaceSets. Dragging the pointing device over the Box geometry and holding the primary pointing device button down should generate an isActive = true event for the SphereSensor and prevent the TouchSensor from activating (and the PointLight from coming ON), When the button is released, and the cursor is over the Box, the PointLight should turn on. </td>
		</tr>
		<tr>
			<td style="text-align:right; vertical-align: text-top;"> <i> identifier </i> </td>
			<td> <a href="https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/SphereSensor/multisensor.x3d" target="_blank">https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/SphereSensor/multisensor.x3d</a> </td>
		</tr>
		<tr>
...
public class multisensor
{
  ...
  .setHead(new head()
    .addMeta(new meta().setName(meta.NAME_TITLE      ).setContent("multisensor.x3d"))
    .addMeta(new meta().setName(meta.NAME_MOVINGIMAGE).setContent("multisensor-movie.mpg"))
    ...
    .addMeta(new meta().setName(meta.NAME_DESCRIPTION).setContent("Test if a SphereSensor grabs all further motion events from pointing device when the sensor generates \"isActive\" true event for primitive geometry. The Box geometry has an associated SphereSensor that routes its \"rotation_changed\" events to that geometrys Transform node. As a result, pressing the primary pointing device button while over the Box should rotate that geometry. In addition, a TouchSensor is assocated with the Box geometry and a PointLight, but is nested \"higher\" in the sensor hierarchy. Passing the primary pointing device over the Box should route and isActive event to a PointLight and illuminate the IndexedFaceSets. Dragging the pointing device over the Box geometry and holding the primary pointing device button down should generate an isActive = true event for the SphereSensor and prevent the TouchSensor from activating (and the PointLight from coming ON), When the button is released, and the cursor is over the Box, the PointLight should turn on."))
    .addMeta(new meta().setName(meta.NAME_IDENTIFIER ).setContent("https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/SphereSensor/multisensor.x3d"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html"))
    .addMeta(new meta().setName(meta.NAME_GENERATOR  ).setContent("X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit"))
    .addMeta(new meta().setName(meta.NAME_LICENSE    ).setContent("../../license.html")))
  .setScene(new Scene()
    .addChild(new WorldInfo().setTitle("multisensor.x3d"))
    ...
    .addChild(new Group()
      ...
      .addChild(new Viewpoint().setDescription("Left View").setOrientation(0.0,1.0,0.0,-1.57).setPosition(-10.0,0.0,0.0)))
    .addChild(new Group()
      .addChild(new PointLight("LIGHT").setLocation(0.0,0.0,2.0).setOn(false))
      ...
      .addChild(new Transform()
        ...
        .addChild(new Transform()
          .addChild(new SphereSensor("BOXSENSOR").setDescription("click and drag to rotate"))
    ...
    .addChild(new ROUTE().setFromNode("TOUCH").setFromField("isOver").setToNode("LIGHT").setToField("on")));
    }
    ...
    {
        X3D thisExampleX3dModel = new multisensor().getX3dModel();
		...
		if      (argumentsLoadNewModel)
			System.out.println("WARNING: \"net.x3djsonld.data.multisensor\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		else if (hasArguments) // if no arguments provided, this method produces usage warning
			thisExampleX3dModel.handleArguments(args);

		if (validate)
		{
			System.out.print("Java program \"net.x3djsonld.data.multisensor\" self-validation test results: ");
