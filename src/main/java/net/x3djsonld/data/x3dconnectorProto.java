...
public class x3dconnectorProto
{
  ...
  .setScene(new SceneObject()
    ...
    .addChild(new TransformObject("G3").setTranslation(new SFVec3fObject(new float[] {1.0f,1.0f,.01f}))
      .addChild(new ShapeObject()
        .setAppearance(new AppearanceObject()
          .setMaterial(new MaterialObject().setDiffuseColor(new SFColorObject(new float[] {0.2f,0.7f,0.2f}))))
        .setGeometry(new SphereObject().setRadius(.1f)))
      .addChild(new PlaneSensorObject("PS3").setDescription("Grab to move").setOffset(new SFVec3fObject(new float[] {1.0f,1.0f,.01f})))
      .addChild(new ROUTEObject().setFromNode("PS3").setFromField("translation_changed").setToNode("G3").setToField("set_translation")))
    .addChild(new TransformObject("G4").setTranslation(new SFVec3fObject(new float[] {-1.0f,1.0f,.01f}))
      .addChild(new ShapeObject()
        .setAppearance(new AppearanceObject()
          .setMaterial(new MaterialObject().setDiffuseColor(new SFColorObject(new float[] {0.2f,0.7f,0.2f}))))
        .setGeometry(new SphereObject().setRadius(.1f)))
      .addChild(new PlaneSensorObject("PS4").setDescription("Grab to move").setOffset(new SFVec3fObject(new float[] {-1.0f,1.0f,.01f})))
...
"        }" + "\n")
    ...
    {
        X3DObject thisExampleX3dObject = new x3dconnectorProto().getX3dModel();
		...
		if      (argumentsLoadNewModel)
			System.out.print("WARNING: \"x3dconnectorProto\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		else if (hasArguments) // if no arguments provided, this method produces usage warning
			thisExampleX3dObject.handleArguments(args);

		if (validate)
		{
			System.out.print("Java program \"x3dconnectorProto\" self-validation test results: ");
