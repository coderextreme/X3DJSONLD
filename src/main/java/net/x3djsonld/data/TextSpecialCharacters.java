...
public class TextSpecialCharacters
{
  ...
  .setHead(new head()
    ...
    .addMeta(new meta().setName(meta.NAME_REFERENCE  ).setContent("Character entity references in HTML 4"))
    .addMeta(new meta().setName(meta.NAME_REFERENCE  ).setContent("http://www.w3.org/TR/REC-html40/sgml/entities.html"))
    .addMeta(new meta().setName(meta.NAME_RIGHTS     ).setContent("Copyright (c) Don Brutzman and Leonard Daly, 2008"))
    .addMeta(new meta().setName(meta.NAME_IDENTIFIER ).setContent("http://X3dGraphics.com/examples/X3dForWebAuthors/Chapter02GeometryPrimitives/TextSpecialCharacters.x3d"))
  ...
  .setScene(new Scene()
    ...
    public static void main(String args[])
    {
		...
		if      (argumentsLoadNewModel)
			System.out.println("WARNING: \"net.x3djsonld.data.TextSpecialCharacters\" model invocation is attempting to load file \"" + fileName + "\" instead of simply validating itself... file loading ignored.");
		else if (hasArguments) // if no arguments provided, this method produces usage warning
			thisExampleX3dModel.handleArguments(args);

		if (validate)
		{
			System.out.print("Java program \"net.x3djsonld.data.TextSpecialCharacters\" self-validation test results: ");
