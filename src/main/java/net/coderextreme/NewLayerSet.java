...
public class NewLayerSet extends org.web3d.x3d.jsail.Layering.LayerSet {
	private X3DNode[] layers = null;
	public X3DNode[] getLayers() { layers = super.getLayers(); return layers; }
	public X3DNode[] removeSelectedLayers(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getLayers()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setLayers(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getLayers().length);
		return super.getLayers();
	}
	public LayerSet setLayers(X3DNode[] leftOver) { layers = leftOver; super.setLayers(leftOver); return this; }
