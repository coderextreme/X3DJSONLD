...
public class NewRigidBody extends org.web3d.x3d.jsail.RigidBodyPhysics.RigidBody {
	private X3DNode[] geometry = null;
	public X3DNode[] getGeometry() { geometry = super.getGeometry(); return geometry; }
	public X3DNode[] removeSelectedGeometry(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getGeometry()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setGeometry(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getGeometry().length);
		return super.getGeometry();
	}
	public RigidBody setGeometry(X3DNode[] leftOver) { geometry = leftOver; super.setGeometry(leftOver); return this; }
