...
public class NewMultiTextureCoordinate extends org.web3d.x3d.jsail.Texturing.MultiTextureCoordinate {
	private X3DNode[] texCoord = null;
	public X3DNode[] getTexCoord() { texCoord = super.getTexCoord(); return texCoord; }
	public X3DNode[] removeSelectedTexCoord(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getTexCoord()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setTexCoord(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getTexCoord().length);
		return super.getTexCoord();
	}
	public MultiTextureCoordinate setTexCoord(X3DNode[] leftOver) { texCoord = leftOver; super.setTexCoord(leftOver); return this; }
