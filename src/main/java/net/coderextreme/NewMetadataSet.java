...
public class NewMetadataSet extends org.web3d.x3d.jsail.Core.MetadataSet {
	private X3DNode[] value = null;
	public X3DNode[] getValue() { value = super.getValue(); return value; }
	public X3DNode[] removeSelectedValue(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getValue()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setValue(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getValue().length);
		return super.getValue();
	}
	public MetadataSet setValue(X3DNode[] leftOver) { value = leftOver; super.setValue(leftOver); return this; }
