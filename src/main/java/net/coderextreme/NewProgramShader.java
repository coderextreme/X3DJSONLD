...
public class NewProgramShader extends org.web3d.x3d.jsail.Shaders.ProgramShader {
	private X3DNode[] programs = null;
	public X3DNode[] getPrograms() { programs = super.getPrograms(); return programs; }
	public X3DNode[] removeSelectedPrograms(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getPrograms()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setPrograms(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getPrograms().length);
		return super.getPrograms();
	}
	public ProgramShader setPrograms(X3DNode[] leftOver) { programs = leftOver; super.setPrograms(leftOver); return this; }
