...
ProtoInstanceObject ProtoInstance0 = null;
      ...
      .setScene(new SceneObject()
        ...
        .addChild(new ExternProtoDeclareObject().setAppinfo("Heads-up display (HUD) keeps child geometry aligned on screen in a consistent location").setName("HeadsUpDisplay").setUrl(new MFStringObject(new MFString0().getArray()))
          ...
          .addField(new fieldObject().setType("SFVec3f").setAccessType(fieldObject.ACCESSTYPE_OUTPUTONLY).setAppinfo("HUD position update (in world coordinates) relative to original location").setName("position_changed"))
          .addField(new fieldObject().setType("SFRotation").setAccessType(fieldObject.ACCESSTYPE_OUTPUTONLY).setAppinfo("HUD orientation update relative to original location").setName("orientation_changed")))
