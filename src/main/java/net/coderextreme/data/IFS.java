...
public class IFS {
  public static void main(String[] args) {
    ...
    X3D model = new IFS().initialize();
    System.out.print(model.validationReport().trim());
    model.toFileJSON("../data/IFS.new.json");
    }
    public X3D initialize() {
      X3D X3D0 =  new X3D().setProfile("Immersive").setVersion("4.0")
      .setHead(new head()
        .addComponent(new component().setName("HAnim").setLevel(1))
        .addMeta(new meta().setName("title").setContent("JohnIFS.x3d"))
        ...
        .addChild(new Group()
          ...
          .addChild(new Transform().setTranslation(new float[] {0f,2.1f,0f})
            .addChild(new Shape().setDEF("HAnimSiteShape")
              .setGeometry(new IndexedFaceSet("HAnimSiteShape").setDEF("DiamondIFS").setCreaseAngle(0.5f).setSolid(false).setCoordIndex(new MFInt320().getArray())
                .setColor(new ColorRGBA("DiamondIFS").setDEF("HAnimSiteColorRGBA").setColor(new MFColorRGBA1().getArray()))
                .setCoord(new Coordinate("DiamondIFS").setPoint(new MFVec3f2().getArray())))
