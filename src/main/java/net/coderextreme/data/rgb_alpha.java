...
public class rgb_alpha implements X3DRoots {
  public static void main(String[] args) {
    ...
    public X3D initialize() {
      ...
      .setHead(new head()
        .addMeta(new meta().setName(new SFString("title")).setContent(new SFString("rgb_alpha.x3d")))
        .addMeta(new meta().setName(new SFString("Image")).setContent(new SFString("rgb_alpha-front.jpg")))
        .addMeta(new meta().setName(new SFString("Image")).setContent(new SFString("rgb_alpha-rear.jpg")))
        .addMeta(new meta().setName(new SFString("Image")).setContent(new SFString("rgb_alpha-top.jpg")))
        .addMeta(new meta().setName(new SFString("Image")).setContent(new SFString("rgb_alpha-bottom.jpg")))
        ...
        .addMeta(new meta().setName(new SFString("modified")).setContent(new SFString("16 January 2011")))
        .addMeta(new meta().setName(new SFString("description")).setContent(new SFString("Test browser ability to map a RGB plus alpha opacity to geometry. A checkerboard of four colored squares: lower left (red), lower right (transparent), uppser left (transparent) and upper right (red) map onto the faces of all geometry. For the sphere, the texture should cover the entire surface, and wrap counterclockwise from the back of the sphere. For the cone, the texture should wrap counterclockwise (from above) starting at the back of the cone. A circle cutout of the texture is applied right side up to the base of the cone when the cone is tilted toward the -z axis. For the cylinder, the texture should wrap counterclockwise (from above) starting at the back of the cylinder. A circle cutout of the texture is applied right side up to the top and bottom caps of the cylinder. For the box, the texture should be applied right side up in its entirety to each face of the box.")))
        .addMeta(new meta().setName(new SFString("identifier")).setContent(new SFString("https://www.web3d.org/x3d/content/examples/ConformanceNist/Appearance/PixelTexture/rgb_alpha.x3d")))
          ...
          .addChild(new Transform().setTranslation(new float[] {6.14221f ,0.0694613f ,-0.000999451f })
            .addChild(new Shape()
              .setAppearance(new Appearance()
                ...
                .setTexture(new PixelTexture().setDEF(new SFString("RgbOpacityCheckerboard")).setImage(new int[] {2,2,4,0xFF0000FF,0xFFFF0000,0xFFFF0000,0xFF0000FF})))
              .setGeometry(new Box())))
          .addChild(new Transform().setTranslation(new float[] {-4.85443f ,0.0694381f ,-0.00149918f })
            .addChild(new Shape()
              .setAppearance(new Appearance()
                .setMaterial(new Material())
                .setTexture(new PixelTexture().setUSE(new SFString("RgbOpacityCheckerboard"))))
