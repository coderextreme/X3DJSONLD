...
public class InlineSoundSource implements X3DRoots {
  public static void main(String[] args) {
    ...
    public X3D initialize() {
      ...
      .setHead(new head()
        .addMeta(new meta().setName(new SFString("title")).setContent(new SFString("InlineSoundSource.x3d")))
        .addMeta(new meta().setName(new SFString("description")).setContent(new SFString("This demo is the same with the Single Audio Demo, with the difference that it uses the Inline grouping node. Namely, the scene is an X3D file and all nodes are loaded by Inline node via the \"url\" of X3D file.")))
        ...
        .addMeta(new meta().setName(new SFString("modified")).setContent(new SFString("4 August 2021")))
        ...
        .addMeta(new meta().setName(new SFString("reference")).setContent(new SFString("https://medialab.hmu.gr/minipages/x3domAudio")))
        .addMeta(new meta().setName(new SFString("identifier")).setContent(new SFString("https://x3dgraphics.com/examples/X3dForAdvancedModeling/AudioSpatialSound/InlineSoundSource.x3d")))
      ...
      .setScene(new Scene()
        .addChild(new WorldInfo().setTitle(new SFString("InlineSoundSource.x3d")))
        .addChild(new NavigationInfo().setDEF(new SFString("NAV")))
        ...
        .addChild(new Viewpoint().setDEF(new SFString("Camera001")).setDescription(new SFString("Camera001")).setFarDistance(0f ).setNearDistance(1f ).setOrientation(new float[] {1f ,0f ,0f ,-0.523599f }).setPosition(new float[] {0f ,2000f ,3500f }))
        .addChild(new Transform().setDEF(new SFString("Floor")).setTranslation(new float[] {1.241f ,0f ,0.358f })
          .addChild(new Shape()
            .setAppearance(new Appearance().setDEF(new SFString("WireColor"))
              .setMaterial(new Material().setDiffuseColor(new float[] {0.122f ,0.114f ,0.125f })))
            .setGeometry(new IndexedFaceSet().setDEF(new SFString("Box001-GEOMETRY")).setCoordIndex(new MFInt326().getArray()).setSolid(false)
              .setCoord(new Coordinate().setPoint(new MFVec3f7().getArray()))
              .setNormal(new Normal().setVector(new MFVec3f8().getArray())))))
        .addChild(new Transform().setDEF(new SFString("InlineScene"))
          .addChild(new Inline().setDEF(new SFString("inline")).setUrl(new MFString9().getArray()))));
    return X3D0;
    }
