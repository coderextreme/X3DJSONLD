...
public class Filters implements X3DRoots {
  public static void main(String[] args) {
    ...
    public X3D initialize() {
      ...
      .setHead(new head()
        .addMeta(new meta().setName(new SFString("title")).setContent(new SFString("Filters.x3d")))
        .addMeta(new meta().setName(new SFString("description")).setContent(new SFString("This X3D scene involves three sound sources. Each of them is visualized by a 3D object (in our case is a sphere) that depicts the sound effects. Specifically, we have added filters through of them we are able to manage the different sound effects in an impressive way. Filters can be composed of a number of attributes, frequency, detune, gain and the quality factor which also known as Q. Furthermore, the filters are classified in some specific types, depending on the sound effects that produce. In detail, there is the Low-pass filter which can create more muffled sound. Another one is the High-pass filter, which is used to generate tinny sound. Equally important is the Band-pass filter, which cuts off low and high frequencies and passes through only these within a certain range. On the contrary, the Notch filter has exactly the opposite operation of the Band-pass filter. Then is the Low-shelf filter, its role is to change the amount of bass in a sound, as a result the frequencies that are lower than the current frequency get a boost, while them that are over it remain unchanged. Next, the High-shelf filter is responsible for the quantity of treble in a sound. Moreover, Peaking filter is used in order to handle the amount of midrange in a sound. Lastly, there is the All-pass filter, whose role is to introduce phaser effects.")))
        .addMeta(new meta().setName(new SFString("info")).setContent(new SFString("This work presents an innovative solution of the spatial sound in X3DOM framework, that based on a combinational methodology. Specifically, we suggested the enrichment of X3DOM with spatial sound features, using both the X3D sound nodes and the structure of Web Audio API.")))
        .addMeta(new meta().setName(new SFString("creator")).setContent(new SFString("Efi Lakka, Athanasios Malamos, Dick Puk, Don Brutzman")))
        .addMeta(new meta().setName(new SFString("created")).setContent(new SFString("28 October 2020")))
        .addMeta(new meta().setName(new SFString("modified")).setContent(new SFString("5 December 2021")))
        .addMeta(new meta().setName(new SFString("reference")).setContent(new SFString("CHANGELOG.txt")))
        .addMeta(new meta().setName(new SFString("TODO")).setContent(new SFString("credit for audio files")))
        .addMeta(new meta().setName(new SFString("reference")).setContent(new SFString("https://www.medialab.hmu.gr/minipages/x3domAudio")))
        .addMeta(new meta().setName(new SFString("identifier")).setContent(new SFString("https://x3dgraphics.com/examples/X3dForAdvancedModeling/AudioSpatialSound/Filters.x3d")))
      ...
      .setScene(new Scene()
        .addChild(new WorldInfo().setTitle(new SFString("Filters.x3d")))
        .addChild(new NavigationInfo())
        .addChild(new Background().setSkyColor(new MFColor0().getArray()))
        .addChild(new Viewpoint().setDescription(new SFString("View entire audio model")).setOrientation(new float[] {1f ,0f ,0f ,-0.5f }).setPosition(new float[] {0f ,500f ,600f }).setRetainUserOffsets(true))
        .addChild(new Transform().setDEF(new SFString("TransformAudio1")).setTranslation(new float[] {-200f ,50f ,0f })
          .addChild(new Shape()
            .setAppearance(new Appearance().setDEF(new SFString("audio_emit"))
        ...
        .addChild(new Transform().setDEF(new SFString("TransformAudio2")).setTranslation(new float[] {0f ,50f ,0f })
          .addChild(new Shape()
            .setAppearance(new Appearance().setDEF(new SFString("audio_emit2"))
              .setMaterial(new Material().setDiffuseColor(new float[] {0.1f ,0.1f ,0.1f }).setEmissiveColor(new float[] {0.8f ,0.8f ,0.8f }).setSpecularColor(new float[] {0.01f ,0.01f ,0.01f })))
            .setGeometry(new Sphere().setRadius(30f ))))
        .addChild(new Transform().setDEF(new SFString("TransformAudio3")).setTranslation(new float[] {200f ,50f ,0f })
          .addChild(new Shape()
            .setAppearance(new Appearance().setDEF(new SFString("audio_emit3"))
              .setMaterial(new Material().setDiffuseColor(new float[] {0.1f ,0.1f ,0.1f }).setEmissiveColor(new float[] {0.8f ,0.8f ,0.8f }).setSpecularColor(new float[] {0.01f ,0.01f ,0.01f })))
            .setGeometry(new Sphere().setRadius(30f ))))
        .addChild(new Transform().setDEF(new SFString("AnimData")).setTranslation(new float[] {0f ,50f ,0f }))
        .addChild(new Transform().setDEF(new SFString("AnimDataBoxH")).setTranslation(new float[] {0f ,100f ,0f }))
        .addChild(new Transform().setDEF(new SFString("AnimDataBoxM")).setTranslation(new float[] {15f ,100f ,0f }))
        .addChild(new Transform().setDEF(new SFString("AnimDataBoxL")).setTranslation(new float[] {30f ,100f ,0f }))
        .addChild(new Transform().setDEF(new SFString("AnimDataBoxMM")).setTranslation(new float[] {-15f ,100f ,0f }))
        .addChild(new Transform().setDEF(new SFString("AnimDataBoxLM")).setTranslation(new float[] {-30f ,100f ,0f }))
        .addChild(new Transform()
          .addChild(new Shape()
            .setAppearance(new Appearance().setDEF(new SFString("floor"))
        ...
        .addChild(new StreamAudioDestination()
          .addChild(new DynamicsCompressor()
            .addChild(new Gain()
              .addChild(new SpatialSound().setDEF(new SFString("Audio1"))
                .addChild(new Gain()
                  .addChild(new Analyser()
                    .addChild(new BiquadFilter().setType("ALLPASS").setDetune(50f ).setFrequency(600f ).setQualityFactor(30f )
                      .addChild(new AudioClip().setDescription(new SFString("Techno beat")).setLoop(true).setUrl(new MFString1().getArray()))))))
              .addChild(new SpatialSound().setDEF(new SFString("Audio2"))
                .addChild(new Gain()
                  .addChild(new Analyser()
                    .addChild(new BiquadFilter().setType("ALLPASS").setDetune(15f ).setFrequency(600f ).setQualityFactor(15f )
                      .addChild(new AudioClip().setDescription(new SFString("Simple beat")).setLoop(true).setUrl(new MFString2().getArray()))))))
              .addChild(new SpatialSound().setDEF(new SFString("Audio3"))
                .addChild(new Gain()
                  .addChild(new Analyser()
                    .addChild(new BiquadFilter().setType("ALLPASS").setFrequency(1000f ).setQualityFactor(0f )
                      .addChild(new AudioClip().setDescription(new SFString("Wobble loop")).setLoop(true).setUrl(new MFString3().getArray()))))))))));
    return X3D0;
    }
