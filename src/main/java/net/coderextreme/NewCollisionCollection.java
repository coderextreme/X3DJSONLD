...
public class NewCollisionCollection extends org.web3d.x3d.jsail.RigidBodyPhysics.CollisionCollection {
	private X3DNode[] collidables = null;
	public X3DNode[] getCollidables() { collidables = super.getCollidables(); return collidables; }
	public X3DNode[] removeSelectedCollidables(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getCollidables()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setCollidables(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getCollidables().length);
		return super.getCollidables();
	}
	public CollisionCollection setCollidables(X3DNode[] leftOver) { collidables = leftOver; super.setCollidables(leftOver); return this; }
