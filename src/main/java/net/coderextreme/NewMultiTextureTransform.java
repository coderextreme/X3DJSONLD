...
public class NewMultiTextureTransform extends org.web3d.x3d.jsail.Texturing.MultiTextureTransform {
	private X3DNode[] textureTransform = null;
	public X3DNode[] getTextureTransform() { textureTransform = super.getTextureTransform(); return textureTransform; }
	public X3DNode[] removeSelectedTextureTransform(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getTextureTransform()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setTextureTransform(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getTextureTransform().length);
		return super.getTextureTransform();
	}
	public MultiTextureTransform setTextureTransform(X3DNode[] leftOver) { textureTransform = leftOver; super.setTextureTransform(leftOver); return this; }
