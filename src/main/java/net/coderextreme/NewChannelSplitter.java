...
public class NewChannelSplitter extends org.web3d.x3d.jsail.Sound.ChannelSplitter {
	...
	public X3DNode[] removeSelectedChildren(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getChildren()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setChildren(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getChildren().length);
		return super.getChildren();
	}
	public ChannelSplitter setChildren(X3DNode[] leftOver) { children = leftOver; super.setChildren(leftOver); return this; }
	private X3DNode[] outputs = null;
	public X3DNode[] getOutputs() { outputs = super.getOutputs(); return outputs; }
	public X3DNode[] removeSelectedOutputs(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getOutputs()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setOutputs(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getOutputs().length);
		return super.getOutputs();
	}
	public ChannelSplitter setOutputs(X3DNode[] leftOver) { outputs = leftOver; super.setOutputs(leftOver); return this; }
