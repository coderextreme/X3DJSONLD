...
public class NewMultiTexture extends org.web3d.x3d.jsail.Texturing.MultiTexture {
	private X3DNode[] texture = null;
	public X3DNode[] getTexture() { texture = super.getTexture(); return texture; }
	public X3DNode[] removeSelectedTexture(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getTexture()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setTexture(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getTexture().length);
		return super.getTexture();
	}
	public MultiTexture setTexture(X3DNode[] leftOver) { texture = leftOver; super.setTexture(leftOver); return this; }
