...
public class NewParticleSystem extends org.web3d.x3d.jsail.ParticleSystems.ParticleSystem {
	private X3DNode[] physics = null;
	public X3DNode[] getPhysics() { physics = super.getPhysics(); return physics; }
	public X3DNode[] removeSelectedPhysics(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getPhysics()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setPhysics(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getPhysics().length);
		return super.getPhysics();
	}
	public ParticleSystem setPhysics(X3DNode[] leftOver) { physics = leftOver; super.setPhysics(leftOver); return this; }
