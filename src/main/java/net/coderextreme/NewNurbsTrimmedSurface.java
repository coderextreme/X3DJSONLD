...
public class NewNurbsTrimmedSurface extends org.web3d.x3d.jsail.NURBS.NurbsTrimmedSurface {
	private X3DNode[] trimmingContour = null;
	public X3DNode[] getTrimmingContour() { trimmingContour = super.getTrimmingContour(); return trimmingContour; }
	public X3DNode[] removeSelectedTrimmingContour(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getTrimmingContour()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setTrimmingContour(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getTrimmingContour().length);
		return super.getTrimmingContour();
	}
	public NurbsTrimmedSurface setTrimmingContour(X3DNode[] leftOver) { trimmingContour = leftOver; super.setTrimmingContour(leftOver); return this; }
