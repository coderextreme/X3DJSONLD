...
public class NewHAnimSegment extends org.web3d.x3d.jsail.HAnim.HAnimSegment {
	...
	public X3DNode[] removeSelectedDisplacers(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getDisplacers()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setDisplacers(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getDisplacers().length);
