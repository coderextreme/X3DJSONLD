...
public class NewIsoSurfaceVolumeData extends org.web3d.x3d.jsail.VolumeRendering.IsoSurfaceVolumeData {
	private X3DNode[] renderStyle = null;
	public X3DNode[] getRenderStyle() { renderStyle = super.getRenderStyle(); return renderStyle; }
	public X3DNode[] removeSelectedRenderStyle(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getRenderStyle()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setRenderStyle(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getRenderStyle().length);
		return super.getRenderStyle();
	}
	public IsoSurfaceVolumeData setRenderStyle(X3DNode[] leftOver) { renderStyle = leftOver; super.setRenderStyle(leftOver); return this; }
