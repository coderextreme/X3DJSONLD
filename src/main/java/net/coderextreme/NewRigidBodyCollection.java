...
public class NewRigidBodyCollection extends org.web3d.x3d.jsail.RigidBodyPhysics.RigidBodyCollection {
	private X3DNode[] bodies = null;
	public X3DNode[] getBodies() { bodies = super.getBodies(); return bodies; }
	public X3DNode[] removeSelectedBodies(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getBodies()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setBodies(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getBodies().length);
		return super.getBodies();
	}
	public RigidBodyCollection setBodies(X3DNode[] leftOver) { bodies = leftOver; super.setBodies(leftOver); return this; }
	private X3DNode[] joints = null;
	public X3DNode[] getJoints() { joints = super.getJoints(); return joints; }
	public X3DNode[] removeSelectedJoints(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getJoints()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setJoints(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getJoints().length);
		return super.getJoints();
	}
	public RigidBodyCollection setJoints(X3DNode[] leftOver) { joints = leftOver; super.setJoints(leftOver); return this; }
