...
public class NewGeoMetadata extends org.web3d.x3d.jsail.Geospatial.GeoMetadata {
	private X3DNode[] data = null;
	public X3DNode[] getData() { data = super.getData(); return data; }
	public X3DNode[] removeSelectedData(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getData()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setData(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getData().length);
		return super.getData();
	}
	public GeoMetadata setData(X3DNode[] leftOver) { data = leftOver; super.setData(leftOver); return this; }
