...
public class NewPrimitivePickSensor extends org.web3d.x3d.jsail.Picking.PrimitivePickSensor {
	private X3DNode[] pickedGeometry = null;
	public X3DNode[] getPickedGeometry() { pickedGeometry = super.getPickedGeometry(); return pickedGeometry; }
	private X3DNode[] pickTarget = null;
	public X3DNode[] getPickTarget() { pickTarget = super.getPickTarget(); return pickTarget; }
	public X3DNode[] removeSelectedPickTarget(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getPickTarget()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setPickTarget(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getPickTarget().length);
		return super.getPickTarget();
	}
	public PrimitivePickSensor setPickTarget(X3DNode[] leftOver) { pickTarget = leftOver; super.setPickTarget(leftOver); return this; }
