...
public class NewIndexedTriangleStripSet extends org.web3d.x3d.jsail.Rendering.IndexedTriangleStripSet {
	private X3DNode[] attrib = null;
	public X3DNode[] getAttrib() { attrib = super.getAttrib(); return attrib; }
	public X3DNode[] removeSelectedAttrib(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getAttrib()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setAttrib(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getAttrib().length);
		return super.getAttrib();
	}
	public IndexedTriangleStripSet setAttrib(X3DNode[] leftOver) { attrib = leftOver; super.setAttrib(leftOver); return this; }
