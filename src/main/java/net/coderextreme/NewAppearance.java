...
public class NewAppearance extends org.web3d.x3d.jsail.Shape.Appearance {
	private X3DNode[] shaders = null;
	public X3DNode[] getShaders() { shaders = super.getShaders(); return shaders; }
	public X3DNode[] removeSelectedShaders(X3DNode[] children, HashSet toRemove) {
		List<X3DNode> list = Arrays.stream(super.getShaders()).collect(Collectors.toList());
		System.err.println("List length = "+list.size());
		if (list.removeAll(toRemove)) { super.setShaders(list.toArray(new X3DNode[0])); };
		System.err.println("After list length = "+super.getShaders().length);
		return super.getShaders();
	}
	public Appearance setShaders(X3DNode[] leftOver) { shaders = leftOver; super.setShaders(leftOver); return this; }
