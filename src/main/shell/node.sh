...
(ls ../node/net/*/data/*.js; find www_web3d_org -name '*.js') | sed 's/\.js/.new.x3d/' | xargs ls 2>&1| grep "No such file or directory" | awk '{ print $4; }'| sed "s/^'\.\///" | sed "s/\.new\.x3d':/.js/" | sed "s/^'//" | xargs -L 1 -P ${PROCESSORS-8} node 2> jserrors.txt
