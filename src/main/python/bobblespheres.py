...
# --- CORE RENDERING AND UPDATE LOGIC ---
def update_camera():
...
def render_scene():
    if not anim_state.needs_render:
        return
    #print("Rendering...")
    frame.render()
...
def on_motion(event):
    ...
    update_camera()
    render_scene() # **FIX 2**: Render immediately on drag for better responsiveness
...
def on_scroll(event):
    ...
    update_camera()
    render_scene() # **FIX 2**: Render immediately on scroll for better responsiveness
...
update_camera()
render_scene() # **FIX 1**: Render the very first frame before showing the window

timer = AnimationTimer(interval=33)
timer.start()
