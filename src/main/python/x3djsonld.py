...
def parseObject(parent, data,indent=0):
    out = "\n"+(("  ")*indent)
    els = 0
    if els == 0:
        ndent = 0
        ...
        ndent = 1
    if parent in ("meta", "field", "fieldValue", "connect"):
        out += parent+"("
    for k,v in data.items():
        ...
        if k.startswith("#"):
            ...
            if key in ("children", "url", "justify", "string", "field", "fieldValue", "connect"):
                out += "\n"*ndent+(("  ")*ndent)+key
                out += "=["
