from x3d import *
from X3Dautoclass import X3dClassLookup


X3dFieldTypesLookup = {
"AcousticProperties" : {
	"absorption" : SFFloat,
	"description" : SFString,
	"diffuse" : SFFloat,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"refraction" : SFFloat,
	"specular" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Analyser" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"fftSize" : SFInt32,
	"frequencyBinCount" : SFInt32,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"maxDecibels" : SFFloat,
	"metadata" : SFNode,
	"minDecibels" : SFFloat,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"smoothingTimeConstant" : SFFloat,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Anchor" : {
	"addChildren" : MFNode,
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"description" : SFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"parameter" : MFString,
	"removeChildren" : MFNode,
	"url" : MFString,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Appearance" : {
	"acousticProperties" : SFNode,
	"alphaCutoff" : SFFloat,
	"alphaMode" : SFString,
	"backMaterial" : SFNode,
	"fillProperties" : SFNode,
	"IS" : SFNode,
	"lineProperties" : SFNode,
	"material" : SFNode,
	"metadata" : SFNode,
	"pointProperties" : SFNode,
	"shaders" : MFNode,
	"texture" : SFNode,
	"textureTransform" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
	"blendMode" : SFNode,
	"depthMode" : SFNode,
},
"Arc2D" : {
	"endAngle" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"radius" : SFFloat,
	"startAngle" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ArcClose2D" : {
	"closureType" : SFString,
	"endAngle" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"radius" : SFFloat,
	"solid" : SFBool,
	"startAngle" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"AudioClip" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"duration_changed" : SFTime,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"load" : SFBool,
	"loop" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"pitch" : SFFloat,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"AudioDestination" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"maxChannelCount" : SFInt32,
	"mediaDeviceID" : SFString,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Background" : {
	"backUrl" : MFString,
	"bindTime" : SFTime,
	"bottomUrl" : MFString,
	"frontUrl" : MFString,
	"groundAngle" : MFFloat,
	"groundColor" : MFColor,
	"IS" : SFNode,
	"isBound" : SFBool,
	"leftUrl" : MFString,
	"metadata" : SFNode,
	"rightUrl" : MFString,
	"set_bind" : SFBool,
	"skyAngle" : MFFloat,
	"skyColor" : MFColor,
	"topUrl" : MFString,
	"transparency" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BallJoint" : {
	"anchorPoint" : SFVec3f,
	"body1" : SFNode,
	"body1AnchorPoint" : SFVec3f,
	"body2" : SFNode,
	"body2AnchorPoint" : SFVec3f,
	"forceOutput" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Billboard" : {
	"addChildren" : MFNode,
	"axisOfRotation" : SFVec3f,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BiquadFilter" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"detune" : SFFloat,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"frequency" : SFFloat,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"qualityFactor" : SFFloat,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"type" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BlendedVolumeStyle" : {
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"renderStyle" : SFNode,
	"voxels" : SFNode,
	"weightConstant1" : SFFloat,
	"weightConstant2" : SFFloat,
	"weightFunction1" : SFString,
	"weightFunction2" : SFString,
	"weightTransferFunction1" : SFNode,
	"weightTransferFunction2" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BooleanFilter" : {
	"inputFalse" : SFBool,
	"inputNegate" : SFBool,
	"inputTrue" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"set_boolean" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BooleanSequencer" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFBool,
	"metadata" : SFNode,
	"next" : SFBool,
	"previous" : SFBool,
	"set_fraction" : SFFloat,
	"value_changed" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BooleanToggle" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"set_boolean" : SFBool,
	"toggle" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BooleanTrigger" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"set_triggerTime" : SFTime,
	"triggerTrue" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BoundaryEnhancementVolumeStyle" : {
	"boundaryOpacity" : SFFloat,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"opacityFactor" : SFFloat,
	"retainedOpacity" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BoundedPhysicsModel" : {
	"enabled" : SFBool,
	"geometry" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
	"damping" : SFFloat,
},
"Box" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"size" : SFVec3f,
	"solid" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"BufferAudioSource" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"buffer" : MFFloat,
	"bufferDuration" : SFTime,
	"bufferlength" : SFInt32,
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"description" : SFString,
	"detune" : SFFloat,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"length" : SFInt32,
	"load" : SFBool,
	"loop" : SFBool,
	"loopEnd" : SFFloat,
	"loopStart" : SFFloat,
	"metadata" : SFNode,
	"numberOfChannels" : SFInt32,
	"pauseTime" : SFTime,
	"playbackRate" : SFFloat,
	"resumeTime" : SFTime,
	"sampleRate" : SFFloat,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CADAssembly" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CADFace" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"shape" : SFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CADLayer" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CADPart" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"removeChildren" : MFNode,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CartoonVolumeStyle" : {
	"colorSteps" : SFInt32,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"orthogonalColor" : SFColorRGBA,
	"parallelColor" : SFColorRGBA,
	"surfaceNormals" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ChannelMerger" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ChannelSelector" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"channelSelection" : SFInt32,
	"children" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ChannelSplitter" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"outputs" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Circle2D" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"radius" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ClipPlane" : {
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"plane" : SFVec4f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CollidableOffset" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"collidable" : SFNode,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"rotation" : SFRotation,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CollidableShape" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"rotation" : SFRotation,
	"shape" : SFNode,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Collision" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"collideTime" : SFTime,
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"proxy" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CollisionCollection" : {
	"appliedParameters" : MFString,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"bounce" : SFFloat,
	"collidables" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"frictionCoefficients" : SFVec2f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"minBounceSpeed" : SFFloat,
	"slipFactors" : SFVec2f,
	"softnessConstantForceMix" : SFFloat,
	"softnessErrorCorrection" : SFFloat,
	"surfaceSpeed" : SFVec2f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CollisionSensor" : {
	"collider" : SFNode,
	"contacts" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"intersections" : MFNode,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CollisionSpace" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"collidables" : MFNode,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"useGeometry" : SFBool,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Color" : {
	"color" : MFColor,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ColorChaser" : {
	"duration" : SFTime,
	"initialDestination" : SFColor,
	"initialValue" : SFColor,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : SFColor,
	"set_value" : SFColor,
	"value_changed" : SFColor,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ColorDamper" : {
	"initialDestination" : SFColor,
	"initialValue" : SFColor,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : SFColor,
	"set_value" : SFColor,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : SFColor,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ColorInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFColor,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : SFColor,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ColorRGBA" : {
	"color" : MFColorRGBA,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ComposedCubeMapTexture" : {
	"backTexture" : SFNode,
	"bottomTexture" : SFNode,
	"description" : SFString,
	"frontTexture" : SFNode,
	"IS" : SFNode,
	"leftTexture" : SFNode,
	"metadata" : SFNode,
	"rightTexture" : SFNode,
	"textureProperties" : SFNode,
	"topTexture" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ComposedShader" : {
	"activate" : SFBool,
	"field" : MFNode,
	"IS" : SFNode,
	"isSelected" : SFBool,
	"isValid" : SFBool,
	"language" : SFString,
	"metadata" : SFNode,
	"parts" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ComposedTexture3D" : {
	"description" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"repeatR" : SFBool,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"texture" : MFNode,
	"textureProperties" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ComposedVolumeStyle" : {
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"renderStyle" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Cone" : {
	"bottom" : SFBool,
	"bottomRadius" : SFFloat,
	"height" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"side" : SFBool,
	"solid" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ConeEmitter" : {
	"angle" : SFFloat,
	"direction" : SFVec3f,
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"on" : SFBool,
	"position" : SFVec3f,
	"speed" : SFFloat,
	"surfaceArea" : SFFloat,
	"variation" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Contact" : {
	"appliedParameters" : MFString,
	"body1" : SFNode,
	"body2" : SFNode,
	"bounce" : SFFloat,
	"contactNormal" : SFVec3f,
	"depth" : SFFloat,
	"frictionCoefficients" : SFVec2f,
	"frictionDirection" : SFVec3f,
	"geometry1" : SFNode,
	"geometry2" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"minBounceSpeed" : SFFloat,
	"position" : SFVec3f,
	"slipCoefficients" : SFVec2f,
	"softnessConstantForceMix" : SFFloat,
	"softnessErrorCorrection" : SFFloat,
	"surfaceSpeed" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Contour2D" : {
	"addChildren" : MFNode,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ContourPolyline2D" : {
	"controlPoint" : MFVec2d,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Convolver" : {
	"buffer" : MFFloat,
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"normalize" : SFBool,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Coordinate" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"point" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CoordinateChaser" : {
	"duration" : SFTime,
	"initialDestination" : MFVec3f,
	"initialValue" : MFVec3f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : MFVec3f,
	"set_value" : MFVec3f,
	"value_changed" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CoordinateDamper" : {
	"initialDestination" : MFVec3f,
	"initialValue" : MFVec3f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : MFVec3f,
	"set_value" : MFVec3f,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CoordinateDouble" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"point" : MFVec3d,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CoordinateInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec3f,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CoordinateInterpolator2D" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec2f,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : MFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Cylinder" : {
	"bottom" : SFBool,
	"height" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"radius" : SFFloat,
	"side" : SFBool,
	"solid" : SFBool,
	"top" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"CylinderSensor" : {
	"autoOffset" : SFBool,
	"axisRotation" : SFRotation,
	"description" : SFString,
	"diskAngle" : SFFloat,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isOver" : SFBool,
	"maxAngle" : SFFloat,
	"metadata" : SFNode,
	"minAngle" : SFFloat,
	"offset" : SFFloat,
	"rotation_changed" : SFRotation,
	"trackPoint_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Delay" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"delayTime" : SFTime,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"maxDelayTime" : SFTime,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"DirectionalLight" : {
	"ambientIntensity" : SFFloat,
	"color" : SFColor,
	"direction" : SFVec3f,
	"global" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"on" : SFBool,
	"shadowIntensity" : SFFloat,
	"shadows" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"DISEntityManager" : {
	"addedEntities" : MFNode,
	"address" : SFString,
	"applicationID" : SFInt32,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"port" : SFInt32,
	"removedEntities" : MFNode,
	"siteID" : SFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"DISEntityTypeMapping" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"category" : SFInt32,
	"country" : SFInt32,
	"description" : SFString,
	"domain" : SFInt32,
	"extra" : SFInt32,
	"IS" : SFNode,
	"kind" : SFInt32,
	"load" : SFBool,
	"metadata" : SFNode,
	"specific" : SFInt32,
	"subcategory" : SFInt32,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Disk2D" : {
	"innerRadius" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"outerRadius" : SFFloat,
	"solid" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"DoubleAxisHingeJoint" : {
	"anchorPoint" : SFVec3f,
	"axis1" : SFVec3f,
	"axis2" : SFVec3f,
	"body1" : SFNode,
	"body1AnchorPoint" : SFVec3f,
	"body1Axis" : SFVec3f,
	"body2" : SFNode,
	"body2AnchorPoint" : SFVec3f,
	"body2Axis" : SFVec3f,
	"desiredAngularVelocity1" : SFFloat,
	"desiredAngularVelocity2" : SFFloat,
	"forceOutput" : MFString,
	"hinge1Angle" : SFFloat,
	"hinge1AngleRate" : SFFloat,
	"hinge2Angle" : SFFloat,
	"hinge2AngleRate" : SFFloat,
	"IS" : SFNode,
	"maxAngle1" : SFFloat,
	"maxTorque1" : SFFloat,
	"maxTorque2" : SFFloat,
	"metadata" : SFNode,
	"minAngle1" : SFFloat,
	"stop1Bounce" : SFFloat,
	"stop1ConstantForceMix" : SFFloat,
	"stop1ErrorCorrection" : SFFloat,
	"suspensionErrorCorrection" : SFFloat,
	"suspensionForce" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"DynamicsCompressor" : {
	"attack" : SFTime,
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"knee" : SFFloat,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"ratio" : SFFloat,
	"reduction" : SFFloat,
	"release" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"threshold" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"EaseInEaseOut" : {
	"easeInEaseOut" : MFVec2f,
	"IS" : SFNode,
	"key" : MFFloat,
	"metadata" : SFNode,
	"modifiedFraction_changed" : SFFloat,
	"set_fraction" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"EdgeEnhancementVolumeStyle" : {
	"edgeColor" : SFColorRGBA,
	"enabled" : SFBool,
	"gradientThreshold" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"surfaceNormals" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ElevationGrid" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"creaseAngle" : SFFloat,
	"fogCoord" : SFNode,
	"height" : MFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"set_height" : MFFloat,
	"solid" : SFBool,
	"texCoord" : SFNode,
	"xDimension" : SFInt32,
	"xSpacing" : SFFloat,
	"zDimension" : SFInt32,
	"zSpacing" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"EnvironmentLight" : {
	"ambientIntensity" : SFFloat,
	"color" : SFColor,
	"diffuse" : SFNode,
	"diffuseCoefficients" : MFFloat,
	"diffuseTexture" : SFNode,
	"global" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"metadata" : SFNode,
	"on" : SFBool,
	"rotation" : SFRotation,
	"shadowIntensity" : SFFloat,
	"shadows" : SFBool,
	"specularTexture" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"EspduTransform" : {
	"addChildren" : MFNode,
	"address" : SFString,
	"applicationID" : SFInt32,
	"articulationParameterArray" : MFFloat,
	"articulationParameterChangeIndicatorArray" : MFInt32,
	"articulationParameterCount" : SFInt32,
	"articulationParameterDesignatorArray" : MFInt32,
	"articulationParameterIdPartAttachedToArray" : MFInt32,
	"articulationParameterTypeArray" : MFInt32,
	"articulationParameterValue0_changed" : SFFloat,
	"articulationParameterValue1_changed" : SFFloat,
	"articulationParameterValue2_changed" : SFFloat,
	"articulationParameterValue3_changed" : SFFloat,
	"articulationParameterValue4_changed" : SFFloat,
	"articulationParameterValue5_changed" : SFFloat,
	"articulationParameterValue6_changed" : SFFloat,
	"articulationParameterValue7_changed" : SFFloat,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"collideTime" : SFTime,
	"collisionType" : SFInt32,
	"deadReckoning" : SFInt32,
	"description" : SFString,
	"detonateTime" : SFTime,
	"detonationLocation" : SFVec3f,
	"detonationRelativeLocation" : SFVec3f,
	"detonationResult" : SFInt32,
	"enabled" : SFBool,
	"entityCategory" : SFInt32,
	"entityCountry" : SFInt32,
	"entityDomain" : SFInt32,
	"entityExtra" : SFInt32,
	"entityID" : SFInt32,
	"entityKind" : SFInt32,
	"entitySpecific" : SFInt32,
	"entitySubcategory" : SFInt32,
	"eventApplicationID" : SFInt32,
	"eventEntityID" : SFInt32,
	"eventNumber" : SFInt32,
	"eventSiteID" : SFInt32,
	"fired1" : SFBool,
	"fired2" : SFBool,
	"firedTime" : SFTime,
	"fireMissionIndex" : SFInt32,
	"firingRange" : SFFloat,
	"firingRate" : SFInt32,
	"forceID" : SFInt32,
	"fuse" : SFInt32,
	"geoCoords" : SFVec3d,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isCollided" : SFBool,
	"isDetonated" : SFBool,
	"isNetworkReader" : SFBool,
	"isNetworkWriter" : SFBool,
	"isRtpHeaderHeard" : SFBool,
	"isStandAlone" : SFBool,
	"linearAcceleration" : SFVec3f,
	"linearVelocity" : SFVec3f,
	"marking" : SFString,
	"metadata" : SFNode,
	"multicastRelayHost" : SFString,
	"multicastRelayPort" : SFInt32,
	"munitionApplicationID" : SFInt32,
	"munitionEndPoint" : SFVec3f,
	"munitionEntityID" : SFInt32,
	"munitionQuantity" : SFInt32,
	"munitionSiteID" : SFInt32,
	"munitionStartPoint" : SFVec3f,
	"networkMode" : SFString,
	"port" : SFInt32,
	"readInterval" : SFTime,
	"removeChildren" : MFNode,
	"rotation" : SFRotation,
	"rtpHeaderExpected" : SFBool,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"set_articulationParameterValue0" : SFFloat,
	"set_articulationParameterValue1" : SFFloat,
	"set_articulationParameterValue2" : SFFloat,
	"set_articulationParameterValue3" : SFFloat,
	"set_articulationParameterValue4" : SFFloat,
	"set_articulationParameterValue5" : SFFloat,
	"set_articulationParameterValue6" : SFFloat,
	"set_articulationParameterValue7" : SFFloat,
	"siteID" : SFInt32,
	"timestamp" : SFTime,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"warhead" : SFInt32,
	"writeInterval" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ExplosionEmitter" : {
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"on" : SFBool,
	"position" : SFVec3f,
	"speed" : SFFloat,
	"surfaceArea" : SFFloat,
	"variation" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Extrusion" : {
	"beginCap" : SFBool,
	"ccw" : SFBool,
	"convex" : SFBool,
	"creaseAngle" : SFFloat,
	"crossSection" : MFVec2f,
	"endCap" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"orientation" : MFRotation,
	"scale" : MFVec2f,
	"set_crossSection" : MFVec2f,
	"set_orientation" : MFRotation,
	"set_scale" : MFVec2f,
	"set_spine" : MFVec3f,
	"solid" : SFBool,
	"spine" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"FillProperties" : {
	"filled" : SFBool,
	"hatchColor" : SFColor,
	"hatched" : SFBool,
	"hatchStyle" : SFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"FloatVertexAttribute" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : xs:NMTOKEN,
	"numComponents" : SFInt32,
	"value" : MFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Fog" : {
	"bindTime" : SFTime,
	"color" : SFColor,
	"fogType" : SFString,
	"IS" : SFNode,
	"isBound" : SFBool,
	"metadata" : SFNode,
	"set_bind" : SFBool,
	"visibilityRange" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"FogCoordinate" : {
	"depth" : MFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"FontLibrary" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"family" : MFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"FontStyle" : {
	"class" : SFString,
	"family" : MFString,
	"horizontal" : SFBool,
	"id" : SFString,
	"IS" : SFNode,
	"justify" : MFString,
	"language" : SFString,
	"leftToRight" : SFBool,
	"metadata" : SFNode,
	"size" : SFFloat,
	"spacing" : SFFloat,
	"style" : SFString,
	"topToBottom" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
},
"ForcePhysicsModel" : {
	"enabled" : SFBool,
	"force" : SFVec3f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Gain" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeneratedCubeMapTexture" : {
	"description" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"size" : SFInt32,
	"textureProperties" : SFNode,
	"update" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoCoordinate" : {
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"point" : MFVec3d,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoElevationGrid" : {
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"creaseAngle" : SFDouble,
	"geoGridOrigin" : SFVec3d,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"height" : MFDouble,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"set_height" : MFDouble,
	"solid" : SFBool,
	"texCoord" : SFNode,
	"xDimension" : SFInt32,
	"xSpacing" : SFDouble,
	"yScale" : SFFloat,
	"zDimension" : SFInt32,
	"zSpacing" : SFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoLocation" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"geoCoords" : SFVec3d,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoLOD" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3d,
	"child1Url" : MFString,
	"child2Url" : MFString,
	"child3Url" : MFString,
	"child4Url" : MFString,
	"children" : MFNode,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"level_changed" : SFInt32,
	"metadata" : SFNode,
	"range" : SFFloat,
	"rootNode" : MFNode,
	"rootUrl" : MFString,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoMetadata" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"data" : MFNode,
	"description" : SFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"summary" : MFString,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoOrigin" : {
	"geoCoords" : SFVec3d,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"rotateYUp" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoPositionInterpolator" : {
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"geovalue_changed" : SFVec3d,
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec3d,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoProximitySensor" : {
	"center" : SFVec3d,
	"centerOfRotation_changed" : SFVec3f,
	"description" : SFString,
	"enabled" : SFBool,
	"enterTime" : SFTime,
	"exitTime" : SFTime,
	"geoCenter" : SFVec3d,
	"geoCoord_changed" : SFVec3d,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"orientation_changed" : SFRotation,
	"position_changed" : SFVec3f,
	"size" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoTouchSensor" : {
	"description" : SFString,
	"enabled" : SFBool,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"hitGeoCoord_changed" : SFVec3d,
	"hitNormal_changed" : SFVec3f,
	"hitPoint_changed" : SFVec3f,
	"hitTexCoord_changed" : SFVec2f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isOver" : SFBool,
	"metadata" : SFNode,
	"touchTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoTransform" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"geoCenter" : SFVec3d,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"GeoViewpoint" : {
	"bindTime" : SFTime,
	"centerOfRotation" : SFVec3d,
	"description" : SFString,
	"farDistance" : SFFloat,
	"fieldOfView" : SFFloat,
	"geoOrigin" : SFNode,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"isBound" : SFBool,
	"jump" : SFBool,
	"metadata" : SFNode,
	"navigationInfo" : SFNode,
	"nearDistance" : SFFloat,
	"orientation" : SFRotation,
	"position" : SFVec3d,
	"retainUserOffsets" : SFBool,
	"set_bind" : SFBool,
	"speedFactor" : SFFloat,
	"viewAll" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Group" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimDisplacer" : {
	"coordIndex" : MFInt32,
	"description" : SFString,
	"displacements" : MFVec3f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"weight" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimHumanoid" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"description" : SFString,
	"info" : MFString,
	"IS" : SFNode,
	"jointBindingPositions" : MFVec3f,
	"jointBindingRotations" : MFRotation,
	"jointBindingScales" : MFVec3f,
	"joints" : MFNode,
	"loa" : SFInt32,
	"metadata" : SFNode,
	"motions" : MFNode,
	"motionsEnabled" : MFBool,
	"name" : SFString,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"segments" : MFNode,
	"sites" : MFNode,
	"skeletalConfiguration" : SFString,
	"skeleton" : MFNode,
	"skin" : MFNode,
	"skinBindingCoords" : SFNode,
	"skinBindingNormals" : SFNode,
	"skinCoord" : SFNode,
	"skinNormal" : SFNode,
	"translation" : SFVec3f,
	"version" : SFString,
	"viewpoints" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimJoint" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"description" : SFString,
	"displacers" : MFNode,
	"IS" : SFNode,
	"limitOrientation" : SFRotation,
	"llimit" : MFFloat,
	"metadata" : SFNode,
	"name" : SFString,
	"removeChildren" : MFNode,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"skinCoordIndex" : MFInt32,
	"skinCoordWeight" : MFFloat,
	"stiffness" : MFFloat,
	"translation" : SFVec3f,
	"ulimit" : MFFloat,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimMotion" : {
	"channels" : SFString,
	"channelsEnabled" : MFBool,
	"cycleTime" : SFTime,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"endFrame" : SFInt32,
	"frameCount" : SFInt32,
	"frameDuration" : SFTime,
	"frameIncrement" : SFInt32,
	"frameIndex" : SFInt32,
	"IS" : SFNode,
	"joints" : SFString,
	"loa" : SFInt32,
	"loop" : SFBool,
	"metadata" : SFNode,
	"name" : SFString,
	"next" : SFBool,
	"previous" : SFBool,
	"startFrame" : SFInt32,
	"values" : MFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimPose" : {
	"commencePose" : SFBool,
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"loa" : SFInt32,
	"metadata" : SFNode,
	"name" : SFString,
	"resetAllJoints" : SFBool,
	"set_fraction" : SFTime,
	"set_startTime" : SFTime,
	"transitionDuration" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimSegment" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"centerOfMass" : SFVec3f,
	"children" : MFNode,
	"coord" : SFNode,
	"description" : SFString,
	"displacers" : MFNode,
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"momentsOfInertia" : MFFloat,
	"name" : SFString,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"HAnimSite" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"description" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"removeChildren" : MFNode,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ImageCubeMapTexture" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"textureProperties" : SFNode,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ImageTexture" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"textureProperties" : SFNode,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
	"colorSpaceConversion" : SFBool,
},
"ImageTexture3D" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"repeatR" : SFBool,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"textureProperties" : SFNode,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IndexedFaceSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorIndex" : MFInt32,
	"colorPerVertex" : SFBool,
	"convex" : SFBool,
	"coord" : SFNode,
	"coordIndex" : MFInt32,
	"creaseAngle" : SFFloat,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalIndex" : MFInt32,
	"normalPerVertex" : SFBool,
	"set_colorIndex" : MFInt32,
	"set_coordIndex" : MFInt32,
	"set_normalIndex" : MFInt32,
	"set_texCoordIndex" : MFInt32,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"texCoordIndex" : MFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IndexedLineSet" : {
	"attrib" : MFNode,
	"color" : SFNode,
	"colorIndex" : MFInt32,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"coordIndex" : MFInt32,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"set_colorIndex" : MFInt32,
	"set_coordIndex" : MFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IndexedQuadSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"index" : MFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"set_index" : MFInt32,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IndexedTriangleFanSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"index" : MFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"set_index" : MFInt32,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IndexedTriangleSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"index" : MFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"set_index" : MFInt32,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IndexedTriangleStripSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"index" : MFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"set_index" : MFInt32,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Inline" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"description" : SFString,
	"global" : SFBool,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"url" : MFString,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IntegerSequencer" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFInt32,
	"metadata" : SFNode,
	"next" : SFBool,
	"previous" : SFBool,
	"set_fraction" : SFFloat,
	"value_changed" : SFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IntegerTrigger" : {
	"integerKey" : SFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"set_boolean" : SFBool,
	"triggerValue" : SFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"IsoSurfaceVolumeData" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"contourStepSize" : SFFloat,
	"dimensions" : SFVec3f,
	"gradients" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"renderStyle" : MFNode,
	"surfaceTolerance" : SFFloat,
	"surfaceValues" : MFFloat,
	"visible" : SFBool,
	"voxels" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"KeySensor" : {
	"actionKeyPress" : SFInt32,
	"actionKeyRelease" : SFInt32,
	"altKey" : SFBool,
	"controlKey" : SFBool,
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"keyPress" : SFString,
	"keyRelease" : SFString,
	"metadata" : SFNode,
	"shiftKey" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Layer" : {
	"addChildren" : MFNode,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickable" : SFBool,
	"removeChildren" : MFNode,
	"viewport" : SFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LayerSet" : {
	"activeLayer" : SFInt32,
	"IS" : SFNode,
	"layers" : MFNode,
	"metadata" : SFNode,
	"order" : MFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Layout" : {
	"align" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"offset" : MFFloat,
	"offsetUnits" : MFString,
	"scaleMode" : MFString,
	"size" : MFFloat,
	"sizeUnits" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LayoutGroup" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"layout" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"viewport" : SFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LayoutLayer" : {
	"addChildren" : MFNode,
	"children" : MFNode,
	"IS" : SFNode,
	"layout" : SFNode,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickable" : SFBool,
	"removeChildren" : MFNode,
	"viewport" : SFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LinePickSensor" : {
	"description" : SFString,
	"enabled" : SFBool,
	"intersectionType" : SFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"matchCriterion" : SFString,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickedGeometry" : MFNode,
	"pickedNormal" : MFVec3f,
	"pickedPoint" : MFVec3f,
	"pickedTextureCoordinate" : MFVec3f,
	"pickingGeometry" : SFNode,
	"pickTarget" : MFNode,
	"sortOrder" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LineProperties" : {
	"applied" : SFBool,
	"IS" : SFNode,
	"linetype" : SFInt32,
	"linewidthScaleFactor" : SFFloat,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LineSet" : {
	"attrib" : MFNode,
	"color" : SFNode,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"vertexCount" : MFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ListenerPointSource" : {
	"description" : SFString,
	"dopplerEnabled" : SFBool,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"interauralDistance" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"orientation" : SFRotation,
	"pauseTime" : SFTime,
	"position" : SFVec3f,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"trackCurrentView" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LoadSensor" : {
	"children" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isLoaded" : SFBool,
	"loadTime" : SFTime,
	"metadata" : SFNode,
	"progress" : SFFloat,
	"timeOut" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LocalFog" : {
	"color" : SFColor,
	"enabled" : SFBool,
	"fogType" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"visibilityRange" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"LOD" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"forceTransitions" : SFBool,
	"IS" : SFNode,
	"level_changed" : SFInt32,
	"metadata" : SFNode,
	"range" : MFFloat,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Material" : {
	"ambientIntensity" : SFFloat,
	"ambientTexture" : SFNode,
	"ambientTextureMapping" : SFString,
	"diffuseColor" : SFColor,
	"diffuseTexture" : SFNode,
	"diffuseTextureMapping" : SFString,
	"emissiveColor" : SFColor,
	"emissiveTexture" : SFNode,
	"emissiveTextureMapping" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normalScale" : SFFloat,
	"normalTexture" : SFNode,
	"normalTextureMapping" : SFString,
	"occlusionStrength" : SFFloat,
	"occlusionTexture" : SFNode,
	"occlusionTextureMapping" : SFString,
	"shininess" : SFFloat,
	"shininessTexture" : SFNode,
	"shininessTextureMapping" : SFString,
	"specularColor" : SFColor,
	"specularTexture" : SFNode,
	"specularTextureMapping" : SFString,
	"transparency" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Matrix3VertexAttribute" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : xs:NMTOKEN,
	"value" : MFMatrix3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Matrix4VertexAttribute" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : xs:NMTOKEN,
	"value" : MFMatrix4f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MetadataBoolean" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"reference" : SFString,
	"value" : MFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MetadataDouble" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"reference" : SFString,
	"value" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MetadataFloat" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"reference" : SFString,
	"value" : MFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MetadataInteger" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"reference" : SFString,
	"value" : MFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MetadataSet" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"reference" : SFString,
	"value" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MetadataString" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"reference" : SFString,
	"value" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MicrophoneSource" : {
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"mediaDeviceID" : SFString,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MotorJoint" : {
	"autoCalc" : SFBool,
	"axis1Angle" : SFFloat,
	"axis1Torque" : SFFloat,
	"axis2Angle" : SFFloat,
	"axis2Torque" : SFFloat,
	"axis3Angle" : SFFloat,
	"axis3Torque" : SFFloat,
	"body1" : SFNode,
	"body2" : SFNode,
	"enabledAxes" : SFInt32,
	"forceOutput" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"motor1Angle" : SFFloat,
	"motor1AngleRate" : SFFloat,
	"motor1Axis" : SFVec3f,
	"motor2Angle" : SFFloat,
	"motor2AngleRate" : SFFloat,
	"motor2Axis" : SFVec3f,
	"motor3Angle" : SFFloat,
	"motor3AngleRate" : SFFloat,
	"motor3Axis" : SFVec3f,
	"stop1Bounce" : SFFloat,
	"stop1ErrorCorrection" : SFFloat,
	"stop2Bounce" : SFFloat,
	"stop2ErrorCorrection" : SFFloat,
	"stop3Bounce" : SFFloat,
	"stop3ErrorCorrection" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MovieTexture" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"duration_changed" : SFTime,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"load" : SFBool,
	"loop" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"pitch" : SFFloat,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"resumeTime" : SFTime,
	"speed" : SFFloat,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"textureProperties" : SFNode,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MultiTexture" : {
	"alpha" : SFFloat,
	"color" : SFColor,
	"description" : SFString,
	"function" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"mode" : MFString,
	"source" : MFString,
	"texture" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MultiTextureCoordinate" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"texCoord" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"MultiTextureTransform" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"textureTransform" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NavigationInfo" : {
	"avatarSize" : MFFloat,
	"bindTime" : SFTime,
	"headlight" : SFBool,
	"IS" : SFNode,
	"isBound" : SFBool,
	"metadata" : SFNode,
	"set_bind" : SFBool,
	"speed" : SFFloat,
	"transitionComplete" : SFBool,
	"transitionTime" : SFTime,
	"transitionType" : MFString,
	"type" : MFString,
	"visibilityLimit" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Normal" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"vector" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NormalInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec3f,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsCurve" : {
	"closed" : SFBool,
	"controlPoint" : SFNode,
	"IS" : SFNode,
	"knot" : MFDouble,
	"metadata" : SFNode,
	"order" : SFInt32,
	"tessellation" : SFInt32,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsCurve2D" : {
	"closed" : SFBool,
	"controlPoint" : MFVec2d,
	"IS" : SFNode,
	"knot" : MFDouble,
	"metadata" : SFNode,
	"order" : SFInt32,
	"tessellation" : SFInt32,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsOrientationInterpolator" : {
	"controlPoint" : SFNode,
	"IS" : SFNode,
	"knot" : MFDouble,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_fraction" : SFFloat,
	"value_changed" : SFRotation,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsPatchSurface" : {
	"controlPoint" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"solid" : SFBool,
	"texCoord" : SFNode,
	"uClosed" : SFBool,
	"uDimension" : SFInt32,
	"uKnot" : MFDouble,
	"uOrder" : SFInt32,
	"uTessellation" : SFInt32,
	"vClosed" : SFBool,
	"vDimension" : SFInt32,
	"vKnot" : MFDouble,
	"vOrder" : SFInt32,
	"vTessellation" : SFInt32,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsPositionInterpolator" : {
	"controlPoint" : SFNode,
	"IS" : SFNode,
	"knot" : MFDouble,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_fraction" : SFFloat,
	"value_changed" : SFVec3f,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsSet" : {
	"addGeometry" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"geometry" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeGeometry" : MFNode,
	"tessellationScale" : SFFloat,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsSurfaceInterpolator" : {
	"controlPoint" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal_changed" : SFVec3f,
	"position_changed" : SFVec3f,
	"set_fraction" : SFVec2f,
	"uDimension" : SFInt32,
	"uKnot" : MFDouble,
	"uOrder" : SFInt32,
	"vDimension" : SFInt32,
	"vKnot" : MFDouble,
	"vOrder" : SFInt32,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsSweptSurface" : {
	"ccw" : SFBool,
	"crossSectionCurve" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"solid" : SFBool,
	"trajectoryCurve" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsSwungSurface" : {
	"ccw" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"profileCurve" : SFNode,
	"solid" : SFBool,
	"trajectoryCurve" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsTextureCoordinate" : {
	"controlPoint" : MFVec2f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"uDimension" : SFInt32,
	"uKnot" : MFDouble,
	"uOrder" : SFInt32,
	"vDimension" : SFInt32,
	"vKnot" : MFDouble,
	"vOrder" : SFInt32,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"NurbsTrimmedSurface" : {
	"addTrimmingContour" : MFNode,
	"controlPoint" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeTrimmingContour" : MFNode,
	"solid" : SFBool,
	"texCoord" : SFNode,
	"trimmingContour" : MFNode,
	"uClosed" : SFBool,
	"uDimension" : SFInt32,
	"uKnot" : MFDouble,
	"uOrder" : SFInt32,
	"uTessellation" : SFInt32,
	"vClosed" : SFBool,
	"vDimension" : SFInt32,
	"vKnot" : MFDouble,
	"vOrder" : SFInt32,
	"vTessellation" : SFInt32,
	"weight" : MFDouble,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"OpacityMapVolumeStyle" : {
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"transferFunction" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"OrientationChaser" : {
	"duration" : SFTime,
	"initialDestination" : SFRotation,
	"initialValue" : SFRotation,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : SFRotation,
	"set_value" : SFRotation,
	"value_changed" : SFRotation,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"OrientationDamper" : {
	"initialDestination" : SFRotation,
	"initialValue" : SFRotation,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : SFRotation,
	"set_value" : SFRotation,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : SFRotation,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"OrientationInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFRotation,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : SFRotation,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"OrthoViewpoint" : {
	"bindTime" : SFTime,
	"centerOfRotation" : SFVec3f,
	"description" : SFString,
	"farDistance" : SFFloat,
	"fieldOfView" : MFFloat,
	"IS" : SFNode,
	"isBound" : SFBool,
	"jump" : SFBool,
	"metadata" : SFNode,
	"navigationInfo" : SFNode,
	"nearDistance" : SFFloat,
	"orientation" : SFRotation,
	"position" : SFVec3f,
	"retainUserOffsets" : SFBool,
	"set_bind" : SFBool,
	"viewAll" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"OscillatorSource" : {
	"description" : SFString,
	"detune" : SFFloat,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"frequency" : SFFloat,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"periodicWave" : SFNode,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PackagedShader" : {
	"activate" : SFBool,
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"field" : MFNode,
	"IS" : SFNode,
	"isSelected" : SFBool,
	"isValid" : SFBool,
	"language" : SFString,
	"load" : SFBool,
	"metadata" : SFNode,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ParticleSystem" : {
	"appearance" : SFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"castShadow" : SFBool,
	"color" : SFNode,
	"colorKey" : MFFloat,
	"createParticles" : SFBool,
	"emitter" : SFNode,
	"enabled" : SFBool,
	"geometry" : SFNode,
	"geometryType" : SFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"lifetimeVariation" : SFFloat,
	"maxParticles" : SFInt32,
	"metadata" : SFNode,
	"particleLifetime" : SFFloat,
	"particleSize" : SFVec2f,
	"physics" : MFNode,
	"texCoord" : SFNode,
	"texCoordKey" : MFFloat,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
	"pointerEvents" : SFBool,
},
"PeriodicWave" : {
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"optionsImag" : MFFloat,
	"optionsReal" : MFFloat,
	"type" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PhysicalMaterial" : {
	"baseColor" : SFColor,
	"baseTexture" : SFNode,
	"baseTextureMapping" : SFString,
	"emissiveColor" : SFColor,
	"emissiveTexture" : SFNode,
	"emissiveTextureMapping" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"metallic" : SFFloat,
	"metallicRoughnessTexture" : SFNode,
	"metallicRoughnessTextureMapping" : SFString,
	"normalScale" : SFFloat,
	"normalTexture" : SFNode,
	"normalTextureMapping" : SFString,
	"occlusionStrength" : SFFloat,
	"occlusionTexture" : SFNode,
	"occlusionTextureMapping" : SFString,
	"roughness" : SFFloat,
	"transparency" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
	"extensions" : MFNode,
},
"PickableGroup" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"description" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickable" : SFBool,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PixelTexture" : {
	"description" : SFString,
	"image" : SFImage,
	"IS" : SFNode,
	"metadata" : SFNode,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"textureProperties" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PixelTexture3D" : {
	"description" : SFString,
	"image" : MFInt32,
	"IS" : SFNode,
	"metadata" : SFNode,
	"repeatR" : SFBool,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"textureProperties" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PlaneSensor" : {
	"autoOffset" : SFBool,
	"axisRotation" : SFRotation,
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isOver" : SFBool,
	"maxPosition" : SFVec2f,
	"metadata" : SFNode,
	"minPosition" : SFVec2f,
	"offset" : SFVec3f,
	"trackPoint_changed" : SFVec3f,
	"translation_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PointEmitter" : {
	"direction" : SFVec3f,
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"on" : SFBool,
	"position" : SFVec3f,
	"speed" : SFFloat,
	"surfaceArea" : SFFloat,
	"variation" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PointLight" : {
	"ambientIntensity" : SFFloat,
	"attenuation" : SFVec3f,
	"color" : SFColor,
	"global" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"metadata" : SFNode,
	"on" : SFBool,
	"radius" : SFFloat,
	"shadowIntensity" : SFFloat,
	"shadows" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PointPickSensor" : {
	"description" : SFString,
	"enabled" : SFBool,
	"intersectionType" : SFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"matchCriterion" : SFString,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickedGeometry" : MFNode,
	"pickedPoint" : MFVec3f,
	"pickingGeometry" : SFNode,
	"pickTarget" : MFNode,
	"sortOrder" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PointProperties" : {
	"attenuation" : SFVec3f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"pointSizeMaxValue" : SFFloat,
	"pointSizeMinValue" : SFFloat,
	"pointSizeScaleFactor" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PointSet" : {
	"attrib" : MFNode,
	"color" : SFNode,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Polyline2D" : {
	"IS" : SFNode,
	"lineSegments" : MFVec2f,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PolylineEmitter" : {
	"coord" : SFNode,
	"coordIndex" : MFInt32,
	"direction" : SFVec3f,
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"on" : SFBool,
	"set_coordIndex" : MFInt32,
	"speed" : SFFloat,
	"surfaceArea" : SFFloat,
	"variation" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Polypoint2D" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"point" : MFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PositionChaser" : {
	"duration" : SFTime,
	"initialDestination" : SFVec3f,
	"initialValue" : SFVec3f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : SFVec3f,
	"set_value" : SFVec3f,
	"value_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PositionChaser2D" : {
	"duration" : SFTime,
	"initialDestination" : SFVec2f,
	"initialValue" : SFVec2f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : SFVec2f,
	"set_value" : SFVec2f,
	"value_changed" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PositionDamper" : {
	"initialDestination" : SFVec3f,
	"initialValue" : SFVec3f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : SFVec3f,
	"set_value" : SFVec3f,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PositionDamper2D" : {
	"initialDestination" : SFVec2f,
	"initialValue" : SFVec2f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : SFVec2f,
	"set_value" : SFVec2f,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PositionInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec3f,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PositionInterpolator2D" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec2f,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"PrimitivePickSensor" : {
	"description" : SFString,
	"enabled" : SFBool,
	"intersectionType" : SFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"matchCriterion" : SFString,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickedGeometry" : MFNode,
	"pickingGeometry" : SFNode,
	"pickTarget" : MFNode,
	"sortOrder" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ProgramShader" : {
	"activate" : SFBool,
	"IS" : SFNode,
	"isSelected" : SFBool,
	"isValid" : SFBool,
	"language" : SFString,
	"metadata" : SFNode,
	"programs" : MFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ProjectionVolumeStyle" : {
	"enabled" : SFBool,
	"intensityThreshold" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"type" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ProtoInstance" : {
	"fieldValue" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"name" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ProximitySensor" : {
	"center" : SFVec3f,
	"centerOfRotation_changed" : SFVec3f,
	"description" : SFString,
	"enabled" : SFBool,
	"enterTime" : SFTime,
	"exitTime" : SFTime,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"orientation_changed" : SFRotation,
	"position_changed" : SFVec3f,
	"size" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"QuadSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ReceiverPdu" : {
	"address" : SFString,
	"applicationID" : SFInt32,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"description" : SFString,
	"enabled" : SFBool,
	"entityID" : SFInt32,
	"geoCoords" : SFVec3d,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isNetworkReader" : SFBool,
	"isNetworkWriter" : SFBool,
	"isRtpHeaderHeard" : SFBool,
	"isStandAlone" : SFBool,
	"metadata" : SFNode,
	"multicastRelayHost" : SFString,
	"multicastRelayPort" : SFInt32,
	"networkMode" : SFString,
	"port" : SFInt32,
	"radioID" : SFInt32,
	"readInterval" : SFTime,
	"receivedPower" : SFFloat,
	"receiverState" : SFInt32,
	"rtpHeaderExpected" : SFBool,
	"siteID" : SFInt32,
	"timestamp" : SFTime,
	"transmitterApplicationID" : SFInt32,
	"transmitterEntityID" : SFInt32,
	"transmitterRadioID" : SFInt32,
	"transmitterSiteID" : SFInt32,
	"visible" : SFBool,
	"whichGeometry" : SFInt32,
	"writeInterval" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Rectangle2D" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"size" : SFVec2f,
	"solid" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"RigidBody" : {
	"angularDampingFactor" : SFFloat,
	"angularVelocity" : SFVec3f,
	"autoDamp" : SFBool,
	"autoDisable" : SFBool,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"centerOfMass" : SFVec3f,
	"disableAngularSpeed" : SFFloat,
	"disableLinearSpeed" : SFFloat,
	"disableTime" : SFTime,
	"enabled" : SFBool,
	"finiteRotationAxis" : SFVec3f,
	"fixed" : SFBool,
	"forces" : MFVec3f,
	"geometry" : MFNode,
	"inertia" : SFMatrix3f,
	"IS" : SFNode,
	"linearDampingFactor" : SFFloat,
	"linearVelocity" : SFVec3f,
	"mass" : SFFloat,
	"massDensityModel" : SFNode,
	"metadata" : SFNode,
	"orientation" : SFRotation,
	"position" : SFVec3f,
	"torques" : MFVec3f,
	"useFiniteRotation" : SFBool,
	"useGlobalGravity" : SFBool,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"RigidBodyCollection" : {
	"autoDisable" : SFBool,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"bodies" : MFNode,
	"collider" : SFNode,
	"constantForceMix" : SFFloat,
	"contactSurfaceThickness" : SFFloat,
	"disableAngularSpeed" : SFFloat,
	"disableLinearSpeed" : SFFloat,
	"disableTime" : SFTime,
	"enabled" : SFBool,
	"errorCorrection" : SFFloat,
	"gravity" : SFVec3f,
	"IS" : SFNode,
	"iterations" : SFInt32,
	"joints" : MFNode,
	"maxCorrectionSpeed" : SFFloat,
	"metadata" : SFNode,
	"preferAccuracy" : SFBool,
	"set_contacts" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ScalarChaser" : {
	"duration" : SFTime,
	"initialDestination" : SFFloat,
	"initialValue" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : SFFloat,
	"set_value" : SFFloat,
	"value_changed" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ScalarDamper" : {
	"initialDestination" : SFFloat,
	"initialValue" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : SFFloat,
	"set_value" : SFFloat,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ScalarInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFFloat,
	"metadata" : SFNode,
	"set_fraction" : SFFloat,
	"value_changed" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ScreenFontStyle" : {
	"class" : SFString,
	"family" : MFString,
	"horizontal" : SFBool,
	"id" : SFString,
	"IS" : SFNode,
	"justify" : MFString,
	"language" : SFString,
	"leftToRight" : SFBool,
	"metadata" : SFNode,
	"pointSize" : SFFloat,
	"spacing" : SFFloat,
	"style" : SFString,
	"topToBottom" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
},
"ScreenGroup" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Script" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"directOutput" : SFBool,
	"field" : MFNode,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"mustEvaluate" : SFBool,
	"sourceCode" : SFString,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SegmentedVolumeData" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"dimensions" : SFVec3f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"renderStyle" : MFNode,
	"segmentEnabled" : MFBool,
	"segmentIdentifiers" : SFNode,
	"visible" : SFBool,
	"voxels" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ShadedVolumeStyle" : {
	"enabled" : SFBool,
	"IS" : SFNode,
	"lighting" : SFBool,
	"material" : SFNode,
	"metadata" : SFNode,
	"phaseFunction" : SFString,
	"shadows" : SFBool,
	"surfaceNormals" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ShaderPart" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"sourceCode" : SFString,
	"type" : SFString,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ShaderProgram" : {
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"description" : SFString,
	"field" : MFNode,
	"IS" : SFNode,
	"load" : SFBool,
	"metadata" : SFNode,
	"sourceCode" : SFString,
	"type" : SFString,
	"url" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Shape" : {
	"appearance" : SFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"castShadow" : SFBool,
	"geometry" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
	"pointerEvents" : SFBool,
},
"SignalPdu" : {
	"address" : SFString,
	"applicationID" : SFInt32,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"data" : MFInt32,
	"dataLength" : SFInt32,
	"description" : SFString,
	"enabled" : SFBool,
	"encodingScheme" : SFInt32,
	"entityID" : SFInt32,
	"geoCoords" : SFVec3d,
	"geoSystem" : MFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isNetworkReader" : SFBool,
	"isNetworkWriter" : SFBool,
	"isRtpHeaderHeard" : SFBool,
	"isStandAlone" : SFBool,
	"metadata" : SFNode,
	"multicastRelayHost" : SFString,
	"multicastRelayPort" : SFInt32,
	"networkMode" : SFString,
	"port" : SFInt32,
	"radioID" : SFInt32,
	"readInterval" : SFTime,
	"rtpHeaderExpected" : SFBool,
	"sampleRate" : SFInt32,
	"samples" : SFInt32,
	"siteID" : SFInt32,
	"tdlType" : SFInt32,
	"timestamp" : SFTime,
	"visible" : SFBool,
	"whichGeometry" : SFInt32,
	"writeInterval" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SilhouetteEnhancementVolumeStyle" : {
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"silhouetteBoundaryOpacity" : SFFloat,
	"silhouetteRetainedOpacity" : SFFloat,
	"silhouetteSharpness" : SFFloat,
	"surfaceNormals" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SingleAxisHingeJoint" : {
	"anchorPoint" : SFVec3f,
	"angle" : SFFloat,
	"angleRate" : SFFloat,
	"axis" : SFVec3f,
	"body1" : SFNode,
	"body1AnchorPoint" : SFVec3f,
	"body2" : SFNode,
	"body2AnchorPoint" : SFVec3f,
	"forceOutput" : MFString,
	"IS" : SFNode,
	"maxAngle" : SFFloat,
	"metadata" : SFNode,
	"minAngle" : SFFloat,
	"stopBounce" : SFFloat,
	"stopErrorCorrection" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SliderJoint" : {
	"axis" : SFVec3f,
	"body1" : SFNode,
	"body2" : SFNode,
	"forceOutput" : MFString,
	"IS" : SFNode,
	"maxSeparation" : SFFloat,
	"metadata" : SFNode,
	"minSeparation" : SFFloat,
	"separation" : SFFloat,
	"separationRate" : SFFloat,
	"sliderForce" : SFFloat,
	"stopBounce" : SFFloat,
	"stopErrorCorrection" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Sound" : {
	"description" : SFString,
	"direction" : SFVec3f,
	"enabled" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"maxBack" : SFFloat,
	"maxFront" : SFFloat,
	"metadata" : SFNode,
	"minBack" : SFFloat,
	"minFront" : SFFloat,
	"priority" : SFFloat,
	"source" : SFNode,
	"spatialize" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SpatialSound" : {
	"children" : MFNode,
	"coneInnerAngle" : SFFloat,
	"coneOuterAngle" : SFFloat,
	"coneOuterGain" : SFFloat,
	"description" : SFString,
	"direction" : SFVec3f,
	"distanceModel" : SFString,
	"dopplerEnabled" : SFBool,
	"enabled" : SFBool,
	"enableHRTF" : SFBool,
	"gain" : SFFloat,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"maxDistance" : SFFloat,
	"metadata" : SFNode,
	"priority" : SFFloat,
	"referenceDistance" : SFFloat,
	"rolloffFactor" : SFFloat,
	"spatialize" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Sphere" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"radius" : SFFloat,
	"solid" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SphereSensor" : {
	"autoOffset" : SFBool,
	"description" : SFString,
	"enabled" : SFBool,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isOver" : SFBool,
	"metadata" : SFNode,
	"offset" : SFRotation,
	"rotation_changed" : SFRotation,
	"trackPoint_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SplinePositionInterpolator" : {
	"closed" : SFBool,
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec3f,
	"keyVelocity" : MFVec3f,
	"metadata" : SFNode,
	"normalizeVelocity" : SFBool,
	"set_fraction" : SFFloat,
	"value_changed" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SplinePositionInterpolator2D" : {
	"closed" : SFBool,
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFVec2f,
	"keyVelocity" : MFVec2f,
	"metadata" : SFNode,
	"normalizeVelocity" : SFBool,
	"set_fraction" : SFFloat,
	"value_changed" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SplineScalarInterpolator" : {
	"closed" : SFBool,
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFFloat,
	"keyVelocity" : MFFloat,
	"metadata" : SFNode,
	"normalizeVelocity" : SFBool,
	"set_fraction" : SFFloat,
	"value_changed" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SpotLight" : {
	"ambientIntensity" : SFFloat,
	"attenuation" : SFVec3f,
	"beamWidth" : SFFloat,
	"color" : SFColor,
	"cutOffAngle" : SFFloat,
	"direction" : SFVec3f,
	"global" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"metadata" : SFNode,
	"on" : SFBool,
	"radius" : SFFloat,
	"shadowIntensity" : SFFloat,
	"shadows" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SquadOrientationInterpolator" : {
	"IS" : SFNode,
	"key" : MFFloat,
	"keyValue" : MFRotation,
	"metadata" : SFNode,
	"normalizeVelocity" : SFBool,
	"set_fraction" : SFFloat,
	"value_changed" : SFRotation,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"StaticGroup" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"StreamAudioDestination" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"description" : SFString,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"mediaDeviceID" : SFString,
	"metadata" : SFNode,
	"streamIdentifier" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"StreamAudioSource" : {
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"streamIdentifier" : MFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"StringSensor" : {
	"deletionAllowed" : SFBool,
	"description" : SFString,
	"enabled" : SFBool,
	"enteredText" : SFString,
	"finalText" : SFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"SurfaceEmitter" : {
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"on" : SFBool,
	"speed" : SFFloat,
	"surface" : SFNode,
	"surfaceArea" : SFFloat,
	"variation" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Switch" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"whichChoice" : SFInt32,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Tangent" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"vector" : MFVec4f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TexCoordChaser2D" : {
	"duration" : SFTime,
	"initialDestination" : MFVec2f,
	"initialValue" : MFVec2f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"set_destination" : MFVec2f,
	"set_value" : MFVec2f,
	"value_changed" : MFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TexCoordDamper2D" : {
	"initialDestination" : MFVec2f,
	"initialValue" : MFVec2f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"order" : SFInt32,
	"set_destination" : MFVec2f,
	"set_value" : MFVec2f,
	"tau" : SFTime,
	"tolerance" : SFFloat,
	"value_changed" : MFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Text" : {
	"fontStyle" : SFNode,
	"IS" : SFNode,
	"length" : MFFloat,
	"lineBounds" : MFVec2f,
	"maxExtent" : SFFloat,
	"metadata" : SFNode,
	"origin" : SFVec3f,
	"solid" : SFBool,
	"string" : MFString,
	"textBounds" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureBackground" : {
	"backTexture" : SFNode,
	"bindTime" : SFTime,
	"bottomTexture" : SFNode,
	"frontTexture" : SFNode,
	"groundAngle" : MFFloat,
	"groundColor" : MFColor,
	"IS" : SFNode,
	"isBound" : SFBool,
	"leftTexture" : SFNode,
	"metadata" : SFNode,
	"rightTexture" : SFNode,
	"set_bind" : SFBool,
	"skyAngle" : MFFloat,
	"skyColor" : MFColor,
	"topTexture" : SFNode,
	"transparency" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureCoordinate" : {
	"IS" : SFNode,
	"mapping" : SFString,
	"metadata" : SFNode,
	"point" : MFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureCoordinate3D" : {
	"IS" : SFNode,
	"mapping" : SFString,
	"metadata" : SFNode,
	"point" : MFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureCoordinate4D" : {
	"IS" : SFNode,
	"mapping" : SFString,
	"metadata" : SFNode,
	"point" : MFVec4f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureCoordinateGenerator" : {
	"IS" : SFNode,
	"mapping" : SFString,
	"metadata" : SFNode,
	"mode" : SFString,
	"parameter" : MFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureProjector" : {
	"ambientIntensity" : SFFloat,
	"aspectRatio" : SFFloat,
	"color" : SFColor,
	"description" : SFString,
	"direction" : SFVec3f,
	"farDistance" : SFFloat,
	"fieldOfView" : SFFloat,
	"global" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"metadata" : SFNode,
	"nearDistance" : SFFloat,
	"on" : SFBool,
	"shadowIntensity" : SFFloat,
	"shadows" : SFBool,
	"texture" : SFNode,
	"upVector" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureProjectorParallel" : {
	"ambientIntensity" : SFFloat,
	"aspectRatio" : SFFloat,
	"color" : SFColor,
	"description" : SFString,
	"direction" : SFVec3f,
	"farDistance" : SFFloat,
	"fieldOfView" : SFVec4f,
	"global" : SFBool,
	"intensity" : SFFloat,
	"IS" : SFNode,
	"location" : SFVec3f,
	"metadata" : SFNode,
	"nearDistance" : SFFloat,
	"on" : SFBool,
	"shadowIntensity" : SFFloat,
	"shadows" : SFBool,
	"texture" : SFNode,
	"upVector" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureProperties" : {
	"anisotropicDegree" : SFFloat,
	"borderColor" : SFColorRGBA,
	"borderWidth" : SFInt32,
	"boundaryModeR" : SFString,
	"boundaryModeS" : SFString,
	"boundaryModeT" : SFString,
	"generateMipMaps" : SFBool,
	"IS" : SFNode,
	"magnificationFilter" : SFString,
	"metadata" : SFNode,
	"minificationFilter" : SFString,
	"textureCompression" : SFString,
	"texturePriority" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureTransform" : {
	"center" : SFVec2f,
	"IS" : SFNode,
	"mapping" : SFString,
	"metadata" : SFNode,
	"rotation" : SFFloat,
	"scale" : SFVec2f,
	"translation" : SFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureTransform3D" : {
	"center" : SFVec3f,
	"IS" : SFNode,
	"mapping" : SFString,
	"metadata" : SFNode,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"translation" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TextureTransformMatrix3D" : {
	"IS" : SFNode,
	"mapping" : SFString,
	"matrix" : SFMatrix4f,
	"metadata" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TimeSensor" : {
	"cycleInterval" : SFTime,
	"cycleTime" : SFTime,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"fraction_changed" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"loop" : SFBool,
	"metadata" : SFNode,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"time" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TimeTrigger" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"set_boolean" : SFBool,
	"triggerTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ToneMappedVolumeStyle" : {
	"coolColor" : SFColorRGBA,
	"enabled" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"surfaceNormals" : SFNode,
	"warmColor" : SFColorRGBA,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TouchSensor" : {
	"description" : SFString,
	"enabled" : SFBool,
	"hitNormal_changed" : SFVec3f,
	"hitPoint_changed" : SFVec3f,
	"hitTexCoord_changed" : SFVec2f,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isOver" : SFBool,
	"metadata" : SFNode,
	"touchTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Transform" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"center" : SFVec3f,
	"children" : MFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"rotation" : SFRotation,
	"scale" : SFVec3f,
	"scaleOrientation" : SFRotation,
	"translation" : SFVec3f,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TransformSensor" : {
	"center" : SFVec3f,
	"description" : SFString,
	"enabled" : SFBool,
	"enterTime" : SFTime,
	"exitTime" : SFTime,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"orientation_changed" : SFRotation,
	"position_changed" : SFVec3f,
	"size" : SFVec3f,
	"targetObject" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TransmitterPdu" : {
	"address" : SFString,
	"antennaLocation" : SFVec3f,
	"antennaPatternLength" : SFInt32,
	"antennaPatternType" : SFInt32,
	"applicationID" : SFInt32,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"cryptoKeyID" : SFInt32,
	"cryptoSystem" : SFInt32,
	"description" : SFString,
	"enabled" : SFBool,
	"entityID" : SFInt32,
	"frequency" : SFInt32,
	"geoCoords" : SFVec3d,
	"geoSystem" : MFString,
	"inputSource" : SFInt32,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isNetworkReader" : SFBool,
	"isNetworkWriter" : SFBool,
	"isRtpHeaderHeard" : SFBool,
	"isStandAlone" : SFBool,
	"lengthOfModulationParameters" : SFInt32,
	"metadata" : SFNode,
	"modulationTypeDetail" : SFInt32,
	"modulationTypeMajor" : SFInt32,
	"modulationTypeSpreadSpectrum" : SFInt32,
	"modulationTypeSystem" : SFInt32,
	"multicastRelayHost" : SFString,
	"multicastRelayPort" : SFInt32,
	"networkMode" : SFString,
	"port" : SFInt32,
	"power" : SFFloat,
	"radioEntityTypeCategory" : SFInt32,
	"radioEntityTypeCountry" : SFInt32,
	"radioEntityTypeDomain" : SFInt32,
	"radioEntityTypeKind" : SFInt32,
	"radioEntityTypeNomenclature" : SFInt32,
	"radioEntityTypeNomenclatureVersion" : SFInt32,
	"radioID" : SFInt32,
	"readInterval" : SFTime,
	"relativeAntennaLocation" : SFVec3f,
	"rtpHeaderExpected" : SFBool,
	"siteID" : SFInt32,
	"timestamp" : SFTime,
	"transmitFrequencyBandwidth" : SFFloat,
	"transmitState" : SFInt32,
	"visible" : SFBool,
	"whichGeometry" : SFInt32,
	"writeInterval" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TriangleFanSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fanCount" : MFInt32,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TriangleSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"solid" : SFBool,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TriangleSet2D" : {
	"IS" : SFNode,
	"metadata" : SFNode,
	"solid" : SFBool,
	"vertices" : MFVec2f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TriangleStripSet" : {
	"attrib" : MFNode,
	"ccw" : SFBool,
	"color" : SFNode,
	"colorPerVertex" : SFBool,
	"coord" : SFNode,
	"fogCoord" : SFNode,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normal" : SFNode,
	"normalPerVertex" : SFBool,
	"solid" : SFBool,
	"stripCount" : MFInt32,
	"tangent" : SFNode,
	"texCoord" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"TwoSidedMaterial" : {
	"ambientIntensity" : SFFloat,
	"backAmbientIntensity" : SFFloat,
	"backDiffuseColor" : SFColor,
	"backEmissiveColor" : SFColor,
	"backShininess" : SFFloat,
	"backSpecularColor" : SFColor,
	"backTransparency" : SFFloat,
	"diffuseColor" : SFColor,
	"emissiveColor" : SFColor,
	"IS" : SFNode,
	"metadata" : SFNode,
	"separateBackColor" : SFBool,
	"shininess" : SFFloat,
	"specularColor" : SFColor,
	"transparency" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"UniversalJoint" : {
	"anchorPoint" : SFVec3f,
	"axis1" : SFVec3f,
	"axis2" : SFVec3f,
	"body1" : SFNode,
	"body1AnchorPoint" : SFVec3f,
	"body1Axis" : SFVec3f,
	"body2" : SFNode,
	"body2AnchorPoint" : SFVec3f,
	"body2Axis" : SFVec3f,
	"forceOutput" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"stop1Bounce" : SFFloat,
	"stop1ErrorCorrection" : SFFloat,
	"stop2Bounce" : SFFloat,
	"stop2ErrorCorrection" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"UnlitMaterial" : {
	"emissiveColor" : SFColor,
	"emissiveTexture" : SFNode,
	"emissiveTextureMapping" : SFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"normalScale" : SFFloat,
	"normalTexture" : SFNode,
	"normalTextureMapping" : SFString,
	"transparency" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Viewpoint" : {
	"bindTime" : SFTime,
	"centerOfRotation" : SFVec3f,
	"description" : SFString,
	"farDistance" : SFFloat,
	"fieldOfView" : SFFloat,
	"IS" : SFNode,
	"isBound" : SFBool,
	"jump" : SFBool,
	"metadata" : SFNode,
	"navigationInfo" : SFNode,
	"nearDistance" : SFFloat,
	"orientation" : SFRotation,
	"position" : SFVec3f,
	"retainUserOffsets" : SFBool,
	"set_bind" : SFBool,
	"viewAll" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ViewpointGroup" : {
	"center" : SFVec3f,
	"children" : MFNode,
	"description" : SFString,
	"displayed" : SFBool,
	"IS" : SFNode,
	"metadata" : SFNode,
	"retainUserOffsets" : SFBool,
	"size" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"Viewport" : {
	"addChildren" : MFNode,
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"children" : MFNode,
	"clipBoundary" : MFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"removeChildren" : MFNode,
	"visible" : SFBool,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"VisibilitySensor" : {
	"center" : SFVec3f,
	"description" : SFString,
	"enabled" : SFBool,
	"enterTime" : SFTime,
	"exitTime" : SFTime,
	"IS" : SFNode,
	"isActive" : SFBool,
	"metadata" : SFNode,
	"size" : SFVec3f,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"VolumeData" : {
	"bboxCenter" : SFVec3f,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"dimensions" : SFVec3f,
	"IS" : SFNode,
	"metadata" : SFNode,
	"renderStyle" : SFNode,
	"visible" : SFBool,
	"voxels" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"VolumeEmitter" : {
	"coord" : SFNode,
	"coordIndex" : MFInt32,
	"direction" : SFVec3f,
	"internal" : SFBool,
	"IS" : SFNode,
	"mass" : SFFloat,
	"metadata" : SFNode,
	"on" : SFBool,
	"set_coordIndex" : MFInt32,
	"speed" : SFFloat,
	"surfaceArea" : SFFloat,
	"variation" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"VolumePickSensor" : {
	"description" : SFString,
	"enabled" : SFBool,
	"intersectionType" : SFString,
	"IS" : SFNode,
	"isActive" : SFBool,
	"matchCriterion" : SFString,
	"metadata" : SFNode,
	"objectType" : MFString,
	"pickedGeometry" : MFNode,
	"pickingGeometry" : SFNode,
	"pickTarget" : MFNode,
	"sortOrder" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"WaveShaper" : {
	"channelCount" : SFInt32,
	"channelCountMode" : SFString,
	"channelInterpretation" : SFString,
	"children" : MFNode,
	"curve" : MFFloat,
	"description" : SFString,
	"elapsedTime" : SFTime,
	"enabled" : SFBool,
	"gain" : SFFloat,
	"IS" : SFNode,
	"isActive" : SFBool,
	"isPaused" : SFBool,
	"metadata" : SFNode,
	"oversample" : SFString,
	"pauseTime" : SFTime,
	"resumeTime" : SFTime,
	"startTime" : SFTime,
	"stopTime" : SFTime,
	"tailTime" : SFTime,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"WindPhysicsModel" : {
	"direction" : SFVec3f,
	"enabled" : SFBool,
	"gustiness" : SFFloat,
	"IS" : SFNode,
	"metadata" : SFNode,
	"speed" : SFFloat,
	"turbulence" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"WorldInfo" : {
	"info" : MFString,
	"IS" : SFNode,
	"metadata" : SFNode,
	"title" : SFString,
	"DEF" : SFString,
	"USE" : SFString,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ImageTextureAtlas" : {
	"metadata" : SFNode,
	"description" : SFString,
	"load" : SFBool,
	"url" : MFString,
	"autoRefresh" : SFTime,
	"autoRefreshTimeLimit" : SFTime,
	"slicesOverX" : SFInt32,
	"slicesOverY" : SFInt32,
	"numberOfSlices" : SFInt32,
	"repeatS" : SFBool,
	"repeatT" : SFBool,
	"repeatR" : SFBool,
	"textureProperties" : SFNode,
},
"AnisotropyMaterialExtension" : {
	"metadata" : SFNode,
	"anisotropyStrength" : SFFloat,
	"anisotropyRotation" : SFFloat,
	"anisotropyTextureMapping" : SFString,
	"anisotropyTexture" : SFNode,
},
"BlendMode" : {
	"metadata" : SFNode,
	"blendColor" : SFColorRGBA,
	"sourceColorFactor" : SFString,
	"sourceAlphaFactor" : SFString,
	"destinationColorFactor" : SFString,
	"destinationAlphaFactor" : SFString,
	"colorEquation" : SFString,
	"alphaEquation" : SFString,
},
"ClearcoatMaterialExtension" : {
	"metadata" : SFNode,
	"clearcoat" : SFFloat,
	"clearcoatTextureMapping" : SFString,
	"clearcoatTexture" : SFNode,
	"clearcoatRoughness" : SFFloat,
	"clearcoatRoughnessTextureMapping" : SFString,
	"clearcoatRoughnessTexture" : SFNode,
	"clearcoatNormalTextureMapping" : SFString,
	"clearcoatNormalTexture" : SFNode,
},
"DepthMode" : {
	"metadata" : SFNode,
	"polygonOffset" : SFVec2f,
	"depthRange" : SFVec2f,
	"depthTest" : SFBool,
	"depthFunction" : SFString,
	"depthMask" : SFBool,
},
"DispersionMaterialExtension" : {
	"metadata" : SFNode,
	"dispersion" : SFFloat,
},
"EmissiveStrengthMaterialExtension" : {
	"metadata" : SFNode,
	"emissiveStrength" : SFFloat,
},
"IORMaterialExtension" : {
	"metadata" : SFNode,
	"indexOfRefraction" : SFFloat,
	"DEF" : SFString,
	"USE" : SFString,
},
"InstancedShape" : {
	"metadata" : SFNode,
	"translations" : MFVec3f,
	"rotations" : MFRotation,
	"scales" : MFVec3f,
	"scaleOrientations" : MFRotation,
	"centers" : MFVec3f,
	"pointerEvents" : SFBool,
	"castShadow" : SFBool,
	"visible" : SFBool,
	"bboxDisplay" : SFBool,
	"bboxSize" : SFVec3f,
	"bboxCenter" : SFVec3f,
	"appearance" : SFNode,
	"geometry" : SFNode,
},
"IridescenceMaterialExtension" : {
	"metadata" : SFNode,
	"iridescence" : SFFloat,
	"iridescenceTextureMapping" : SFString,
	"iridescenceTexture" : SFNode,
	"iridescenceIndexOfRefraction" : SFFloat,
	"iridescenceThicknessMinimum" : SFFloat,
	"iridescenceThicknessMaximum" : SFFloat,
	"iridescenceThicknessTextureMapping" : SFString,
	"iridescenceThicknessTexture" : SFNode,
	"DEF" : SFString,
	"USE" : SFString,
},
"SheenMaterialExtension" : {
	"metadata" : SFNode,
	"sheenColor" : SFColor,
	"sheenColorTextureMapping" : SFString,
	"sheenColorTexture" : SFNode,
	"sheenRoughness" : SFFloat,
	"sheenRoughnessTextureMapping" : SFString,
	"sheenRoughnessTexture" : SFNode,
},
"SpecularGlossinessMaterial" : {
	"metadata" : SFNode,
	"diffuseColor" : SFColor,
	"diffuseTextureMapping" : SFString,
	"diffuseTexture" : SFNode,
	"specularColor" : SFColor,
	"glossiness" : SFFloat,
	"specularGlossinessTextureMapping" : SFString,
	"specularGlossinessTexture" : SFNode,
	"emissiveColor" : SFColor,
	"emissiveTextureMapping" : SFString,
	"emissiveTexture" : SFNode,
	"occlusionStrength" : SFFloat,
	"occlusionTextureMapping" : SFString,
	"occlusionTexture" : SFNode,
	"normalScale" : SFFloat,
	"normalTextureMapping" : SFString,
	"normalTexture" : SFNode,
	"transparency" : SFFloat,
	"extensions" : MFNode,
},
"SpecularMaterialExtension" : {
	"metadata" : SFNode,
	"specular" : SFFloat,
	"specularTextureMapping" : SFString,
	"specularTexture" : SFNode,
	"specularColor" : SFColor,
	"specularColorTextureMapping" : SFString,
	"specularColorTexture" : SFNode,
},
"TransmissionMaterialExtension" : {
	"metadata" : SFNode,
	"transmission" : SFFloat,
	"transmissionTextureMapping" : SFString,
	"transmissionTexture" : SFNode,
},
"VolumeMaterialExtension" : {
	"metadata" : SFNode,
	"thickness" : SFFloat,
	"thicknessTextureMapping" : SFString,
	"thicknessTexture" : SFNode,
	"attenuationDistance" : SFFloat,
	"attenuationColor" : SFColor,
},
"DiffuseTransmissionMaterialExtension" : {
	"metadata" : SFNode,
	"diffuseTransmission" : SFFloat,
	"diffuseTransmissionTextureMapping" : SFString,
	"diffuseTransmissionTexture" : SFNode,
	"diffuseTransmissionColor" : SFColor,
	"diffuseTransmissionColorTextureMapping" : SFString,
	"diffuseTransmissionColorTexture" : SFNode,
},
"component" : {
	"class" : SFString,
	"id" : SFString,
	"level" : SFInt32,
	"name" : SFString,
	"style" : SFString,
},
"connect" : {
	"class" : SFString,
	"id" : SFString,
	"nodeField" : SFString,
	"protoField" : SFString,
	"style" : SFString,
},
"EXPORT" : {
	"AS" : SFString,
	"class" : SFString,
	"id" : SFString,
	"localDEF" : SFString,
	"style" : SFString,
},
"ExternProtoDeclare" : {
	"appinfo" : SFString,
	"class" : SFString,
	"documentation" : SFString,
	"field" : MFNode,
	"id" : SFString,
	"name" : SFString,
	"style" : SFString,
	"url" : MFString,
},
"field" : {
	"accessType" : SFString,
	"appinfo" : SFString,
	"children" : MFNode,
	"class" : SFString,
	"documentation" : SFString,
	"id" : SFString,
	"name" : SFString,
	"style" : SFString,
	"type" : SFString,
	"value" : SFString,
},
"fieldValue" : {
	"children" : MFNode,
	"class" : SFString,
	"id" : SFString,
	"name" : SFString,
	"style" : SFString,
	"value" : SFString,
},
"head" : {
	"class" : SFString,
	"component" : MFNode,
	"id" : SFString,
	"meta" : MFNode,
	"style" : SFString,
	"unit" : MFNode,
},
"IMPORT" : {
	"AS" : SFString,
	"class" : SFString,
	"id" : SFString,
	"importedDEF" : SFString,
	"inlineDEF" : SFString,
	"style" : SFString,
},
"IS" : {
	"class" : SFString,
	"connect" : MFNode,
	"id" : SFString,
	"style" : SFString,
},
"meta" : {
	"class" : SFString,
	"content" : SFString,
	"dir" : SFString,
	"http-equiv" : SFString,
	"id" : SFString,
	"lang" : SFString,
	"name" : SFString,
	"scheme" : SFString,
	"style" : SFString,
},
"ProtoBody" : {
	"children" : MFNode,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"ProtoDeclare" : {
	"appinfo" : SFString,
	"class" : SFString,
	"documentation" : SFString,
	"id" : SFString,
	"name" : SFString,
	"ProtoBody" : SFNode,
	"ProtoInterface" : SFNode,
	"style" : SFString,
},
"ProtoInterface" : {
	"class" : SFString,
	"field" : MFNode,
	"id" : SFString,
	"style" : SFString,
},
"ROUTE" : {
	"class" : SFString,
	"fromField" : SFString,
	"fromNode" : SFString,
	"id" : SFString,
	"style" : SFString,
	"toField" : SFString,
	"toNode" : SFString,
},
"Scene" : {
	"children" : MFNode,
	"class" : SFString,
	"id" : SFString,
	"style" : SFString,
},
"unit" : {
	"category" : SFString,
	"class" : SFString,
	"conversionFactor" : SFDouble,
	"id" : SFString,
	"name" : SFString,
	"style" : SFString,
},
"X3D" : {
	"class" : SFString,
	"head" : SFNode,
	"id" : SFString,
	"profile" : SFString,
	"Scene" : SFNode,
	"style" : SFString,
	"version" : SFString,
},
};
