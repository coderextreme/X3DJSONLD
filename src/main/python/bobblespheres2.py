...
import random
import time
from matplotlib.animation import FuncAnimation # Use FuncAnimation as the main loop

if anari.has_cuda_capable_gpu():
   print('@pynari: detected cuda-capable GPU; using higher res and sample count')
   fb_size = (1600,800)
   # Reduced samples for better interactivity
...
start_time = time.time() # Global start time for animation

# --- MOUSE EVENT HANDLERS (they now only change state) ---
def on_press(event):
...
# --- MAIN UPDATE AND RENDER FUNCTION (for FuncAnimation) ---
def update(frame_num):
    # 1. Update sphere positions based on elapsed time
    ...
    # 2. Update camera position based on interactive state
...
# Create the animation object. It will call the 'update' function repeatedly.
# blit=False is more robust for this kind of full-frame redrawing.
