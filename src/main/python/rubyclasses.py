    ...
    def printClass(self):
        ...
        try:
            if self.node['name'] in ["X3DBoundedObject", "X3DPickableObject", "X3DPrototypeInstance", "X3DUrlObject", "X3DFogObject", "X3DMetadataObject", "X3DProgrammableShaderObject"] or self.node['name'].endswith("Node") and not self.node['name'] in ["SFNode", "MFNode", "X3DConcreteNode"]:
                superpackage = "sai"
            str += self.node['name'][0].upper() + self.node['name'][1:] + " = JavaUtilities.get_proxy_class(\"org.web3d.x3d."+superpackage+"."+package+"." + self.node['name'] + self.metaInfo + "\")\n"
        except:
            if self.node['type'] in ["X3DBoundedObject", "X3DPickableObject", "X3DPrototypeInstance", "X3DUrlObject", "X3DFogObject", "X3DMetadataObject", "X3DProgrammableShaderObject"] or self.node['type'].endswith("Node") and not self.node['type'] in ["SFNode", "MFNode", "X3DConcreteNode"]:
                superpackage = "sai"
            str += self.node['type'][0].upper() + self.node['type'][1:] + " = JavaUtilities.get_proxy_class(\"org.web3d.x3d."+superpackage+"."+package+"." + self.node['type'] + self.metaInfo + "\")\n"
        self.printed = True
        return str

code = 'require "java"\n'
code += 'require "../../../X3DJSAIL.4.0.full.jar"\n\n'

code += "ConfigurationProperties = JavaUtilities.get_proxy_class(\"org.web3d.x3d.jsail.ConfigurationProperties\")\n"
code += "CommentsBlock = JavaUtilities.get_proxy_class(\"org.web3d.x3d.jsail.Core.CommentsBlock\")\n"
