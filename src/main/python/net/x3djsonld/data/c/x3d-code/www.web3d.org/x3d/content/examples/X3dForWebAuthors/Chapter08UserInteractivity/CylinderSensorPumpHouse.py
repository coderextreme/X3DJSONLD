...
newModel=X3D(profile='Interactive',version='3.3',
  head=head(
    ...
    meta(content='A CylinderSensor changes the viewing position of a positive-displacement cylinder pump house.',name='description'),
    ...
    meta(content='http://X3dGraphics.com/examples/X3dForWebAuthors/Chapter08UserInteractivity/CylinderSensorPumpHouse.x3d',name='identifier'),
    meta(content='X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit',name='generator'),
    meta(content='Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html',name='generator'),
    meta(content='../license.html',name='license')]),
  Scene=Scene(
    children=[
    WorldInfo(title='CylinderSensorPumpHouse.x3d'),
    Background(skyColor=[(1,1,1)]),
    Viewpoint(DEF='AdjustableView',description='Adjustible Viewpoint',position=(0.02,0.45,5)),
    Transform(translation=(1.75,1,0),
      children=[
      CylinderSensor(DEF='Mover',description='click and drag to rotate',diskAngle=1.56,maxAngle=0.3,minAngle=-0.3),
      Shape(
        appearance=Appearance(
          material=Material(diffuseColor=(.9,.9,.9))),
        geometry=Box(size=(1,1.25,.02))),
      Transform(
        children=[
        Transform(
          children=[
          Transform(rotation=(1,0,0,1.57),
            children=[
            Shape(
              appearance=Appearance(DEF='Indicator',
        ...
        Transform(rotation=(1,0,0,-1.57),
          ...
          Transform(DEF='Handle',
            ...
            Transform(rotation=(0,0,1,-1.57),translation=(.25,0,0),
              children=[
              Shape(
                appearance=Appearance(USE='Indicator'),
                geometry=Cone(bottomRadius=.1,height=.6))])])])]),
    ...
    ROUTE(fromField='isActive',fromNode='PumpTouched',toField='enabled',toNode='PistonClock'),
...
print ("python x3d.py load successful for CylinderSensorPumpHouse.py")
