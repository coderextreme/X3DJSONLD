...
newModel=X3D(profile='Interchange',version='3.0',
  head=head(
    ...
    meta(content='Test of mapping a PixelTexture image to an IndexedFaceSet with two long equal length bounding box sides. The PixelTexture is RED (lower left quarter), YELLOW (lower right quarter), YELLOW (upper left quarter) and RED (upper right quarter). The bounding box for this geometry has two equal length long sides (Y and Z) and one short side (X). The PixelTexture will be projected onto the bounding box with the S (image horizontal) axis parallel with the Y axis, the T (image vertical) axis parallel to the Z axis. The result: the PixelTexture should appear "sideways" on the sides of the IndexedFaceSet, with the bottom of the logo along the Y axis, and image point (0,0) at the lower end of the Y axis. All remaining faces have the "smeared" end pixels of the front and rear textures each face of the box.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/Appearance/PixelTexture/twosidesbig.x3d',name='identifier'),
  ...
  Scene=Scene(
    ...
    Shape(
      ...
      geometry=IndexedFaceSet(colorPerVertex=False,coordIndex=[0,1,3,2,-1,4,5,7,6,-1,6,7,1,0,-1,2,3,5,4,-1,6,0,2,4,-1,1,7,5,3,-1],creaseAngle=0.5,
        ...
        coord=Coordinate(point=[(-1,2,2),(-1,-2,2),(1,2,2),(1,-2,2),(1,2,-2),(1,-2,-2),(-1,2,-2),(-1,-2,-2)])))])
) # X3D model complete
