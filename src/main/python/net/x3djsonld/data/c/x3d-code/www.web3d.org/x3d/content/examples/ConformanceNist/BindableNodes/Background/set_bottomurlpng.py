...
newModel=X3D(profile='Immersive',version='3.0',
  head=head(
    children=[
    meta(content='set_bottomurlpng.x3d',name='title'),
    ...
    meta(content='Test browser to switch between different backgrounds where each one contains a different bottomurl PNG image associated with it. The bottom image may be changed by clicking on the appropriate text as indicated. The test should generate a different PNG image each time the appropriate text is clicked. All images are on PNG format and are of the mountains of the High Sierra. A viewpoint node is set so that the image will be rotated -1.57 radians along the X axis the make it possible to see and toggle the images.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/BindableNodes/Background/set_bottomurlpng.x3d',name='identifier'),
    meta(content='Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html',name='generator'),
    meta(content='X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit',name='generator'),
    meta(content='../../license.html',name='license')]),
  Scene=Scene(
    children=[
    WorldInfo(title='set_bottomurlpng.x3d'),
    NavigationInfo(type=["EXAMINE","WALK","FLY","ANY"]),
    Group(
      children=[
      Viewpoint(DEF='the-view1',description='Bottom View',orientation=(1,0,0,-1.57),position=(0,0,23))]),
    Group(
      children=[
    ...
    Transform(rotation=(1,0,0,-1.57),translation=(0,-10,23),
      children=[
      Transform(scale=(0.02,0.02,0.02),translation=(-5.4,-2,0),
        ...
        Shape(
          ...
          geometry=Text(string=["CLICK TO SELECT BOTTOM URL IMAGE"],
...
print ("python x3d.py load successful for set_bottomurlpng.py")
