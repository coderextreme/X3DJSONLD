...
newModel=X3D(profile='Immersive',version='3.0',
  head=head(
    children=[
    meta(content='set_lefturlpng.x3d',name='title'),
    ...
    meta(content='Test browser to switch between different backgrounds where each one contains a different lefturl PNG image associated with it. The left image may be changed by clicking on the appropriate text as indicated. The test should generate a different PNG image each time the appropriate text is clicked. All images are on PNG format and are from the from mountains of the High Sierra. A viewpoint node is set so that the image will be rotated 1.57 radians along the Y axis the make it possible to see and toggle the images.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/BindableNodes/Background/set_lefturlpng.x3d',name='identifier'),
    meta(content='Vrml97ToX3dNist, http://ovrt.nist.gov/v2_x3d.html',name='generator'),
    meta(content='X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit',name='generator'),
    meta(content='../../license.html',name='license')]),
  Scene=Scene(
    children=[
    WorldInfo(title='set_lefturlpng.x3d'),
    ...
    Transform(DEF='big-trans',rotation=(0,1,0,1.57),translation=(-20,0,20),
      ...
      Transform(scale=(0.02,0.02,0.02),translation=(-5.4,-2,0),
        ...
        Shape(
          ...
          geometry=Text(string=["CLICK TO SELECT LEFT URL IMAGE"],
...
print ("python x3d.py load successful for set_lefturlpng.py")
