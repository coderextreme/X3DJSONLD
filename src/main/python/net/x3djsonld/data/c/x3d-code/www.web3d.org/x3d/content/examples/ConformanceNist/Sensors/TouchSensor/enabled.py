...
newModel=X3D(profile='Interactive',version='3.0',
  head=head(
    ...
    meta(content='A TouchSensor is associated with a white Sphere geometry. The enabled field is set to false. Because the TouchSensor is disabled, it will not generate fields when the pointing device is over the white Sphere, or when the pointing device indicates the geometry. As a result, the isActive event routed to the "on" field of a PointLight will not turn the light on when the pointing device indicates the white Sphere geometry.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/TouchSensor/enabled.x3d',name='identifier'),
  ...
  Scene=Scene(
    ...
    Group(
      ...
      Group(
        children=[
        TouchSensor(DEF='TOUCH',enabled=False),
        Shape(DEF='BULB',
          appearance=Appearance(
            material=Material(),),
