...
newModel=X3D(profile='Interactive',version='3.0',
  head=head(
    ...
    meta(content='Test the function of SphereSensors field "enabled". An IndexedFaceSet geometry has an associated SphereSensor node, with the "enabled" field set to false, and its rotation_changed field routed to the geometrys Transform node. As a result, despite the routing of the rotation_changed event from the SphereSensor node, the geometry should not rotate when the pointing device is "dragged" over the geometry.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/SphereSensor/enabled.x3d',name='identifier'),
  ...
  Scene=Scene(
    ...
    Group(
      ...
      Transform(
        children=[
        SphereSensor(DEF='SPHERE_SENSOR',description='click and drag to rotate',enabled=False),
        Shape(DEF='AXIS',
          appearance=Appearance(
            material=Material(emissiveColor=(1,1,1))),
          geometry=IndexedLineSet(coordIndex=[0,1,-1,0,2,-1,0,3,-1],
            coord=Coordinate(point=[(0,0,0),(3,0,0),(0,3,0),(0,0,3)]))),
        Transform(DEF='BOX',
          children=[
          Shape(
            appearance=Appearance(
