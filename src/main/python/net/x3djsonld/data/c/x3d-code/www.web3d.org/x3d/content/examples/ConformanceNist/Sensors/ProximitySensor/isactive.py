...
newModel=X3D(profile='Interactive',version='3.0',
  head=head(
    ...
    meta(content='Test isActive event. A ProximitySensor bounding box surrounds a Sphere geometry. Entering the ProximitySensor bounding box (represented by a white IndexedLineSet box) results in the ProximitySensor isActive event being sent to a TimeSensor node to begin a color interpolation of the Sphere geometry. Exiting the ProximitySensor bounding box sends the isActive false event to the TimeSensor to stop the color interpolation. The Sphere geometry should cycle from red to green to blue, then repeat the cycle until the viewer avatar leaves the ProximitySensor bounding box.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/ProximitySensor/isactive.x3d',name='identifier'),
  ...
  Scene=Scene(
    ...
    ROUTE(fromField='isActive',fromNode='PROX_SENSOR',toField='enabled',toNode='TIME'),
