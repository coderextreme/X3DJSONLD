...
newModel=X3D(profile='Interactive',version='3.0',
  head=head(
    ...
    meta(content='Test if a TouchSensor generates an isActive true event when the pointing device indicates a white Sphere geometry. The isActive event of the TouchSensor associated with the white Sphere is routed to the "on" field of a PointLight. When the pointing device indicates the white Sphere (by pressing down on the primary pointing device button while its bearing intersects the white Sphere geometry), the PointLight inside of sphere should turn on, illuminating the red Spheres around it. When the primary pointing device button is released, the PointLight should turn off.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/Sensors/TouchSensor/isactive.x3d',name='identifier'),
  ...
  Scene=Scene(
    ...
    ROUTE(fromField='isActive',fromNode='TOUCH',toField='on',toNode='LIGHT')])
) # X3D model complete
