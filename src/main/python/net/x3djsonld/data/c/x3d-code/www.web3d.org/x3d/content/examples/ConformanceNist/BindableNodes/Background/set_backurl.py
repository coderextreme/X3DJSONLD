...
newModel=X3D(profile='Immersive',version='3.0',
  head=head(
    children=[
    meta(content='set_backurl.x3d',name='title'),
    ...
    meta(content='Test browser to switch between different backgrounds where each one contains a different backurl JPEG image associated with it. The back image may be changed by clicking on the appropriate text as indicated. The test should generate a different JPEG image each time the appropriate text is clicked. All images are on JPEG format and are from the from mountains of the High Sierra. A viewpoint node is set so that the image will be rotated 3.14 radians along the Y axis the make it possible to see and toggle the images.',name='description'),
    meta(content='https://www.web3d.org/x3d/content/examples/ConformanceNist/BindableNodes/Background/set_backurl.x3d',name='identifier'),
    meta(content='X3D-Edit 3.3, https://savage.nps.edu/X3D-Edit',name='generator'),
    meta(content='../../license.html',name='license')]),
  Scene=Scene(
    children=[
    WorldInfo(title='set_backurl.x3d'),
    NavigationInfo(type=["EXAMINE","WALK","FLY","ANY"]),
    Group(
      children=[
      Viewpoint(DEF='the-view1',description='Back View',orientation=(0,1,0,3.14),position=(-10,0,20))]),
    Group(
      children=[
    ...
    Transform(rotation=(0,1,0,3.14),translation=(-10,0,30),
      children=[
      Transform(scale=(0.02,0.02,0.02),translation=(-5.4,-2,0),
        ...
        Shape(
          ...
          geometry=Text(string=["CLICK TO SELECT BACK URL IMAGE"],
...
print ("python x3d.py load successful for set_backurl.py")
