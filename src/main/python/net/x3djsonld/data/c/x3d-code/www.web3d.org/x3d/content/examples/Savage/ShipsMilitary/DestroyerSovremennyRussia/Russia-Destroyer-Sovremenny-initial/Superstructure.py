...
newModel=X3D(version='3.0',profile='Immersive',
  head=head(
    ...
    meta(content='Superstructure for Sovremmeny Ship. Calls YardArm.wrl inline',name='description'),
    meta(content='Superstructure.x3d',name='identifier'),
    ...
    Group(
      ...
      Transform(DEF='RadarAntennaPole',
        ...
        Transform(rotation=(0,1,0,3.14),scale=(.8,.7,.7),translation=(2.2,4.1,0),
          children=[
          Shape(USE='Antenna')])]),
      Transform(rotation=(0,1,0,1.57),scale=(.85,.5,.6),translation=(1.4,4.3,0),
        children=[
        Inline(url=["YardArm.wrl","YardArm.x3d"])])])])
) # X3D model complete
