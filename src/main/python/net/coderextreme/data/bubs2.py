...
meta(name="converted", content="Sat, 02 Sep 2023 10:59:18 GMT")
...
], sourceCode="""['ecmascript:\r', 'function initialize() {\r', '    velocity = new SFVec3f(Math.random() * 0.25 - 0.125, Math.random() * 0.25 - 0.125, Math.random() * 0.25 - 0.125);\r', '\r', '    scalvel = new SFVec3f(Math.random() * 0.4, Math.random() * 0.4, Math.random() * 0.4);\r', '}\r', '\r', 'function set_fraction(value) {\r', "    if (typeof translation === 'undefined') {\r", '\t\ttranslation = new SFVec3f(0, 0, 0);\r', '    }\r', "    if (typeof velocity === 'undefined') {\r", '\t\tvelocity = new SFVec3f(0, 0, 0);\r', '    }\r', "    if (typeof scalvel === 'undefined') {\r", '\t\tscalvel = new SFVec3f(0, 0, 0);\r', '    }\r', "    if (typeof scale === 'undefined') {\r", '\t\tscale = new SFVec3f(1, 1, 1);\r', '    }\r', '    translation = new SFVec3f(\ttranslation.x + velocity.x, translation.y + velocity.y, translation.z + velocity.z);\r', '    scale = new SFVec3f(scale.x + scalvel.x, scale.y + scalvel.y, scale.z + scalvel.z);\r', '    // if you get to far away or too big, explode\r', '    if ( Math.abs(translation.x) > 256) {\r', '\ttranslation.x = 0;\r', '\tinitialize();\r', '    }\r', '    if ( Math.abs(translation.y) > 256) {\r', '\ttranslation.y = 0;\r', '\tinitialize();\r', '    }\r', '    if ( Math.abs(translation.z) > 256) {\r', '\ttranslation.z = 0;\r', '\tinitialize();\r', '    }\r', '    if (Math.abs(scale.x) > 20) {\r', '\tscale.x = scale.x/20;\r', '\ttranslation.x = 0;\r', '\tinitialize();\r', '    }\r', '    if (Math.abs(scale.y) > 20) {\r', '\tscale.y = scale.y/20;\r', '\ttranslation.y = 0;\r', '\tinitialize();\r', '    }\r', '    if (Math.abs(scale.z) > 20) {\r', '\tscale.z = scale.z/20;\r', '\ttranslation.z = 0;\r', '\tinitialize();\r', '    }\r', '}']""",),
TimeSensor(DEF="bubbleClock", cycleInterval=10, loop=True)]),
ROUTE(fromNode="bounce", fromField="translation", toNode="body_trans", toField="translation"),
ROUTE(fromNode="bounce", fromField="scale", toNode="body_trans", toField="scale"),
