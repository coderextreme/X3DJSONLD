...
meta(name="modified", content="3 July 2023"),
...
Anchor(DEF="siteAnchor", description="select for X3D Java SAI Library (X3DJSAIL) description", url=["../X3DJSAIL.html","https://www.web3d.org/specifications/java/X3DJSAIL.html"], children=[
Shape(DEF="BoxShape", 
appearance=
Appearance(
material=
...
Shape(
metadata=(
MetadataString(name="findThisNameValue", DEF="FindableMetadataStringTest", value=["test case"])), 
appearance=
...
Comment(value=''' SFBool default=false, true=true, false=false, negate()=true '''),
...
Comment(value=''' regex test SFVec3f().matches("1 2 3")=true, regex test SFVec3f().matches("1 2 3 4")=false, regex test (SFRotation.matches("0 0 0 0")=true, failure detecting illegal (zero axis) rotation value '''),
]),
...
Comment(value=''' Test success: Anchor.isNode()=true, siteAnchor.isNode()=true '''),

Comment(value=''' Test success: Anchor.isStatement()=false, siteAnchor.isStatement()=false '''),

Comment(value=''' Test success: ROUTE.isNode()=false, orbitPositionROUTE.isNode()=false '''),

Comment(value=''' Test success: ROUTE.isStatement()=true, orbitPositionROUTE.isStatement()=true '''),
...
DISEntityTypeMapping()]),
LoadSensor(children=[
Comment(value=''' Contained nodes typically must be USE references for nodes previously DEFined in the scene '''),

Comment(value=''' The following nodes are test cases for all X3DUrlObject nodes '''),
