    ...
    children=[
        ...
        ProtoDeclare(
          name="flower", ProtoBody=ProtoBody(
            children=[
                Transform(
                  DEF="animate_transform", children=[
                      Shape(
                            ...
                            material=Material(
                                ...
                                ComposedShader(
                                  DEF="x3dom", language="GLSL", field=[
                                      ...
                                      field(name="pdelta", type="SFFloat", accessType="inputOutput", value=0)], children=[#<field name='cube' type='SFNode' accessType="inputOutput"> <ComposedCubeMapTexture USE="texture"/> </field>
], parts=(
                                      ShaderPart(
                                        url=["../shaders/x3dom_flowers_chromatic.vs","https://coderextreme.net/X3DJSONLD/src/main/shaders/x3dom_flowers_chromatic.vs"], type="VERTEX"),
                                      ShaderPart(
                                        url=["../shaders/common.fs","https://coderextreme.net/X3DJSONLD/src/main/shaders/common.fs"], type="FRAGMENT"))),
                                ComposedShader(
                                  DEF="x_ite", language="GLSL", field=[
                                      ...
                                      field(name="pdelta", type="SFFloat", accessType="inputOnly", value=0)], parts=(
                                      ShaderPart(
                                        url=["../shaders/x_ite_flowers_chromatic.vs","https://coderextreme.net/X3DJSONLD/src/main/shaders/x_ite_flowers_chromatic.vs"], type="VERTEX"),
                                      ShaderPart(
                                        url=["../shaders/x_ite.fs","https://coderextreme.net/X3DJSONLD/src/main/shaders/x_ite.fs"], type="FRAGMENT"))))), geometry=Sphere(
                            ))]),
                Script(
                  DEF="Animate", field=[
                      ...
                      field(name="pdelta", type="SFFloat", accessType="inputOutput", value=0.5)], #['', '', 'ecmascript:', '', '\t\t\tfunction initialize() {', '\t\t\t    translation = new SFVec3f(0, 0, 0);', '\t\t\t    velocity = new SFVec3f(', '\t\t\t    \tMath.random() - 0.5,', '\t\t\t\tMath.random() - 0.5,', '\t\t\t\tMath.random() - 0.5);', '\t\t\t}', '\t\t\tfunction set_fraction() {', '\t\t\t    translation = new SFVec3f(', '\t\t\t    \ttranslation.x + velocity.x,', '\t\t\t\ttranslation.y + velocity.y,', '\t\t\t\ttranslation.z + velocity.z);', '\t\t\t    for (var j = 0; j <= 2; j++) {', '\t\t\t\t    if (Math.abs(translation.x) > 10) {', '\t\t\t\t\tinitialize();', '\t\t\t\t    } else if (Math.abs(translation.y) > 10) {', '\t\t\t\t\tinitialize();', '\t\t\t\t    } else if (Math.abs(translation.z) > 10) {', '\t\t\t\t\tinitialize();', '\t\t\t\t    } else {', '\t\t\t\t\tvelocity.x += Math.random() * 0.2 - 0.1;', '\t\t\t\t\tvelocity.y += Math.random() * 0.2 - 0.1;', '\t\t\t\t\tvelocity.z += Math.random() * 0.2 - 0.1;', '\t\t\t\t    }', '\t\t\t    }', '\t\t\t    animate_flowers();', '\t\t\t}', '', '\t\t\tfunction animate_flowers(fraction, eventTime) {', '\t\t\t\tvar choice = Math.floor(Math.random() * 4);', '\t\t\t\tswitch (choice) {', '\t\t\t\tcase 0:', '\t\t\t\t\ta += Math.random() * 0.2 - 0.1;', '\t\t\t\t\tbreak;', '\t\t\t\tcase 1:', '\t\t\t\t\tb += Math.random() * 0.2 - 0.1;', '\t\t\t\t\tbreak;', '\t\t\t\tcase 2:', '\t\t\t\t\tc += Math.random() * 2 - 1;', '\t\t\t\t\tbreak;', '\t\t\t\tcase 3:', '\t\t\t\t\td += Math.random() * 2 - 1;', '\t\t\t\t\tbreak;', '\t\t\t\t}', '\t\t\t\ttdelta = tdelta + 0.5;', '\t\t\t\tpdelta = pdelta + 0.5;', '\t\t\t\tif (a > 1) {', '\t\t\t\t\ta =  0.5;', '\t\t\t\t}', '\t\t\t\tif (b > 1) {', '\t\t\t\t\tb =  0.5;', '\t\t\t\t}', '\t\t\t\tif (c < 1) {', '\t\t\t\t\tc =  4;', '\t\t\t\t}', '\t\t\t\tif (d < 1) {', '\t\t\t\t\td =  4;', '\t\t\t\t}', '\t\t\t\tif (c > 10) {', '\t\t\t\t\tc = 4;', '\t\t\t\t}', '\t\t\t\tif (d > 10) {', '\t\t\t\t\td = 4;', '\t\t\t\t}', '\t\t\t}', '', '']
    
    ),
                TimeSensor(
                  DEF="TourTime", cycleInterval=5, loop=True),
                ROUTE(
                  fromNode="TourTime", fromField="fraction_changed", toNode="Animate", toField="set_fraction"),
                ROUTE(
                  fromNode="Animate", fromField="translation_changed", toNode="animate_transform", toField="set_translation"),
                ROUTE(
                  fromNode="Animate", fromField="a", toNode="x_ite", toField="a"),
                ROUTE(
                  fromNode="Animate", fromField="b", toNode="x_ite", toField="b"),
                ROUTE(
                  fromNode="Animate", fromField="c", toNode="x_ite", toField="c"),
                ROUTE(
                  fromNode="Animate", fromField="d", toNode="x_ite", toField="d"),
                ROUTE(
                  fromNode="Animate", fromField="pdelta", toNode="x_ite", toField="pdelta"),
                ROUTE(
                  fromNode="Animate", fromField="tdelta", toNode="x_ite", toField="tdelta"),
                ROUTE(
                  fromNode="Animate", fromField="a", toNode="x3dom", toField="a"),
                ROUTE(
                  fromNode="Animate", fromField="b", toNode="x3dom", toField="b"),
                ROUTE(
                  fromNode="Animate", fromField="c", toNode="x3dom", toField="c"),
                ROUTE(
                  fromNode="Animate", fromField="d", toNode="x3dom", toField="d"),
                ROUTE(
                  fromNode="Animate", fromField="pdelta", toNode="x3dom", toField="pdelta"),
                ROUTE(
                  fromNode="Animate", fromField="tdelta", toNode="x3dom", toField="tdelta")])),
        ProtoInstance(
          name="flower"),
        ProtoInstance(
          name="flower"),
