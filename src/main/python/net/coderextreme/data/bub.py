...
X3D(
  profile="Immersive", version="4.0", head=head(  children=[
    ...
    children=[
        ...
        Viewpoint(
          position=(0,0,20), description="Look at the bubbles flying"),
        ProtoDeclare(
            ...
            children=[
                Transform(
                  DEF="transform", children=[
                      Shape(
                        DEF="myShape", appearance=Appearance(
                            material=Material(
...
#<ComposedShader DEF='instant' language="GLSL"> <field name='cube' type='SFInt32' accessType="inputOutput" value='0'/> <field name='chromaticDispertion' type='SFVec3f' accessType="inputOutput" value='0.98 1.0 1.033'/> <field name='bias' type='SFFloat' accessType="inputOutput" value='0.5'/> <field name='scale' type='SFFloat' accessType="inputOutput" value='0.5'/> <field name='power' type='SFFloat' accessType="inputOutput" value='2.0'/> <ShaderPart url='"../shaders/instant.vs" "https://coderextreme.net/X3DJSONLD/src/main/shaders/instant.vs"' type='VERTEX'></ShaderPart> <ShaderPart url='"../shaders/pc_bubbles.fs" "https://coderextreme.net/X3DJSONLD/src/main/shaders/pc_bubbles.fs"' type='FRAGMENT'></ShaderPart> </ComposedShader>
                                ...
                                ComposedShader(
                                  DEF="x3dom", language="GLSL", field=[
                                      ...
                                      field(name="power", type="SFFloat", accessType="inputOutput", value=2.0)], parts=(
                                      ShaderPart(
                                        url=["../shaders/x3dom.vs","https://coderextreme.net/X3DJSONLD/src/main/shaders/x3dom.vs"], type="VERTEX"),
                                      ShaderPart(
                                        url=["../shaders/pc_bubbles.fs","https://coderextreme.net/X3DJSONLD/src/main/shaders/pc_bubbles.fs"], type="FRAGMENT"))),
                                ComposedShader(
                                  DEF="x_ite", language="GLSL", field=[
                                      ...
                                      ShaderPart(
                                        url=["../shaders/x_itebubbles.fs","https://coderextreme.net/X3DJSONLD/src/main/shaders/x_itebubbles.fs"], type="FRAGMENT"))))), geometry=Sphere(
                            ))]),
                Script(
                  DEF="Bounce", field=[
                      field(name="translation", accessType="inputOutput", type="SFVec3f", value=(0,0,0)),
                      field(name="velocity", accessType="inputOutput", type="SFVec3f", value=(0,0,0)),
                      field(name="set_fraction", accessType="inputOnly", type="SFTime")], #['', 'ecmascript:', '\t\t\tfunction initialize() {', '\t\t\t    translation = new SFVec3f(0, 0, 0);', '\t\t\t    velocity = new SFVec3f(', '\t\t\t    \tMath.random() - 0.5,', '\t\t\t\tMath.random() - 0.5,', '\t\t\t\tMath.random() - 0.5);', '\t\t\t}', '\t\t\tfunction set_fraction() {', '\t\t\t    translation = new SFVec3f(', '\t\t\t    \ttranslation.x + velocity.x,', '\t\t\t\ttranslation.y + velocity.y,', '\t\t\t\ttranslation.z + velocity.z);', '\t\t\t    if (Math.abs(translation.x) > 10) {', '\t\t\t\tinitialize();', '\t\t\t    } else if (Math.abs(translation.y) > 10) {', '\t\t\t\tinitialize();', '\t\t\t    } else if (Math.abs(translation.z) > 10) {', '\t\t\t\tinitialize();', '\t\t\t    } else {', '\t\t\t\tvelocity.x += Math.random() * 0.2 - 0.1;', '\t\t\t\tvelocity.y += Math.random() * 0.2 - 0.1;', '\t\t\t\tvelocity.z += Math.random() * 0.2 - 0.1;', '\t\t\t    }', '\t\t\t}', '', '']
    
    ),
                TimeSensor(
                  DEF="TourTime", cycleInterval=0.15, loop=True),
                ROUTE(
                  fromNode="TourTime", fromField="cycleTime", toNode="Bounce", toField="set_fraction"),
                ROUTE(
                  fromNode="Bounce", fromField="translation_changed", toNode="transform", toField="set_translation")])),
        ProtoInstance(
          name="Bubble"),
        ProtoInstance(
          name="Bubble"),
