...
X3D(
  profile="Immersive", version="3.3", head=head(  children=[
    ...
    children=[
        ProtoDeclare(
          name="ArchPrototype", appinfo="Create an arch. Can modify general parameters: clearSpanWidth, riseHeight, depth, topAbutmentHeight, pierWidth, pierHeight. - Possibility to create shapes related to an arch: ArchHalf; IntradosOnly; ArchFilled; ArchHalfFilled; Lintel. See the reference file ArchModelingDiagrams.pdf to find further information. See also ArchPrototypeScript_more_readable.js.js.", ProtoInterface=ProtoInterface(
            children=[#COLOR OF ARCH
#INPUT PARAMETERS
...
#Parameters to create to create shapes related to arch: put true to apply
                ...
                field(name="lintel", accessType="initializeOnly", appinfo="lintel: no arc is rendered, but a lintel: topAbutmentHeight on pierHeight, total height is pierHeight + topAbutmentHeight, if needed apply archHalf=true.", type="SFBool", value=False)]), ProtoBody=ProtoBody(
            children=[#First node determines node type of this prototype
#IndexedFaceset creates arch

                Transform(
                  DEF="ArchTransform", children=[
                      Shape(
                        DEF="Arch", children=[#note that convex='false' (meaning concave geometry) is crucial for this IFS of a geometric chord to render properly
], geometry=IndexedFaceSet(
                            DEF="ArchIndex", convex=False, solid=False, coord(
                              Coordinate(
                                DEF="ArchChord"))), appearance=Appearance(
                            material=Material(
                                DEF="MaterialNode", IS=IS(
                                  connect=[
                                      connect(nodeField="emissiveColor", protoField="emissiveColor"),
                                      connect(nodeField="diffuseColor", protoField="diffuseColor")]))))]),#Subsequent nodes do not render, but still must be a valid X3D subgraph
#This embedded Script provides the X3D author with additional visibility and control over prototype inputs and outputs

                Script(
                  DEF="ArchPrototypeScript", url=["../node/ArchPrototypeScript.js","https://coderextreme.net/X3DJSONLD/src/main/node/ArchPrototypeScript.js"], children=[#INPUT PARAMETERS
#General parameters
#Parameters to create to create shapes related to arch: put true to apply
#OUTPUT PARAMETERS
], field=[
                      ...
                      field(name="indexOut", accessType="outputOnly", appinfo="send computed indices to the IndexedFaceSet node", type="MFInt32")], IS=IS(
                    connect=[
                ...
                ROUTE(
                  fromField="computedScale", fromNode="ArchPrototypeScript", toField="scale", toNode="ArchTransform"),
                ROUTE(
                  fromField="pointOut", fromNode="ArchPrototypeScript", toField="point", toNode="ArchChord"),
                ROUTE(
                  fromField="indexOut", fromNode="ArchPrototypeScript", toField="set_coordIndex", toNode="ArchIndex")])),
        ProtoInstance(
          name="ArchPrototype", DEF="ArchInstance", fieldValue=[
              ...
              fieldValue(name="pierHeight", value=2)]),#Add any ROUTEs here that connect ProtoInstance to/from prior nodes in Scene (and outside of ProtoDeclare)
