...
head=head(children=[meta(name="title", content="HeadsUpDisplayPrototype.x3d"),
...
ProtoDeclare(name="HeadsUpDisplay", appinfo="HeadsUpDisplay positions child geometry in screen space, movable by the user", 
ProtoInterface=ProtoInterface(field=[field(name="children", accessType="inputOutput", appinfo="Displayed subscene positioned as a HUD.", type="MFNode", children=[
Comment(value=''' default is null array of nodes '''),
]),
field(name="dragChildren", accessType="inputOutput", appinfo="Additional HUD geometry which can be touched and dragged for repositioning. If this geometry goes offscreen (perhaps due to screen resizing) then it snaps back to original position.", type="MFNode", children=[
Comment(value=''' default is null array of nodes '''),
]),
...
field(name="traceEnabled", accessType="initializeOnly", appinfo="Enable/disable console output for troubleshooting.", type="SFBool", value=False)
]), 
ProtoBody=ProtoBody(children=[
Group(children=[
ProximitySensor(DEF="WhereSensor", size=((1000000000,1000000000,1000000000)), 
IS=IS(connect=[connect(nodeField="center", protoField="locationOffset")
])),
Transform(DEF="FixedLocation", children=[
Transform(DEF="MovableLocation", children=[
Transform(DEF="LocationOffset", 
IS=IS(connect=[connect(nodeField="translation", protoField="locationOffset")
]), children=[
Transform(translation=((0,0,-10)), children=[
Group(
IS=IS(connect=[connect(nodeField="children", protoField="children")
])),
Group(DEF="PlaneMovementSensorGroup", children=[
Group(DEF="DragGeometry", 
IS=IS(connect=[connect(nodeField="children", protoField="dragChildren")
])),
PlaneSensor(DEF="PlaneMovementSensor", description="click and drag to move interface", 
IS=IS(connect=[connect(nodeField="offset", protoField="locationOffset")
])),
VisibilitySensor(DEF="MovementVisibilitySensor"),
Script(DEF="VisibilityControlScript", field=[field(name="traceEnabled", accessType="initializeOnly", type="SFBool"),
...
field(name="translationOffsetChanged", accessType="outputOnly", type="SFVec3f")
], 
IS=IS(connect=[connect(nodeField="traceEnabled", protoField="traceEnabled")
]), sourceCode="""['', '', 'ecmascript:', '', 'function tracePrint (text)', '{', "\tif (traceEnabled) Browser.print ('[HeadsUpDisplayPrototype VisibilityControlScript] ' + text);", '}', 'function setIsVisible (value, timeStamp)', '{', '\tisVisible = value;', "\ttracePrint('isVisible=' + value);", '}', 'function setPlaneSensorIsActive (value, timeStamp)', '{', "\ttracePrint('PlaneSensor isActive=' + value);", '', '\tif (value == false)', '\t{', "\t\ttracePrint('planeSensorTranslation=' + planeSensorTranslation);", '\t\tif (isVisible)', '\t\t{', '\t\t\ttranslationChanged = planeSensorTranslation;', '\t\t}', '\t\telse', '\t\t{', '\t\t\t// fell off screen, reset to center', '\t\t\ttranslationChanged = new SFVec3f(0, 0, 0);', '\t\t\ttranslationOffsetChanged  = new SFVec3f(0, 0, 0);', '\t\t}', '\t}', '}', 'function setPlaneSensorTranslation (value, timeStamp)', '{', '\tplaneSensorTranslation = value;', "\ttracePrint('planeSensorTranslation=' + value);", '}', '', '']""",),
...
Anchor(description="HeadsUpDisplayExample", parameter=("target=_blank"), url=["HeadsUpDisplayExample.x3d","https://savage.nps.edu/Savage/Tools/HeadsUpDisplays/HeadsUpDisplayrExample.x3d","HeadsUpDisplayExample.wrl","https://savage.nps.edu/Savage/Tools/HeadsUpDisplays/HeadsUpDisplayExample.wrl"], children=[
Shape(
appearance=
Appearance(
material=
