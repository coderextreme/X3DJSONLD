...
#['', '', 'ecmascript:', '', 'function tracePrint (text)', '{', "\tif (traceEnabled) Browser.print ('[HeadsUpDisplayPrototype VisibilityControlScript] ' + text);", '}', 'function setIsVisible (value, timeStamp)', '{', '\tisVisible = value;', "\ttracePrint('isVisible=' + value);", '}', 'function setPlaneSensorIsActive (value, timeStamp)', '{', "\ttracePrint('PlaneSensor isActive=' + value);", '', '\tif (value == false)', '\t{', "\t\ttracePrint('planeSensorTranslation=' + planeSensorTranslation);", '\t\tif (isVisible)', '\t\t{', '\t\t\ttranslationChanged = planeSensorTranslation;', '\t\t}', '\t\telse', '\t\t{', '\t\t\t// fell off screen, reset to center', '\t\t\ttranslationChanged = new SFVec3f(0, 0, 0);', '\t\t\ttranslationOffsetChanged  = new SFVec3f(0, 0, 0);', '\t\t}', '\t}', '}', 'function setPlaneSensorTranslation (value, timeStamp)', '{', '\tplaneSensorTranslation = value;', "\ttracePrint('planeSensorTranslation=' + value);", '}', '', '']
),
